...
> - 용어를 하나로 통일했습니다. `continuous integration` 은 **지속적 통합**, `continuous delivery` 는 **지속적 전달**, `continuous deployment` 는 **지속적 배포**로 쓰고 약어는 **CI 하나만** 씁니다. `infrastructure as code` 는 **코드형 인프라**, `change set` 은 **변경 세트**, `deployment configuration` 은 **배포 구성**입니다.
> - 외부 문서로 판별할 수 없는 항목은 표기를 붙이지 않고 본문에서 짚은 뒤 [11장](#11-교재-대비-변경-사항)에 모았습니다.
> - 템플릿과 CLI 예시를 이 문서에 실을 때 **깨진 표기와 지원 종료된 런타임을 교정**했습니다. 무엇을 고쳤는지는 각 절의 교정 표에 밝혀 두었습니다.
...
일부 원본 자료는 이 네 항목의 문구를 두 곳에서 다르게 씁니다. 2번은 "DevOps**가** … 개발 **관행**"과 "DevOps**에서** … 개발 **방식**"으로 갈리고, 3번은 약어 표기가 다릅니다. 이 문서는 한 가지 문구로 통일했습니다([11.1절](#111-교재와-다른-점)).

네 번째 목표인 "다양한 **AWS SAM** 배포 전략 설명"에 대응하는 실무 내용, 즉 **SAM 템플릿에서 배포 전략을 선언하는 방법**은 [10장](#10-배포-전략)에 공식 문서로 정리했습니다. 이 장은 배포 유형·컴퓨팅 플랫폼·사전 정의 구성을 먼저 다루고, 그다음 이를 SAM 템플릿에서 선언하는 방법을 채웁니다.
...
모듈 12의 이름은 자료에 따라 "애플리케이션 사용자에게 액세스 권한 부여"와 "내 애플리케이션의 사용자에게 액세스 권한 부여하기"로 조금씩 다르게 적힙니다([11.1절](#111-교재와-다른-점)).

### 이 모듈에서 다루는 것

이 모듈은 다음 순서로 일곱 개 주제를 다룹니다.

| 주제 | 이 문서 |
|---|---|
| 표지 · 어젠다 · 모듈 목표 | [1장](#1-모듈-개요) |
| 배포 계획 수립 및 방식 | [2장](#2-devops) · [3장](#3-코드형-인프라와-배포-도구) |
| AWS Serverless Application Model(AWS SAM) | [4장](#4-aws-sam) · [5장](#5-sam-템플릿) · [6장](#6-sam-리소스-유형과-커넥터) · [7장](#7-api-액세스-제어) |
| AWS SAM을 이용한 배포 | [8장](#8-sam-cli-설치와-로컬-테스트) · [9장](#9-빌드와-배포) |
| 배포 전략 | [10장](#10-배포-전략) |
| 데모: AWS SAM | [9.9절](#99-데모-절차) |
| 요약 | [1.2절](#12-용어-정리) |

이 모듈은 **전반부와 후반부의 성격이 크게 다릅니다.** 전반부는 DevOps 개념이고, 후반부는 AWS SAM 실무로 YAML 템플릿과 CLI 명령이 등장합니다.

이 모듈에는 실습(Lab)이 없고 데모만 있습니다. 이 문서는 데모 절차를 [9.9절](#99-데모-절차)에 명령 단위로 정리했습니다.
...
| `sam build --use-container` | "일부 언어(예: .NET 또는 Python)에서는 사용할 수 없다"는 설명이 돌기도 함 | 현재 `sam build` 참조 문서에는 **언어·런타임에 따라 못 쓴다는 서술이 없습니다.** 오히려 `--build-image` 설명이 Python 빌드 이미지를 `--use-container` 와 함께 쓰는 예로 듭니다. 명시된 제약은 **`--build-in-source` 와의 비호환**뿐입니다 ([9.2절](#92-sam-build-와-컨테이너-빌드)) |
...
| Lambda 런타임 | 예시에 `python3.8` 과 `python3.9` 가 쓰임 | **둘 다 지원 종료되었습니다.** `python3.8` 은 2024년 10월 14일, `python3.9` 는 2025년 12월 15일입니다 ([11.3절](#113-비권장지원-종료된-항목)) |
...
DevOps · 지속적 통합 · 코드형 인프라 · 변경 세트 · 배포 구성처럼 이 모듈에서 새로 나오는 용어를 공식 문서 정의로 정리했습니다.
...
전통적 배포 방식은 여섯 가지 문제를 안고 있습니다. 이 목록이 이 모듈 전체의 출발점이고, 뒤에서 다루는 도구·전략은 각각 이 문제 중 하나를 겨냥합니다. 마지막 열에 이 문서의 대응 절을 적어 두었습니다.
...
이 여섯 문제의 답은 DevOps에서 찾습니다. 특히 4번 "롤백 계획 없음"을 어떻게 구성하는지는 [9.5절](#95-변경-세트와-롤백)의 CloudFormation 롤백 옵션과 [10.8절](#108-롤백-경로-정리)의 경보 기반 자동 롤백에서 다룹니다.

### 2.2 DevOps 문화

DevOps 문화는 여덟 항목으로 정리됩니다. 일부 원본 자료는 같은 항목의 이름을 조금씩 다르게 적으므로(예: "고객의 요구에 집중" vs "고객 요구 사항 중심") 이 문서는 한 가지 이름으로 통일했습니다([11.1절](#111-교재와-다른-점)).
...
AWS 제품 페이지 `What is DevOps?` 의 `DevOps Cultural Philosophy` 절이 기술하는 내용은 위 여덟 항목 중 여러 개와 방향이 같습니다.
...
DevOps는 지속적 통합 · 지속적 전달 · 지속적 배포 세 프로세스로 나뉩니다. **두 가지를 교정합니다.**

| 프로세스 | 자주 쓰이는 설명 | 확인된 내용 |
|---|---|---|
...
| 지속적 전달 | 프로덕션에 배포하기 **전에 수동 승인 단계를 거쳐야** 한다고 설명되기도 함 | 🔄 **수동 승인은 필수 조건이 아닙니다.** 문서는 프로덕션으로의 최종 푸시 전에 **사람, 자동화된 테스트 또는 비즈니스 규칙** 중 하나가 최종 푸시 시점을 결정한다고 기술합니다. 또 모든 성공적인 변경을 즉시 릴리스할 수 있지만 **모든 변경을 바로 릴리스해야 하는 것은 아니라고** 명시합니다 |
...
**약어 배정에도 주의가 필요합니다.** 지속적 통합에 `CI`, 지속적 배포에 `CD` 를 배정하는 식으로 쓰면, 영어에서 `continuous delivery` 와 `continuous deployment` 가 모두 `CD` 로 줄여 쓰이므로 혼동을 부릅니다. **이 문서는 약어를 `CI` 하나만 쓰고 나머지 둘은 전체 용어로 적습니다.**

세 프로세스를 다루면서도 [2.5절](#25-devops-방식)의 DevOps 방식 여섯 개에는 **지속적 배포가 없어**, 같은 개념군에서 항목 수가 어긋나기도 합니다([11.1절](#111-교재와-다른-점)).
...
DevOps에는 이점 여섯 가지가 있습니다. **이 여섯 항목은 이번 검증에서 근거를 확보하지 못했습니다.** 참조 URL은 응답했지만 가져온 본문에 이점 절이 없었습니다. 그래서 아래 표는 원본 자료의 서술을 옮긴 것이고 공식 문서 확인 항목이 아닙니다([11.5절](#115-검증하지-못한-항목)).
...
여섯 항목 중 3번(모니터링과 로깅) · 4번(코드형 인프라) · 6번(구성 관리)에서 언급되는 방식은 [2.5절](#25-devops-방식)에서 다시 나옵니다. 즉 이 이점들이 "왜"라면 다음 절의 방식은 "무엇으로"입니다.

### 2.5 DevOps 방식 🔄

DevOps 방식은 여섯 가지입니다. 일부 원본 자료는 `모니터링및 로깅` · `커뮤니케이션및 협업` 처럼 **"및" 앞의 공백이 빠져 있거나** 단어가 두 줄로 쪼개진 형태로 표기하는데, 이 문서는 정상 표기로 옮겼습니다([11.1절](#111-교재와-다른-점)).
...
🔄 **문서는 여기에 하나를 더 제시합니다.** `What is DevOps?` 페이지는 인프라 자동화 방식으로 코드형 인프라와 함께 **구성 관리(configuration management)** 를 제시하며, 이 두 방식이 컴퓨팅 리소스를 **탄력적으로** 유지하도록 돕는다고 기술합니다. 위 여섯 개 목록에는 구성 관리가 없습니다.
...
| — | 🆕 **구성 관리** — 문서가 제시하는 일곱 번째 방식 |

문서는 또 **마이크로서비스와 릴리스 빈도 증가의 조합**이 배포 수를 크게 늘려 운영 과제를 만들고, 지속적 통합·지속적 전달 같은 방식이 이를 해결한다고 기술합니다.
...
코드형 인프라는 DevOps 방식 중 하나이자 중요한 방식입니다. 다만 개념만으로는 도구와 이어지지 않습니다. CloudFormation은 도구 목록에 이름이 오르고, AWS SAM은 그 자체가 IaC 도구입니다.
...
네 번째 줄이 [2.1절](#21-전통적인-소프트웨어-배포의-문제점)의 4번 문제("롤백 계획 없음")에 대한 첫 번째 답입니다. 이전 버전의 템플릿으로 되돌릴 수 있다는 점이 롤백의 출발점입니다.
...
DevOps용 AWS 도구는 아홉 개로 정리됩니다(여덟 개를 한 줄로 설명하고 아홉 번째는 "그 외 다수"). 조회한 문서로 확인한 것과 확인하지 못한 것을 구분해 표시했습니다.
...
위 CloudFormation 설명은 원본 자료에서 같은 뜻의 부사가 중복된 문장("손쉽게 … 리소스의 모음을 **쉽게** 생성하고")을 정리한 표기입니다([11.1절](#111-교재와-다른-점)).

CodePipeline 문서의 `DevOps pipeline example` 은 이 세 서비스가 실제로 어떻게 맞물리는지 보여 줍니다. **AWS CodePipeline이 AWS CodeBuild와 AWS CodeDeploy를 오케스트레이션할 수 있다**는 점의 근거입니다.
...
지속적 통합은 "코드 변경 사항을 **중앙 리포지토리**에 정기적으로 병합"하는 방식으로 정의됩니다. 그런데 **DevOps용 도구 목록에는 소스 리포지토리 서비스가 하나도 없습니다.** 그 중앙 리포지토리를 무엇으로 두는지가 목록만으로는 드러나지 않습니다.
...
| AWS CodeCommit | **확인하지 못했습니다.** 개발자 안내서·FAQ·문서 검색을 모두 시도했지만 신규 고객 제공 중단 안내를 찾지 못했습니다. 추측으로 채우지 않았습니다([11.5절](#115-검증하지-못한-항목)). DevOps용 도구 목록에 CodeCommit은 **등장하지 않습니다** |
...
AWS SAM을 한 줄로 요약하면 "서버리스 애플리케이션을 배포하는 데 사용하는 오픈 소스 프레임워크"입니다. 문서는 이를 조금 더 좁혀 규정합니다.
...
"배포하는 데 사용하는"보다 문서의 "코드형 인프라로 **구축하는**"이 더 넓습니다. SAM은 배포만 하는 도구가 아니라 작성·빌드·배포·테스트·모니터링 전 주기를 다룹니다([8.3절](#83-cli-명령-24개)).

> — 출처: [What is the AWS Serverless Application Model (AWS SAM)?](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam.html)

### 4.2 두 구성 요소

AWS SAM은 두 구성 요소로 나뉩니다.
...
CLI로 할 수 있는 일은 네 가지로 정리됩니다. **첫 항목을 수행하는 명령 이름이 잘 알려져 있지 않습니다.**

| # | 하는 일 | 해당 명령 |
|---|---|---|
| 1 | AWS SAM 템플릿 파일이 사양에 맞게 작성되었는지 확인합니다 | 🆕 **`sam validate`** ([8.5절](#85-sam-validate)) |
...
문서가 제시하는 **주요 기능**은 다섯 가지입니다. 뒤의 세 개는 비교적 최근에 더해진 것으로, 이 모듈에서 처음 다룹니다.

| 주요 기능 | 비고 |
|---|---|
| 더 적은 코드로 인프라 코드를 빠르게 정의 | SAM의 기본 |
| AWS SAM CLI로 작성·빌드·배포·테스트·모니터링 전 주기 관리 | 모니터링까지 포함 |
| 🆕 **AWS SAM 커넥터**로 리소스 간 권한을 빠르게 프로비저닝 | [6.3절](#63-커넥터) |
| 🆕 **`sam sync`** 로 개발 중 로컬 변경을 클라우드에 계속 동기화 | [9.6절](#96-sam-sync-로-개발-반복-줄이기) |
| 🆕 AWS SAM CLI로 **Terraform** 서버리스 애플리케이션의 Lambda 함수·계층 로컬 디버깅·테스트 | — |
...
두 번째 항목이 **AWS SAM 템플릿이 AWS CloudFormation 템플릿의 확장**이라는 점의 근거입니다.
...
🔄 **변환은 "언제" 일어날까요.** 문서는 변환이 **배포 중(during deployment)** 수행된다고 기술하며, 이것이 [5.2절](#52-transform-선언)의 `Transform` 선언이 필요한 이유입니다. 템플릿을 CloudFormation에 배포할 때 CloudFormation이 그 선언을 보고 서버리스 변환을 적용합니다.
...
SAM을 언제 쓰는지는 [3.3절](#33-iac-도구-비교)의 비교와 함께 보면 판단 기준이 정리됩니다.
...
SAM 애플리케이션을 공유하는 경로입니다. `sam publish` 명령([8.3절](#83-cli-명령-24개))과 `AWS::Serverless::Application` 리소스 유형([6.1절](#61-리소스-유형-13개))이 여기에 연결되므로, SAM을 주제로 하는 모듈에서 짚어 둘 가치가 있습니다.
...
SAM 템플릿은 여러 섹션으로 이루어지고, 그중 어떤 섹션이 필수인지는 문서가 명시합니다.
...
템플릿 첫 줄에 자주 보이는 `AWSTemplateFormatVersion: '2010-09-09'` 는 문서의 SAM 템플릿 섹션 목록에는 없습니다. 다만 다른 SAM 문서 페이지의 템플릿 예시에 이 줄이 그대로 등장하므로 **잘못된 표기는 아닙니다.**

관련 참조 문서 `sam-specification-template-anatomy.html` 은 **현재도 유효하며** 페이지 제목은 `AWS SAM template anatomy` 입니다.

> — 출처: [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html)

### 5.2 Transform 선언 🔄

`Transform` 선언을 다음처럼 적은 형태를 볼 수 있습니다.

```yaml
# 문제가 있는 형태 — serverless 의 첫 글자가 소문자입니다
...
**같은 자료 안에서도 표기가 갈리기도 합니다.** 같은 코드 블록의 리소스 유형은 `Type: AWS::Serverless::Function` 으로 대문자 `S` 를 쓰는데, `Transform` 만 소문자로 적히면 한 블록 안에서 같은 단어의 대소문자가 어긋납니다([11.1절](#111-교재와-다른-점)).

이 오타가 실무에서 위험한 이유는 분명합니다. 변환 선언이 인식되지 않으면 `AWS::Serverless::*` 리소스가 확장되지 않고, 그러면 CloudFormation은 알 수 없는 리소스 유형을 만난 상태가 됩니다. **소문자 형태를 그대로 옮겨 적으면 배포가 실패합니다.**

> — 출처: [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html)

### 5.3 Globals 섹션 🔄

`Globals:` 를 "AWS SAM 템플릿 내에서 사용할 **전역 변수**를 설정한다"고 설명하는 경우가 있습니다.
...
흔히 `Globals` 안에 적는 내용도 변수가 아닙니다. `Api` 의 `MethodSettings` 이므로 **리소스 속성의 공통값**입니다. 이 예시 자체는 유효합니다. `MethodSettings` 는 `Globals` 의 `Api` 에 지원되는 속성 목록에 있습니다.

```yaml
# Globals 블록 예시 — 예시 자체는 유효합니다
...
🆕 **`Globals` 를 상속하는 리소스 유형은 여덟 개입니다.**
...
list 함수 템플릿을 자주 보이는 형태 그대로 옮기면 다음과 같습니다. **원본 텍스트 상자에는 들여쓰기가 없어 각 줄이 별개 문단으로 들어가 있으므로**, 순서를 유지하면서 YAML로 읽히도록 들여쓰기만 복원했습니다. 원문에는 계층 정보가 없습니다.

```yaml
# 문제가 있는 형태 (들여쓰기만 복원). 교정 전입니다
...
이 템플릿은 AWS Lambda에서 list 함수를 정의하고, 런타임·함수 코드 위치·핸들러를 지정하며, `AmazonDynamoDBReadOnlyAccess` IAM 정책을 연결해 함수가 Amazon DynamoDB를 쿼리할 수 있게 하고, 마지막으로 함수를 Amazon API Gateway 엔드포인트의 `/notes` 에 연결합니다.
...
| 4 | (없음) | `pollyNotesTable` 리소스 정의 추가 | Delete 함수 템플릿이 `!Ref pollyNotesTable` 을 쓰지만 정의가 함께 제시되지 않습니다 |
...
함수 설명 문장도 정리했습니다. "참조된 관리형 IAM 정책, 런타임, 코드 정의된 핸들러를 사용하여 AWS Lambda 함수를 생성합니다"라는 표현은 구조가 불명확합니다. 세 항목으로 나누면 이렇습니다.
...
`Events` 의 `Type: Api` 는 "Amazon API Gateway를 생성하고 필요한 매핑과 권한을 처리한다"고 설명되기도 합니다. 문서 기준으로 이때 만들어지는 것은 **암시적 API** 이며, `HttpApi` 이벤트를 쓰면 `AWS::ApiGatewayV2::Api`(논리 ID `ServerlessHttpApi`)와 `$default` 스테이지가 만들어집니다([4.3절](#43-작동-방식-변환)). 명시적 API 리소스를 참조하려면 `RestApiId` 를 씁니다([7.3절](#73-auth-속성으로-선언하기)).
...
### 5.5 Delete 함수 템플릿 교정 🔄

Delete 함수를 정의하는 템플릿 조각에는 **문제가 가장 많이 몰려 있습니다.**
...
이 조각은 다음처럼 설명됩니다. AWS Lambda에서 **Delete 함수**를 정의하는 AWS SAM 템플릿이며, 런타임, 함수 코드의 위치와 핸들러를 설정합니다. 코드 위치는 AWS SAM 템플릿이 포함된 `template.yml` 파일에 따라 달라지고, 이 예에서 코드는 **`deleteFunction` 이라는 폴더**에 있다고 적힙니다.
...
| 3 | `Events: listNotes:` | `Events: deleteNote:` | Delete 함수인데 이벤트 이름이 list 함수의 `listNotes` 와 같습니다. 복사 흔적입니다 |
| 4 | "`deleteFunction` 이라는 폴더" | 논리 ID는 `deleteFunction`, 코드 폴더는 `delete-function/` | `deleteFunction` 은 폴더 이름이 아니라 **논리 리소스 ID** 입니다 |
| 5 | `Properties: ...` (잘림) | `Path` · `Method` 를 채움 | 원문이 생략 부호로 끊겨 있습니다 |
| 6 | `!Ref pollyNotesTable`(정의 없음) | 테이블 리소스를 함께 표기 | 참조 대상이 함께 제시되지 않습니다 |
| 7 | 레이블 "SAM 템플릿" | "AWS SAM 템플릿" | 다른 곳은 모두 "AWS SAM 템플릿"입니다 |
...
**두 함수의 권한 지정 방식이 다릅니다.** list 함수는 `Policies`, Delete 함수는 `Role` 을 씁니다. 두 속성의 관계는 [5.6절](#56-함수-권한-policies-와-role)에서 다룹니다. 핵심만 미리 말하면, **`Role` 을 설정하면 `Policies` 가 무시됩니다.**
...
앞의 두 함수는 서로 다른 방식으로 권한을 받습니다. 차이와 우선순위를 여기서 정리합니다.
...
Resources:
  ...
  # (2) list 함수 방식 — 관리형 정책 이름. 계정의 모든 테이블에 열립니다
  functionB:
    Type: AWS::Serverless::Function
    Properties:
      CodeUri: fn-b/
...
`AWS::Serverless::Function` 의 다른 기본값도 정리해 둡니다.
...
`Policies` 값으로 AWS 관리형 정책 이름 `AmazonDynamoDBReadOnlyAccess` 하나만 익숙한 경우가 많지만, 정책 템플릿은 **SAM이 권장하는 방식**입니다.
...
앞의 두 함수에 맞는 선택은 다음과 같습니다.

| 함수 | 원래 준 권한 | 맞는 정책 템플릿 |
|---|---|---|
| `listFunction` | `AmazonDynamoDBReadOnlyAccess`(계정의 모든 테이블) | `DynamoDBReadPolicy` 에 `TableName` 지정 |
| `deleteFunction` | `DynamoDBReadRole`(읽기 역할) | `DynamoDBCrudPolicy` 또는 `DynamoDBWritePolicy` |
...
SAM 템플릿 파일 이름을 `template.yml` 로만 아는 경우가 많습니다.
...
"자체 템플릿(**YAML 또는 JSON**)"이라는 설명도 `sam validate` 의 `--template-file` 설명이 `template.yaml` · `template.yml` · `template.json` 을 받는다는 서술로 뒷받침됩니다.
...
`AWS::Serverless::*` 리소스 유형은 흔히 여섯 개로 소개되지만, **현재 문서가 나열하는 유형은 열세 개입니다.**

| # | 리소스 유형 | 초기 6종 | 무엇인가 |
|---|---|---|---|
| 1 | `AWS::Serverless::Api` | 포함 | API Gateway REST API |
| 2 | `AWS::Serverless::Application` | **신규** 🆕 | 중첩 서버리스 애플리케이션 |
| 3 | `AWS::Serverless::CapacityProvider` | **신규** 🆕 | — |
| 4 | `AWS::Serverless::Connector` | **신규** 🆕 | **두 리소스 사이의 권한을 구성** ([6.3절](#63-커넥터)) |
| 5 | `AWS::Serverless::Function` | 포함 | AWS Lambda 함수 |
| 6 | `AWS::Serverless::GraphQLApi` | **신규** 🆕 | AWS AppSync 계열 |
| 7 | `AWS::Serverless::HttpApi` | 포함 | API Gateway HTTP API |
| 8 | `AWS::Serverless::WebSocketApi` | **신규** 🆕 | — |
| 9 | `AWS::Serverless::LayerVersion` | 포함 | Lambda 계층 |
| 10 | `AWS::Serverless::MicrovmImage` | **신규** 🆕 | — |
| 11 | `AWS::Serverless::NetworkConnector` | **신규** 🆕 | — |
| 12 | `AWS::Serverless::SimpleTable` | 포함 | DynamoDB 테이블 |
| 13 | `AWS::Serverless::StateMachine` | 포함 | AWS Step Functions 상태 머신 |

**초기 여섯 개는 모두 현재도 유효합니다.** 사라진 것은 없고 일곱 개가 추가되었습니다. 서버리스 애플리케이션 설계에 직접 영향을 주는 것은 `Connector`(리소스 간 권한) · `GraphQLApi` · `WebSocketApi` · `Application`(중첩 서버리스 애플리케이션)입니다.

리소스 유형 목록 다음 문장도 확인했습니다. "이러한 리소스 외에도, AWS SAM 템플릿에서는 **어떤 AWS CloudFormation 리소스든지** 정의할 수 있습니다." 문서는 `AWS SAM also supports CloudFormation resource and property types` 로 같은 내용을 기술합니다. 이 리소스와 속성은 AWS SAM **축약 구문**으로 정의합니다.
...
리소스 유형과 AWS 서비스의 매핑은 흔히 **여섯 유형 중 네 개만** 소개됩니다.
...
🔄 **참조 링크가 주제와 어긋나기도 합니다.** 리소스 유형 목록과 함께 제시되는 링크 중 'API Gateway 리소스 정책으로 API 액세스 관리'는 사실 **[7장](#7-api-액세스-제어)의 API 액세스 제어 주제**에 해당합니다.

| 참조 링크 | 상태 | 맞는 주제 |
|---|---|---|
| AWS SAM 템플릿 구조(`sam-specification-template-anatomy.html`) | **유효.** 페이지 제목은 `AWS SAM template anatomy` | 템플릿 구조 |
| API Gateway 리소스 정책(`apigateway-resource-policies.html`) | **유효.** 페이지 제목은 `Control access to a REST API with API Gateway resource policies` | **API 액세스 제어** |
...
함수에 권한을 주는 방법으로는 흔히 `Policies` 와 `Role` 두 가지가 소개됩니다. **`AWS::Serverless::Connector` 는 나중에 더해진 세 번째 길이고, 앞서 본 Delete 함수의 권한 불일치 같은 실수를 줄이는 수단입니다.**
...
| 방식 | 무엇을 쓰는가 | 권한 범위 | 비고 |
|---|---|---|---|
| 관리형 정책 이름 | `Policies: AmazonDynamoDBReadOnlyAccess` | **계정의 모든 DynamoDB 테이블** | list 함수 방식 |
| 기존 역할 ARN | `Role: !Sub arn:aws:iam::${AWS::AccountId}:role/...` | 역할에 붙은 정책이 정하는 범위. **`Policies` 는 무시됨** | Delete 함수 방식 |
| 정책 템플릿 | `Policies: - DynamoDBReadPolicy: TableName: ...` | 지정한 **테이블 한정** | 🆕 |
| 커넥터 | `Connectors:` 블록에 `Destination` + `Permissions` | 두 리소스 사이의 **의도(Read/Write)** 로 선언 | 🆕 |
...
`AWS::Serverless::SimpleTable` 은 흔히 "Serverless SimpleTable = Amazon DynamoDB" 한 줄로만 매핑됩니다. 이 리소스가 실제로 무엇을 만드는지 정리합니다.
...
API 액세스 제어 메커니즘은 여섯 가지이고, **여섯 행 전부가 현재 문서와 정확히 일치합니다.**
...
별표의 뜻은 이렇습니다. **Amazon Cognito를 `AWS::Serverless::HttpApi` 리소스 유형의 JSON 웹 토큰(JWT) 발급자로 사용할 수 있습니다.**

`HttpApi` 의 "IAM 권한" 칸이 비어 있는 것도 문서와 같습니다. **HTTP API는 IAM 권한 부여를 지원하지 않고 `AWS::Serverless::Api` 만 지원합니다.**

표만으로는 정의가 드러나지 않는 메커니즘의 정의를 채웁니다.
...
관련 참조 문서 `serverless-controlling-access-to-apis.html` 은 **현재도 유효하며** 페이지 제목은 `Control API access with your AWS SAM template` 입니다. 일부 원본 자료에는 공백 누락이 있습니다 — "`AWS::Serverless::Api`**리**소스 유형에서"([11.1절](#111-교재와-다른-점)).
...
여섯 메커니즘 중 **어느 것을 언제 고르는지**에 대한 선택 지침을 문서가 제시합니다.
...
"AWS SAM 템플릿에서 권한 부여를 활성화해야 한다"는 설명만으로는 **어떤 속성을 쓰는지** 드러나지 않습니다. 실제 속성은 `Auth` 이며 데이터 유형 이름은 `ApiAuth` 입니다.
...
🔄 **앞서 본 list·Delete 함수 템플릿은 `RestApiId` 없이 암시적 API를 만드는 형태입니다.** 즉 그 템플릿에는 권한 부여를 붙일 대상 API 리소스가 없습니다. 액세스 제어를 실제로 걸려면 명시적 API 리소스를 선언하고 함수 이벤트에서 참조해야 합니다.
...
문서는 AWS SAM으로 **일부 API Gateway 오류 응답의 내용을 사용자 지정**할 수 있다고 기술합니다.
...
설치 준비를 세 단계로 소개하는 자료와 네 단계로 세는 자료가 있어 **자료마다 단계 수가 어긋납니다**([11.1절](#111-교재와-다른-점)).
...
| **AWS Command Line Interface(AWS CLI)** | 필요. AWS 자격 증명을 구성하는 데 씁니다 | 🔄 **자주 빠집니다** |
| 도커 | 🔄 **이 페이지에는 도커가 사전 요구 사항으로 나오지 않습니다** | "선택 사항"으로 목록에 넣음 |
| AWS Toolkit for VS Code | (선택) 3단계 | 없음 🆕 |

**그렇다면 도커는 언제 필요한가.** [8.7절](#87-sam-local-invoke-와-도커)에서 보듯 `sam local invoke` 문서는 AWS SAM CLI가 **도커를 사용해 로컬 컨테이너에서 함수를 빌드한 다음 호출**한다고 명시합니다. 즉 도커는 사전 요구 사항 목록에는 없지만 **로컬 테스트를 하려면 사실상 필수**입니다. "선택 사항"이라고만 하거나 "로컬로 테스트할 계획이라면"이라는 조건만 붙이면 이 사정이 반쯤만 전달됩니다.
...
"다양한 IDE 및 런타임 조합과 연동하는 다양한 AWS 도구 키트가 있다"는 설명이 나오기도 하지만, 정작 그 도구 키트를 자세히 다루는 자료는 드뭅니다([11.1절](#111-교재와-다른-점)). 다만 이 문서도 **AWS Toolkit for VS Code 하나만** 확인했고 IDE용 도구 키트 전체 목록은 조회하지 않았습니다([11.5절](#115-검증하지-못한-항목)).
...
설치 방법의 세부는 참조 URL로 안내되는 경우가 많습니다. **그 URL은 현재도 유효합니다.**

| 항목 | 상태 |
|---|---|
| 참조 URL(`serverless-sam-cli-install.html`) | **유효.** `Install the AWS SAM CLI` 페이지 내용을 제공합니다 |
| 현재 문서 안의 정식 경로 | `install-sam-cli.html`. 사전 요구 사항 페이지와 `sam deploy` · `sam local` 페이지의 링크가 모두 이쪽을 가리킵니다 |

🔄 **설치 경로에 새로운 제약이 두 개 생겼습니다.**
...
기본으로 익히는 명령은 흔히 **일곱 개**입니다. `sam init` · `sam build` · `sam local invoke` · `sam local start-api` · `sam local generate-event` · `sam package` · `sam deploy`. **현재 명령 참조 페이지가 나열하는 명령은 스물넷입니다.**

| 명령 | 기본 7종 | 무엇을 하는가 |
|---|---|---|
| `sam build` | 포함 | 다음 단계(로컬 테스트·배포)를 위해 애플리케이션을 준비 ([9.2절](#92-sam-build-와-컨테이너-빌드)) |
| `sam delete` | **신규** 🆕 | 스택·아티팩트·템플릿 파일을 삭제 ([9.8절](#98-정리)) |
| `sam deploy` | 포함 | CloudFormation으로 애플리케이션을 배포 ([9.4절](#94-sam-deploy-와-samconfigtoml)) |
| `sam init` | 포함 | 새 서버리스 애플리케이션 초기화 ([8.4절](#84-sam-init)) |
| `sam list` | **신규** 🆕 | 리소스·엔드포인트·스택 출력 표시 ([9.7절](#97-배포-결과-확인)) |
| `sam local callback` | **신규** 🆕 | — |
| `sam local execution` | **신규** 🆕 | — |
| `sam local generate-event` | 포함 | 샘플 이벤트 생성 ([8.8절](#88-sam-local-generate-event)) |
| `sam local invoke` | 포함 | 함수를 로컬에서 한 번 호출 ([8.7절](#87-sam-local-invoke-와-도커)) |
| `sam local start-api` | 포함 | 로컬 HTTP 서버로 함수 실행 |
| `sam local start-lambda` | **신규** 🆕 | AWS CLI·SDK와 함께 쓸 로컬 HTTP 서버 |
| `sam logs` | **신규** 🆕 | — |
| `sam package` | 포함 | 아티팩트 패키징 ([9.3절](#93-sam-package-는-이제-별도-단계가-아니다)) |
| `sam pipeline bootstrap` | **신규** 🆕 | CI/CD 파이프라인 준비 |
| `sam pipeline init` | **신규** 🆕 | CI/CD 파이프라인 초기화 |
| `sam publish` | **신규** 🆕 | AWS Serverless Application Repository 게시 ([4.5절](#45-aws-serverless-application-repository)) |
| `sam remote callback` | **신규** 🆕 | — |
| `sam remote execution` | **신규** 🆕 | — |
| `sam remote invoke` | **신규** 🆕 | 클라우드에 배포된 리소스를 직접 호출 ([8.9절](#89-클라우드-리소스-직접-호출)) |
| `sam remote test-event` | **신규** 🆕 | — |
| `sam sync` | **신규** 🆕 | 로컬 변경을 클라우드에 동기화 ([9.6절](#96-sam-sync-로-개발-반복-줄이기)) |
| `sam traces` | **신규** 🆕 | — |
| `sam validate` | **신규** 🆕 | 템플릿 유효성 검증 ([8.5절](#85-sam-validate)) |

기본 7종에서 빠진 명령 중 특히 중요한 다섯 개를 짚어 둡니다.

| 명령 | 왜 중요한가 |
|---|---|
| `sam validate` | "템플릿 파일이 사양에 맞게 작성되었는지 확인"이라는 CLI 기능의 **실제 명령 이름**입니다 |
| `sam sync` | 개발 중 반복 주기를 줄이는 별도 경로입니다 |
| `sam delete` | 데모가 리소스를 만들고 끝나므로 **정리 절차**가 필요합니다 |
| `sam remote invoke` | 기본 7종은 **로컬 테스트만** 다룹니다 |
| `sam list` | "생성한 리소스 표시"를 CLI로 하는 경로입니다 |

> — 출처: [AWS SAM CLI command reference](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-command-reference.html)

### 8.4 sam init 🔄

`sam init` 은 흔히 예시 다섯 줄과 옵션 세 개(`--runtime` · `--app-template` · `--location`)로 소개됩니다. **세 옵션은 모두 현재도 존재합니다.**

```bash
# 문제가 있는 형태. python3.8 은 지원 종료된 런타임입니다
...
| 2 | 프롬프트 `$` | 제거 | 복사·붙여넣기를 쉽게 하기 위해 프롬프트 기호를 뺐습니다. 원본 자료는 프롬프트를 `>>` 와 `$` 로 뒤섞어 쓰기도 합니다([11.1절](#111-교재와-다른-점)) |
...
**대화형 흐름은 첫 프롬프트에서 끝나지 않습니다.** 흔히 첫 프롬프트만 소개되지만 실제로는 더 이어집니다.
...
첫 프롬프트("Which template source would you like to use? / 1 - AWS Quick Start Templates / 2 - Custom Template Location")는 **현재 문서 예시와 같습니다.** 데모에서 말하는 "빠른 템플릿"도 이 `AWS Quick Start Templates` 를 가리킵니다(용어만 다릅니다).

🆕 **생성 결과 요약에 `samconfig.toml` 이 포함됩니다.** 이 파일은 별도로 짚어 둘 만합니다([9.4절](#94-sam-deploy-와-samconfigtoml)).
...
프로젝트 구조 그림에 자주 등장하는 표기 문제도 있습니다.
...
**다만 `sam init` 이 만드는 디렉터리 구조 자체는 확인하지 못했습니다.** 생성 결과 요약까지만 확인했습니다([11.5절](#115-검증하지-못한-항목)). 위 두 항목은 원본 자료의 표기 문제를 지적한 것이고, 실제 생성물의 폴더 이름을 단정하는 것이 아닙니다.

Init 관련 설명 중 "내장 애플리케이션이 템플릿이나 사용자 지정 템플릿을 선택할 수 있습니다"라는 문장은 성립하지 않습니다. "내장 애플리케이션 **템플릿**이나 사용자 지정 템플릿을 선택할 수 있습니다"의 오기로 보입니다([11.1절](#111-교재와-다른-점)).
...
CLI로 할 수 있는 일의 첫 항목으로 "AWS SAM 템플릿 파일이 사양에 맞게 작성되었는지 확인합니다"가 꼽히지만, **그 확인을 수행하는 명령 이름은 잘 알려져 있지 않습니다.** `sam validate` 입니다.
...
`sam local` 하위 명령은 흔히 세 개로 소개되지만 **현재는 여섯 개입니다.**

| 하위 명령 | 기본 3종 | 무엇을 하는가 |
|---|---|---|
| `sam local generate-event` | 포함 | 로컬 테스트용 AWS 서비스 이벤트 생성 |
| `sam local invoke` | 포함 | Lambda 함수를 로컬에서 **한 번** 호출 |
| `sam local start-api` | 포함 | **로컬 HTTP 서버**로 Lambda 함수 실행 |
| `sam local start-lambda` | **신규** 🆕 | **AWS CLI·SDK와 함께 쓸** 로컬 HTTP 서버로 Lambda 함수 실행 |
| `sam local callback` | **신규** 🆕 | — |
| `sam local execution` | **신규** 🆕 | — |

기본 세 명령의 설명은 문서와 일치합니다.
...
`sam local invoke "HelloWorldFunction" -e event.json` 도 **함수 논리 ID + 이벤트 파일**을 주는 현재 사용법과 같습니다.
...
🆕 **문서에는 눈여겨볼 주의가 하나 있습니다.** 문서는 **신뢰할 수 없는 코드에 SAM CLI의 로컬 호출 기능을 쓰지 않도록 권고**하고, 완전한 격리가 필요하면 Lambda 서비스에서 직접 실행하라고 안내합니다.

> — 출처: [Introduction to testing with sam local invoke](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-local-invoke.html)

### 8.8 sam local generate-event 🔄

지원 서비스는 흔히 "**대표적인 서비스는 Amazon S3, API Gateway, Amazon SNS**"처럼 세 개만 소개됩니다. 🔄 **실제 지원 목록은 훨씬 깁니다.**
...
문서 출력 예시가 생략 부호로 잘려 있어 **전체 목록은 확인하지 못했습니다**([11.5절](#115-검증하지-못한-항목)). 다만 알파벳 앞부분만 봐도 흔히 소개되는 세 개보다 여섯 개가 더 있습니다.

🆕 **인자를 두 단계로 준다는 점도 짚어 둘 만합니다.**
...
기본 명령만 익히면 **로컬 테스트에만** 머무르기 쉽습니다. 현재 CLI에는 클라우드에 배포된 리소스를 직접 호출하는 경로가 있습니다.
...
표준 워크플로는 네 단계입니다. **프로덕션 경로로서 이 4단계는 여전히 유효합니다.**
...
배포 단계 이후 템플릿이 실제 리소스가 되기까지의 흐름은 다음과 같습니다.

```text
AWS SAM 템플릿  ─▶  AWS CloudFormation  ─▶  변경 세트  ─▶  스택  ─▶  AWS 클라우드
                     (서버리스 변환)         (배포 전 검토)
```
...
위 흐름의 뒷부분(AWS SAM 템플릿 → AWS CloudFormation → 스택 → AWS 클라우드)은 문서와 일치합니다. 다만 흔히 그리는 그림에는 **CloudFormation과 스택 사이의 변경 세트가 빠져 있어**, 위 흐름에는 이를 명시했습니다([9.5절](#95-변경-세트와-롤백)).

배포와 관련해 "구성 파일을 사용할 때 대화형 프롬프트를 통해 배포하려면 `sam deploy` 명령을 실행한다"는 설명은 인과가 뒤집혀 있습니다. **대화형 배포(`--guided`)와 구성 파일 기반 배포는 서로 다른 경로**입니다([11.1절](#111-교재와-다른-점)).

> — 출처: [Introduction to deploying with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-deploy.html)

### 9.2 sam build 와 컨테이너 빌드 🔄

**이 모듈에서 가장 중요한 교정입니다.** 다음과 같은 설명이 돌기도 합니다.
...
**같은 자료 안에서도 표기가 갈리기도 합니다.** 본문은 `--use-container`(하이픈 두 개)로 올바르게 적으면서 설명에서는 `-use-container`(하이픈 한 개)로 적는 식입니다. 정확한 표기는 **`--use-container`** 이고 짧은 형식은 **`-u`** 입니다([11.1절](#111-교재와-다른-점)).
...
`sam deploy` 는 **`.aws-sam` 디렉터리의 빌드 아티팩트를 배포**합니다. 그래서 문서는 원본 파일을 바꾸면 배포 전에 `sam build` 로 `.aws-sam` 디렉터리를 갱신하라고 안내합니다. `sam build && sam local invoke` 도 이 관계 위에서 동작합니다.
...
`sam build && sam package --s3-bucket <bucket_name>` 를 배포 전 단계로 제시하고 "`sam package` 및 `sam deploy` 명령을 함께 사용한다"고 안내하는 경우가 있습니다.
...
`samconfig.toml` 은 잘 다뤄지지 않지만, 이 파일이 `sam deploy --guided` 의 산출물이고 이후 배포의 기준이므로 실무의 핵심입니다.
...
🔄 **`--template-file` 과 `--config-file` 의 구분이 자주 뒤섞입니다.**
...
`--capabilities` 도 자주 놓치는 필수 지식입니다.
...
**변경 세트는 잘 다뤄지지 않습니다.** 흔히 그리는 그림은 `AWS CloudFormation → 스택` 으로 곧바로 이어지고, `sam deploy` 를 실행하면 배포된다고만 보여 줍니다. 그런데 [9.4절](#94-sam-deploy-와-samconfigtoml)에서 봤듯 **배포는 변경 세트를 거칩니다.** 그리고 이것이 [2.1절](#21-전통적인-소프트웨어-배포의-문제점)의 4번(롤백 계획 없음)·5번(인적 오류) 문제에 대한 답입니다.
...
표준 워크플로 4단계에는 `sam sync` 가 없습니다. 이 명령은 **AWS SAM Accelerate** 의 일부입니다.
...
즉 표준 4단계 워크플로는 **프로덕션 경로로서 여전히 맞습니다.** `sam sync` 가 그것을 대체하는 것이 아니라 개발 반복용 별도 경로를 추가한 것입니다.
...
데모의 "생성한 리소스 표시"는 콘솔 조작을 전제하는 것으로 보입니다. 현재 CLI에는 같은 정보를 확인하는 경로가 있습니다.
...
**배포한 리소스를 정리하는 방법은 잘 다뤄지지 않습니다.** 데모가 애플리케이션을 배포하고 테스트하는 데서 끝나므로 정리 절차가 필요합니다.
...
데모 항목만 나열되고 **무엇을 어떤 순서로 시연하는지, 무엇을 확인하면 성공인지가 함께 제시되지 않는** 경우가 많습니다. 각 항목을 명령 단위로 채웠습니다.

| 데모 항목 | 세부 항목 | 명령 | 확인 지점 |
|---|---|---|---|
...
데모 첫 항목의 "**가용 런타임**"은 `sam init` 이 제시하는 런타임 목록을 가리키는 것으로 보입니다. `sam init --runtime` 이 받는 허용 값은 [8.4절](#84-sam-init)에 있습니다. 다만 **대화형 흐름이 화면에 제시하는 런타임 목록 자체는 확인하지 못했습니다**([11.5절](#115-검증하지-못한-항목)).
...
배포 전략은 흔히 `Canary` · `Linear` · `All-at-once` 세 단어와 AWS CodeDeploy 설명으로만 소개됩니다. 그러다 보면 "다양한 AWS SAM 배포 전략 설명"이라는 목표에 대응하는 **SAM 템플릿에서의 선언 방법**이 빠집니다. 이 장은 CodeDeploy 쪽을 먼저 정리하고([10.1절](#101-배포-유형-두-가지)~[10.3절](#103-사전-정의-배포-구성)), 그다음 SAM 템플릿에서 이를 선언하는 방법을 채웁니다([10.4절](#104-sam-템플릿에서-배포-전략-선언하기)~[10.8절](#108-롤백-경로-정리)).

### 10.1 배포 유형 두 가지 🔄

CodeDeploy 배포 옵션은 두 가지입니다. **현재 위치 배포에 관한 서술은 문서와 일치합니다.**
...
🔄 **다만 두 층위를 나란히 놓으면 Lambda 배포가 어느 유형인지 드러나지 않습니다.** `Canary` · `Linear` · `All-at-once` 세 단어는 Lambda 컴퓨팅 플랫폼의 **배포 구성**이고, 현재 위치·블루/그린은 **배포 유형**입니다. 문서는 관계를 명시합니다.
...
🆕 **블루/그린 배포가 현재 위치 배포보다 갖는 이점**도 정리해 둡니다. 특히 두 번째 항목이 [2.1절](#21-전통적인-소프트웨어-배포의-문제점)의 4번(롤백 계획 없음) 문제와 직결됩니다.
...
🆕 현재 위치 배포에 필요한 구성 요소도 정리해 둡니다. **AppSpec 파일(application specification file)** 로 CodeDeploy가 실행할 배포 작업을 정의하고, 배포 가능한 콘텐츠와 AppSpec 파일을 묶은 **애플리케이션 수정 버전(revision)** 을 Amazon S3 버킷이나 GitHub 리포지토리에 업로드한 뒤, 각 인스턴스의 **CodeDeploy 에이전트**가 CodeDeploy를 폴링해 대상 수정 버전을 가져와 배포합니다.
...
🔄 **컴퓨팅 플랫폼은 EC2/온프레미스와 Lambda 두 갈래로만 소개되기 쉽습니다.** 문서의 컴퓨팅 플랫폼은 **세 개**입니다.
...
Amazon ECS가 빠지면 실무에서 오해를 부르는 이유는, 같은 세 배포 구성(canary · linear · all-at-once)이 **ECS 플랫폼에도 있기** 때문입니다. 즉 이 개념은 Lambda 전용이 아닙니다.
...
`Canary` · `Linear` · `All-at-once` 세 단어만으로는 **사전 정의 구성의 이름이 드러나지 않습니다.** 문서는 이름이 정해진 목록을 제시합니다.

먼저 흔히 쓰이는 설명을 확인합니다.

| 구성 | 자주 쓰이는 설명 | 확인 |
|---|---|---|
| Canary | **"트래픽이 2 증분씩 이동합니다."** 최초 증분에서 이동하는 트래픽 비율을 지정하는 사전 정의 옵션을 선택할 수 있고 간격을 분 단위로 선택할 수도 있습니다. 이 이동은 남은 트래픽이 두 번째 증분으로 이동하기 전에 완료됩니다 | 🔄 **첫 문장이 오역입니다.** 원문은 트래픽이 **두 번의 증분으로** 이동한다(shifted in two increments)는 뜻이고, "2 증분씩"은 증분 크기가 2라는 뜻으로 읽힙니다. 같은 설명의 뒤 문장은 올바르게 두 번의 증분을 전제하므로 자료 안에서도 어긋납니다 |
...
| `CodeDeployDefault.OneAtATime` | 🆕 **배포 구성을 지정하지 않으면 CodeDeploy가 이것을 씁니다.** 이 기본값은 잘 알려져 있지 않습니다 |

EC2/온프레미스 배포 구성이 무엇을 지정하는지도 정리합니다. "최소 50%의 인스턴스가 필요하다면 이를 배포 구성에서 지정하면 된다"는 설명이 이것입니다.
...
**여기가 가장 자주 비는 부분입니다.** 모듈 목표 4번이 요구하는 "AWS SAM 배포 전략"의 실체는 `AWS::Serverless::Function` 의 두 속성입니다.
...
🔄 **표기에 주의하세요.** `All-at-once` 처럼 하이픈으로 적는 표기를 자주 보게 되지만 **SAM 템플릿에 쓰는 값은 `AllAtOnce`** 로 하이픈 없는 한 단어입니다. 하이픈 표기를 그대로 옮기면 템플릿이 유효하지 않습니다.
...
[2.1절](#21-전통적인-소프트웨어-배포의-문제점)의 4번 문제("롤백 계획 없음")에 대한 답이 바로 `Alarms` 와 `Hooks` 입니다. **이 둘은 잘 다뤄지지 않습니다.**

> — 출처: [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html)

### 10.6 첫 점진적 배포는 두 단계 🆕

실무에서 반드시 걸리는 제약입니다. **점진적 배포는 CodeDeploy가 트래픽을 옮겨 올 이전 버전을 요구하므로 첫 배포는 두 단계로 해야 합니다.**
...
`Canary` · `Linear` · `All-at-once` 를 CodeDeploy 배포 구성으로만 보면 **그 구성이 실제로 무엇을 조작하는지** 드러나지 않습니다. 문서는 **가중치 별칭 라우팅 구성이 롤링 배포를 구현하는 기반 기능**이라고 명시합니다.
...
"롤백 계획 없음"은 전통적 배포의 문제로 자주 꼽히지만, 정작 **롤백을 어떻게 구성하는지는 잘 다뤄지지 않습니다.** 이 문서에서 확인한 롤백 경로를 한자리에 모았습니다.
...
자주 보이는 문장 문제도 하나 짚어 둡니다. "애플리케이션의 배포를 사용자 지정하려면 **다음을 수행해야 합니다**"로 목록을 열고 AWS DevOps 도구 사용 · 타사 배포 도구 사용 · 자체 솔루션 구현 세 항목을 나열하는데, **세 항목은 택일 대안**입니다. "해야 합니다"로 묶여 셋 다 필요한 것처럼 읽힙니다([11.1절](#111-교재와-다른-점)).
...
앞의 일곱 항목은 **외부 문서로 확인**한 것이고, 뒤의 열아홉 항목은 **원본 자료 안에서 서로 어긋나거나 추출 과정에서 표기가 깨진 내부 불일치**여서 외부 문서로 검증할 대상이 아닙니다. 후자에는 근거 칸에 `—` 를 두었습니다.

| 항목 | 교재의 서술 | 확인된 내용 | 근거 |
|---|---|---|---|
| `Transform` 값의 대소문자 | `Transform: AWS::serverless-2016-10-31` — **소문자 `s`** | 문서가 명시하는 값은 **`AWS::Serverless-2016-10-31`**(대문자 `S`)이며 이 선언은 AWS SAM 템플릿 파일에 **필수**입니다. 같은 코드 블록의 `Type: AWS::Serverless::Function` 은 대문자 `S` 를 쓰므로 교재 안에서도 표기가 갈립니다. 문서는 언어 확장을 쓸 때 `AWS::LanguageExtensions` 를 서버리스 변환보다 앞에 두어야 한다는 순서 규칙도 제시합니다 ([5.2절](#52-transform-선언)) | [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html) |
| `Globals` 를 "전역 변수"로 설명 | 주석 상자 "AWS SAM 템플릿 내에서 사용할 **전역 변수**를 설정합니다" | `Globals` 는 변수를 선언하는 섹션이 아니라 **여러 리소스가 공통으로 쓰는 속성을 한 번만 선언해 상속시키는** 섹션입니다. 값을 파라미터화하는 섹션은 **`Parameters`** 입니다. 상속 리소스는 여덟 개이고 재정의 규칙이 데이터 유형별로 다르며(원시 값은 대체, 맵은 병합, 리스트는 앞에 추가), **선언한 속성을 리소스에서 제거할 수 없습니다.** 교재가 `Globals` 안에 적은 `Api` 의 `MethodSettings` 도 변수가 아니라 리소스 속성의 공통값입니다 ([5.3절](#53-globals-섹션)) | [Globals section of the AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.html) |
| `--use-container` 미지원 언어 | "일부 언어(예: .NET 또는 Python)에서는 `-use-container` 옵션을 사용할 수 없다"는 설명 | **현재 `sam build` 참조 문서에 그런 서술이 없습니다.** 오히려 `--build-image` 설명이 `sam build --use-container --build-image amazon/aws-sam-cli-build-image-python3.8` 을 예로 듭니다. 문서가 명시하는 유일한 제약은 **`--build-in-source` 와의 비호환**이고, `--build-image` · `--container-env-var` · `--container-env-var-file` 은 반대로 `--use-container` 없이 쓰면 오류가 납니다. 또 옵션을 하이픈 하나로 적는 표기가 있는데 정확한 표기는 **`--use-container`**(짧은 형식 `-u`)입니다 ([9.2절](#92-sam-build-와-컨테이너-빌드)) | [sam build](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-build.html) |
| "구성 파일"을 `--template-file` 에 붙임 | 주석 "# 구성 파일을 사용하여 배포" 아래에 `sam deploy --template-file deploy.yml` | `--template-file`(`--template`, `-t`)은 **AWS SAM 템플릿**의 경로·이름을 지정하는 옵션이고 기본값은 `template.yaml` 또는 `template.yml` 입니다. SAM의 구성 파일은 **`--config-file`** 로 지정하며 기본값은 프로젝트 루트의 **`samconfig.toml`** 이고 환경 이름은 `--config-env`(기본값 `default`)로 고릅니다. "구성 파일을 사용할 때 대화형 프롬프트를 통해 배포하려면"이라는 설명도 같은 혼동이며, 대화형 배포(`--guided`)와 구성 파일 기반 배포는 **서로 다른 경로**입니다 ([9.4절](#94-sam-deploy-와-samconfigtoml)) | [sam deploy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-deploy.html) |
| Delete 함수에 읽기 역할 | `Role: !Sub arn:aws:iam::${AWS::AccountId}:role/DynamoDBReadRole` | **DynamoDB 항목을 삭제하려면 쓰기 권한이 필요**하므로 이름과 목적이 어긋납니다. 또 문서는 **`Role` 을 설정하면 `Policies` 가 무시된다**고 명시하고, `Role` 을 주지 않으면 논리 ID가 `<함수-논리-ID>Role` 인 역할이 자동 생성된다고 기술합니다. `Policies` 에는 정책 템플릿·관리형 정책 ARN·관리형 정책 이름·인라인 정책을 쓸 수 있고, Delete 함수에는 `DynamoDBCrudPolicy` 나 `DynamoDBWritePolicy` 가 맞습니다 ([5.5절](#55-delete-함수-템플릿-교정) · [5.6절](#56-함수-권한-policies-와-role)) | [AWS::Serverless::Function](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-function.html) |
...
| 지속적 전달에 수동 승인이 필수라고 단정 | 원본 자료 "지속적 전달은 프로덕션에 배포하기 **전에 수동 승인 단계를 거쳐야** 합니다" | 문서는 지속적 전달을 릴리스 프로세스가 자동화된 방법론으로 기술하고, 프로덕션으로의 최종 푸시 전에 **사람, 자동화된 테스트 또는 비즈니스 규칙**이 최종 푸시 시점을 결정한다고 명시합니다. 결정 주체가 사람일 수도 있지만 자동화된 테스트나 비즈니스 규칙일 수도 있으므로 **수동 승인은 필수가 아닙니다** ([2.3절](#23-devops-프로세스-세-가지)) | [Continuous delivery and continuous integration](https://docs.aws.amazon.com/codepipeline/latest/userguide/concepts-continuous-delivery-integration.html) |
| 코드 폴더 이름 불일치 | 원본 자료 "코드는 **`deleteFunction`** 이라는 폴더에 있습니다" vs 템플릿 `CodeUri: delete-function/` | `deleteFunction` 은 폴더 이름이 아니라 **논리 리소스 ID** 입니다. 이 문서는 논리 ID와 `CodeUri` 경로를 구분해 표기했습니다 ([5.5절](#55-delete-함수-템플릿-교정)) | — |
| Delete 함수의 이벤트 이름이 `listNotes` | `Events:` 아래 항목 이름이 `listNotes` 이고 list 함수의 `listFunction` 이벤트 이름과 같음 | 원본 자료에서 복사한 흔적으로 보입니다. 같은 템플릿이 `!Ref pollyNotesTable` 을 참조하지만 **그 테이블 리소스의 정의가 원본 자료에 없고** 앞이 생략 부호로 잘려 있습니다. 또 이 조각의 레이블은 "SAM 템플릿"인데 다른 곳은 모두 "AWS SAM 템플릿"입니다. 이 문서는 이벤트 이름을 `deleteNote` 로 바꾸고 테이블 리소스를 함께 표기했습니다 ([5.5절](#55-delete-함수-템플릿-교정)) | — |
| 터미널 예시의 표기 오류 네 가지 | 프롬프트 `>>`, `_init_.py`, 스마트 쿼트, `Tests` 만 대문자 | ① 셸 프롬프트를 `>>` 로 쓰는데 다른 예시는 모두 `$` 를 씁니다. ② `_init_.py` 는 **`__init__.py`** 의 오기입니다(앞뒤로 밑줄이 두 개). ③ `sam build` 출력의 `functions` 목록 닫는 인용부호가 곧은 아포스트로피가 아니라 **오른쪽 작은따옴표**입니다. ④ 폴더 이름 `events` · `hello_world` 는 소문자인데 **`Tests` 만 대문자**로 시작합니다 ([8.4절](#84-sam-init)) | — |
| 원본 자료의 문장이 성립하지 않음 | Init "내장 애플리케이션이 템플릿이나 사용자 지정 템플릿을 선택할 수 있습니다" · Deploy "구성 파일을 사용할 때 대화형 프롬프트를 통해 배포하려면" | Init 항목은 "내장 애플리케이션 **템플릿**이나 사용자 지정 템플릿을 선택할 수 있습니다"의 오기로 보입니다. Deploy 항목은 인과가 뒤집혀 있습니다. **대화형 배포와 구성 파일 기반 배포는 서로 다른 경로**이고 다른 예시가 이 둘을 별개로 제시합니다 ([8.4절](#84-sam-init) · [9.1절](#91-표준-워크플로)) | — |
| CI/CD 약어 배정과 항목 수 불일치 | 원본 자료가 지속적 통합 = `CI`, 지속적 배포 = `CD` 로 배정하고 **지속적 전달에는 약어를 주지 않음** | 영어에서 `continuous delivery` 와 `continuous deployment` 모두 `CD` 로 줄여 쓰이므로 이 배정은 혼동을 부릅니다. 또 세 프로세스를 제시하는데 **DevOps 방식 6개에는 지속적 배포가 없습니다.** 이 문서는 약어를 **`CI` 하나만** 쓰고 나머지는 전체 용어로 적습니다 ([2.3절](#23-devops-프로세스-세-가지) · [2.5절](#25-devops-방식)) | — |
| 본문 레이블과 항목 제목 불일치 | 여덟 항목 중 세 항목의 이름이 갈림 | 본문 "고객의 요구에 집중" vs 노트 "고객 요구 사항 중심", 본문 "고도의 협업 환경 조성" vs 노트 "고도로 협력적인 환경 조성", 본문 "가능한 한 자동화" vs 노트 "가능한 경우 자동화". 이 문서는 **본문 레이블 기준**으로 통일했습니다 ([2.2절](#22-devops-문화)) | — |
| 표기의 공백 누락 | `모니터링및 로깅` · `커뮤니케이션및 협업` · `코드형` + `인프라` 두 줄 · `AWS::Serverless::Api리소스 유형` | 원본 자료에서 줄바꿈이나 별개 텍스트 조각으로 나뉘어 있던 것이 합쳐진 결과로 보입니다. 정상 표기는 **모니터링 및 로깅 · 커뮤니케이션 및 협업 · 코드형 인프라 · `AWS::Serverless::Api` 리소스 유형**입니다. M12에서도 같은 유형의 붙어쓰기가 발견되었습니다 ([2.5절](#25-devops-방식) · [7.1절](#71-메커니즘-여섯-가지)) | — |
| 리소스 6개 중 4개만 매핑하고 참조 링크가 다른 주제 | 매핑 표가 네 줄이고 두 번째 참조 링크가 API Gateway 리소스 정책 문서 | 원본 자료 본문은 리소스 유형 여섯 개를 제시하는데 매핑 표에는 **`AWS::Serverless::HttpApi` 와 `AWS::Serverless::LayerVersion` 의 대응 서비스가 없습니다.** 특히 `Api` 와 `HttpApi` 를 "Serverless API" 하나로 뭉갭니다. 또 두 번째 참조 링크의 주제는 **API 액세스 제어**입니다. 링크 자체는 유효합니다 ([6.2절](#62-리소스와-aws-서비스-매핑)) | — |
| 모듈 목표 4번에 대응하는 SAM 내용이 없음 | 목표와 요약이 모두 "다양한 **AWS SAM** 배포 전략 설명"을 제시 | 배포 전략 섹션에 **AWS SAM이 한 번도 나오지 않습니다.** 원본 자료 본문은 세 단어이고 설명 전체가 CodeDeploy 쪽이며, 본문(Lambda 전용 배포 구성)과 설명(EC2/온프레미스 중심의 현재 위치·블루/그린, "최소 50%의 인스턴스" 예시)이 **다른 층위를 섞습니다.** 이 문서는 그 자리를 `AutoPublishAlias` 와 `DeploymentPreference` 로 채웠습니다 ([10장](#10-배포-전략)) | — |
| 대안을 요구 사항처럼 서술 | 원본 자료 "애플리케이션의 배포를 사용자 지정하려면 **다음을 수행해야 합니다**" + 세 항목 | AWS DevOps 도구 사용 · 타사 배포 도구 사용 · 자체 솔루션 구현 세 항목은 **택일 대안**인데 "해야 합니다"로 묶여 셋 다 필요한 것처럼 읽힙니다. "다음 중 하나를 선택합니다"가 맞습니다 ([10.8절](#108-롤백-경로-정리)) | — |
| 본문 3단계 vs 설명 4단계 | 한쪽은 세 단계, 다른 쪽은 AWS 계정 생성을 별도로 세어 네 단계 | 또 도커를 "선택 사항"이라고만 적는 자료와 "애플리케이션을 로컬로 테스트할 계획이라면"이라는 조건을 붙이는 자료가 있습니다. **도커가 실제로 필요한 시점**은 로컬 테스트와 컨테이너 빌드입니다 ([8.1절](#81-사전-요구-사항)) | — |
| 정답 표시 상자가 모두 비어 있음 | 참/거짓 6문항의 정답 표시용 텍스트 상자 여섯 개가 전부 비어 있음 | 체크 표시가 애니메이션이나 그림으로만 들어가 있어 **추출 텍스트로는 어느 쪽이 정답인지 알 수 없고 원본 자료로만 판별**할 수 있습니다. M12에서도 같은 구조입니다 | — |
| 해설이 자료에 없는 내용을 근거로 듦 | 해설 "다양한 IDE 및 런타임 조합과 연동하여 로컬에서 코드를 빌드하고 디버깅하는 다양한 AWS 도구 키트가 있습니다" | **AWS 도구 키트를 자세히 다루는 자료가 드뭅니다.** 설치 안내도 CLI 설치만 다룹니다. 현재 AWS SAM 사전 요구 사항 페이지가 (선택) 3단계로 제시하는 도구 키트는 **AWS Toolkit for VS Code** 입니다 ([8.1절](#81-사전-요구-사항)) | — |
| 데모 항목에 설명이 없음 | 항목만 나열되어 있고 설명과 참조 URL이 모두 없음 | 무엇을 어떤 순서로 시연하는지, 무엇을 확인하면 성공인지가 없습니다. M12의 데모 항목에도 설명이 없었지만 그쪽은 참조 URL을 하나 제공했습니다. 또 항목의 "빠른 템플릿"은 `AWS Quick Start Templates` 를 옮긴 것으로 보이는데 용어가 다릅니다. 이 문서는 데모를 명령 단위 절차로 채웠습니다 ([9.9절](#99-데모-절차)) | — |
| 템플릿 검증 기능의 명령 이름이 없음 | 원본 자료가 CLI 기능으로 "AWS SAM 템플릿 파일이 사양에 맞게 작성되었는지 확인합니다"를 들면서 명령 이름을 주지 않음 | 기본 명령 목록에도 없습니다. 해당 명령은 **`sam validate`** 이고 `--lint` 옵션으로 `cfn-lint` 린팅까지 수행합니다 ([8.5절](#85-sam-validate)) | — |
| 모듈 목표와 모듈 요약의 문구 불일치 | 네 항목 중 두 항목의 문구가 다름 | 2번은 "DevOps**가** … 개발 **관행**"과 "DevOps**에서** … 개발 **방식**"으로 조사와 용어가 갈립니다. 3번은 "AWS Serverless Application Model(AWS SAM)"과 "AWS SAM"으로 약어 표기만 다릅니다. 이 문서는 **한 가지 문구 기준**으로 통일했습니다 ([모듈 목표](#모듈-목표)) | — |
| 어젠다의 모듈 12 이름이 M12 덱과 다름 | "애플리케이션 사용자에게 액세스 권한 부여" | M12 자료의 부제는 "내 애플리케이션의 사용자에게 액세스 권한 부여하기"입니다. 같은 모듈 이름이 두 덱에서 다릅니다 ([이 모듈의 위치](#이-모듈의-위치)) | — |
| 용어(Terminology) 정리가 없음 | 모듈 요약 다음에 바로 "감사합니다"로 끝남 | M12에는 용어 정의가 있었습니다. DevOps · 지속적 통합 · 코드형 인프라 · 스택 · 변경 세트 · 배포 구성처럼 이 모듈에서 새로 나오는 용어를 정리하는 자리가 없습니다. 이 문서는 용어 절을 신설하고 정의마다 출처를 붙였습니다 ([1.2절](#12-용어-정리)) | — |
| 표기의 이중 공백과 부사 중복 | "즉  사람이 수동으로" · "서버리스 애플리케이션을  동작시키세요" · "손쉽게 … 쉽게 생성하고" | 두 곳에서 단어 사이에 공백이 두 칸 들어가 있고, CloudFormation 설명은 같은 뜻의 부사가 중복됩니다. 이 문서는 공백을 한 칸으로 정리하고 중복 부사를 제거했습니다 ([2.1절](#21-전통적인-소프트웨어-배포의-문제점) · [3.2절](#32-devops용-aws-도구)) | — |

### 11.2 동작·기본값이 변경된 항목

| 항목 | 교재의 서술 | 현재 | 근거 |
|---|---|---|---|
| SAM 리소스 유형 목록 | 원본 자료가 **6개** 제시(`Api` · `HttpApi` · `Function` · `LayerVersion` · `SimpleTable` · `StateMachine`) | **13개.** 교재의 여섯 개는 모두 현재도 유효하고 일곱 개가 추가되었습니다 — `Application`(중첩 서버리스 애플리케이션) · `CapacityProvider` · **`Connector`**(리소스 간 권한) · `GraphQLApi` · `WebSocketApi` · `MicrovmImage` · `NetworkConnector`. 이 리소스와 속성은 AWS SAM 축약 구문으로 정의하며 SAM은 CloudFormation 리소스·속성 유형도 지원합니다 ([6.1절](#61-리소스-유형-13개)) | [AWS SAM resources and properties](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-resources-and-properties.html) |
| 리소스 간 권한 지정 수단 | `Policies`(관리형 정책 이름)와 `Role`(기존 역할 ARN) **두 가지만** | 교재 이후 **`AWS::Serverless::Connector`** 가 추가되었습니다. 소스 리소스 안에 `Connectors` 블록을 두는 **임베디드 커넥터 구문**과 독립 리소스로 선언하는 구문 두 가지가 있고, 문서는 대부분의 사용 사례에 임베디드 구문을 권장하며 **중첩 스택 리소스나 공유 리소스**처럼 같은 템플릿에 없는 소스를 참조할 때 독립 리소스 구문을 쓰라고 안내합니다. `Permissions` 의 유효 값은 **`Read` 와 `Write`** 두 개이고 SAM이 이 의도 선언을 필요한 IAM 권한으로 바꿉니다 ([6.3절](#63-커넥터)) | [AWS::Serverless::Connector](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-connector.html) |
| SAM CLI 명령 목록 | 원본 자료가 **7개** 제시 | **24개.** 교재에 없는 명령 중 특히 **`sam validate`**(이름 없이 언급되던 기능) · **`sam sync`** · **`sam delete`** · **`sam remote invoke`** · `sam list` · `sam logs` · `sam traces` · `sam publish` · `sam pipeline`(bootstrap · init)이 중요합니다. `sam local` 하위 명령도 셋이 아니라 **여섯**(`callback` · `execution` · `generate-event` · `invoke` · `start-api` · `start-lambda`)입니다 ([8.3절](#83-cli-명령-24개) · [8.6절](#86-sam-local-하위-명령-여섯-개)) | [AWS SAM CLI command reference](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-command-reference.html) |
| 워크플로 단계 | 표준 워크플로가 `sam init` → `sam build` → `sam local invoke` → `sam deploy` **4단계** | 교재 이후 **`sam sync`** 가 추가되었고 **AWS SAM Accelerate** 의 일부입니다. `--watch` 로 실행하면 빌드 후 CloudFormation에 배포하고 계속 실행되며 로컬 변경을 감시하다가, 갱신된 리소스가 AWS 서비스 API를 지원하면 그것으로 빠르게 갱신하고 지원하지 않으면 CloudFormation 배포를 수행합니다. `--code` 로 코드 변경만, `--resource-id` 로 특정 함수·계층만 동기화할 수 있습니다. **문서는 `sam sync` 를 개발 환경 전용으로 권장하고 프로덕션에는 `sam deploy` 또는 CI/CD 파이프라인을 권장**하므로 교재의 4단계 워크플로는 프로덕션 경로로서 여전히 유효합니다 ([9.6절](#96-sam-sync-로-개발-반복-줄이기)) | [Introduction to using sam sync to sync to AWS Cloud](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/accelerate.html) |
| AWS SAM 배포 전략 | 배포 전략 설명 전체가 CodeDeploy 쪽이고 **SAM 템플릿에서 선언하는 방법이 없음** | `AWS::Serverless::Function` 의 **`AutoPublishAlias`**(별칭 자동 생성·버전 게시)와 **`DeploymentPreference`**(`Type` · `Alarms` · `Hooks`)로 선언합니다. 사전 정의 `Type` 값은 아홉 개이고 **SAM 표기는 `All-at-once` 가 아니라 `AllAtOnce`** 입니다. `DeploymentPreference` 를 지정하면 SAM이 스택당 하나인 CodeDeploy 애플리케이션(`ServerlessDeploymentApplication`) · 함수별 배포 그룹 · `CodeDeployServiceRole` 역할을 만듭니다. **첫 점진적 배포는 두 단계**로 해야 합니다 ([10.4절](#104-sam-템플릿에서-배포-전략-선언하기) ~ [10.6절](#106-첫-점진적-배포는-두-단계)) | [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html) |
| 배포 구성의 이름 | 원본 자료가 `Canary` · `Linear` · `All-at-once` **세 단어만** 제시 | Lambda 컴퓨팅 플랫폼용 사전 정의 배포 구성은 **아홉 개**이고 이름이 정해져 있습니다(`CodeDeployDefault.LambdaCanary10Percent5Minutes` 계열 네 개, `CodeDeployDefault.LambdaLinear10PercentEvery1Minute` 계열 네 개, `CodeDeployDefault.LambdaAllAtOnce`). EC2/온프레미스용은 세 개이고 **배포 구성을 지정하지 않으면 `CodeDeployDefault.OneAtATime` 이 쓰입니다.** 또 교재는 컴퓨팅 플랫폼을 두 갈래로만 설명해 **Amazon ECS** 를 빠뜨립니다 ([10.2절](#102-컴퓨팅-플랫폼-세-가지) · [10.3절](#103-사전-정의-배포-구성)) | [Working with deployment configurations in CodeDeploy](https://docs.aws.amazon.com/codedeploy/latest/userguide/deployment-configurations.html) |
...
| `python3.8` 런타임 | **지원 종료.** Deprecated runtimes 표에 지원 종료일이 **2024년 10월 14일**, 함수 생성 차단이 2027년 2월 1일, 함수 업데이트 차단이 2027년 3월 3일로 기재 | **지원되는 Python 런타임**(`python3.12` 이상 권장. `python3.13` · `python3.14` 는 지원 종료 예정일 2029-06-30, `python3.12` 는 2028-10-31). `sam init --runtime` 은 여전히 이 값을 허용 목록에 두지만, 그 값으로 만든 함수는 지원 종료된 런타임을 씁니다. 현재 지원되는 Python 런타임은 `python3.15`(지원 종료 미예정) · `python3.14` · `python3.13` · `python3.12` · `python3.11` · `python3.10` 입니다 ([5.4절](#54-list-함수-템플릿-교정) · [8.4절](#84-sam-init)) | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| `python3.9` 런타임 | **지원 종료.** Deprecated runtimes 표에 지원 종료일이 **2025년 12월 15일**, 함수 생성 차단이 2027년 2월 1일, 함수 업데이트 차단이 2027년 3월 3일로 기재 | 같음. `sam build` 출력 예시가 런타임을 이 값으로 표시합니다. **같은 자료의 다른 예시는 다른 런타임을 쓰므로 자료 안에서도 런타임 표기가 갈립니다** ([8.4절](#84-sam-init)) | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |

### 11.4 이 자료에서 더한 점

| 항목 | 요약 | 왜 더했는가 | 근거 |
|---|---|---|---|
| `AWS::Serverless::Connector` | 두 리소스 사이의 권한을 구성. 임베디드 `Connectors` 블록(권장)과 독립 리소스 구문. `Permissions` 는 `Read` · `Write` 두 값. 세 속성 모두 AWS SAM 고유이며 CloudFormation 대응 속성이 없음 | 권한 지정에 흔히 쓰이는 두 방식보다 실수를 줄이는 세 번째 길이라서 | [AWS::Serverless::Connector](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-connector.html) |
| `AutoPublishAlias` 와 `DeploymentPreference` | 점진적 Lambda 배포. `Type` 사전 정의 값 9개, `Alarms` 로 경보 기반 자동 롤백, `Hooks` 로 `PreTraffic` · `PostTraffic` 검증. 지정하면 CodeDeploy 애플리케이션·배포 그룹·서비스 역할이 자동 생성. **첫 배포는 2단계**, 호출은 **별칭 한정자** 필요 | 모듈 목표 4번이 요구하는 "SAM 배포 전략"의 실체가 이 두 속성이라서 | [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html) |
| `sam sync --watch`(AWS SAM Accelerate) | 로컬 변경을 감시해 가장 빠른 방법으로 동기화. `--code` · `--resource-id` · `--no-watch` · `--skip-deploy-sync`(변경이 없어도 마지막 배포 후 7일 이상이면 배포) · `--watch-exclude`. **개발 환경 전용**이며 실행 시 확인 프롬프트 | 개발 반복 주기를 줄이는 별도 경로가 표준 워크플로에 빠져 있어서 | [Introduction to using sam sync to sync to AWS Cloud](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/accelerate.html) |
| `samconfig.toml` | `sam deploy --guided` 의 산출물이자 이후 배포의 기준. `--config-file` 기본값, 환경은 `--config-env`(기본값 `default`). `sam init` 생성 결과 요약에 포함 | `sam deploy --guided` 의 산출물이자 이후 배포의 기준인데 잘 다뤄지지 않아서 | [Introduction to deploying with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-deploy.html) |
| CloudFormation 변경 세트 | 실행 전 영향 확인, 추가·수정·삭제 리소스와 속성 전후 비교, 배포 전 검증(속성 구문 오류·리소스 이름 충돌·서비스 할당량 한도). 실행 후 그 스택의 모든 변경 세트 제거. **성공을 보장하지는 않음** | 배포가 실제로 이를 거치고 롤백·검토의 근거가 되는데 그림에서 빠져 있어서 | [Update CloudFormation stacks using change sets](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-updating-stacks-changesets.html) |
| `sam deploy` 의 롤백·기능 옵션 | `--on-failure`(`ROLLBACK` 기본 / `DELETE` / `DO_NOTHING`), `--disable-rollback`(둘은 함께 사용 불가), `--capabilities`(`CAPABILITY_IAM` · `CAPABILITY_NAMED_IAM` · 중첩 애플리케이션에는 `CAPABILITY_AUTO_EXPAND`), `--stack-name` 필수, 템플릿이 51,200바이트 초과 시 `--s3-bucket` 또는 `--resolve-s3` 필요, `SAM_CLI_POLL_DELAY` 환경 변수 | "롤백 계획 없음" 문제의 실질적 답인데 옵션이 제시되지 않아서 | [sam deploy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-deploy.html) |
| `sam validate` | 템플릿 유효성 검증. `--lint` 로 `cfn-lint` 린팅(`cfnlintrc` 로 파라미터 지정). 템플릿이 현재 디렉터리에 표준 이름으로 있거나 방금 `sam build` 를 했으면 `--template-file` 불필요 | "템플릿이 사양에 맞는지 확인"이라는 기능의 실제 명령 이름이 빠져 있어서 | [sam validate](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-validate.html) |
| `sam list` | `endpoints`(클라우드·로컬 엔드포인트) · `resources`(배포 시 만들어질 리소스) · `stack-outputs`(스택 출력). **배포 전후 모두** 사용 | "생성한 리소스 표시"를 콘솔 없이 CLI로 확인하는 경로라서 | [sam list](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-list.html) |
| `sam delete` | 스택 · S3·ECR 아티팩트 · 템플릿 파일 삭제. ECR 컴패니언 스택 삭제 여부를 물음. `--no-prompts` 는 컴패니언 스택과 ECR 리포지토리를 **기본 삭제**하고 `--stack-name` 또는 구성 TOML 파일을 요구 | 데모·실습이 리소스를 만들고 끝나 정리 절차가 필요해서 | [sam delete](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-delete.html) |
| `sam local start-lambda` 와 `sam remote invoke` | `start-lambda` 는 **AWS CLI·SDK와 함께 쓸** 로컬 HTTP 서버. `sam remote invoke` 는 **클라우드에 배포된 리소스를 직접 호출.** 교재는 로컬 테스트만 다룸 | 로컬 테스트 너머의 SDK·클라우드 호출 경로가 빠져 있어서 | [AWS SAM CLI command reference](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-command-reference.html) |
| `sam init` 의 추가 옵션 | `--package-type Zip` / `Image`, `--architecture x86_64` / `arm64`, `--dependency-manager`, `--base-image`, `--name`, `--output-dir`, `--no-interactive`, `--tracing`, `--application-insights`(기본 비활성), `--extra-content`. `--location` 은 Git · Mercurial · HTTP/HTTPS · .zip · 경로를 받고 **Git은 리포지토리 루트**를 요구 | 세 옵션만 다루면 컨테이너·아키텍처 등 실무 선택지가 가려져서 | [sam init](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-init.html) |
| `sam local generate-event` 의 두 단계 인자 | 서비스 이름만 주면 이벤트 유형 목록, 서비스와 유형을 함께 주면 샘플 이벤트. `--region` · `--partition` · `--bucket` · `--key` 로 자리표시자 값 수정 | 인자를 어떻게 주는지가 없으면 이벤트를 만들 수 없어서 | [Introduction to testing with sam local generate-event](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-local-generate-event.html) |
| `Auth` 속성(데이터 유형 `ApiAuth`) | `AWS::Serverless::Api` 의 `Properties` 안에 `Auth` → `DefaultAuthorizer` + `Authorizers`. 함수의 `Api` 이벤트에서 **`RestApiId`** 로 명시적 API 참조. 교재 템플릿은 `RestApiId` 없이 암시적 API를 만드는 형태 | "권한 부여를 활성화"만으로는 어떤 속성을 쓰는지 알 수 없어서 | [Amazon Cognito user pool example for AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-controlling-access-to-apis-cognito-user-pool.html) |
| 액세스 제어 메커니즘 선택 지침과 오류 응답 사용자 지정 | 그린필드는 **Amazon Cognito 사용자 풀**, 기존 인증이 있거나 사용자 지정 로직이 필요하면 **Lambda 권한 부여자**(이전 명칭 custom authorizer). API Gateway 오류 응답 사용자 지정은 **`AWS::Serverless::Api` 만** 지원 | 여섯 메커니즘을 표로만 놓으면 무엇을 언제 고르는지 알 수 없어서 | [Control API access with your AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-controlling-access-to-apis.html) |
| `Globals` 의 상속 리소스·재정의 규칙 | 상속 리소스 8종, `Api` 지원 속성 20개, 재정의 규칙(원시 값 대체 / 맵 병합 / 리스트 앞에 추가), **선언한 속성 제거 불가**, 암시적 API 속성 재정의 | `Globals` 를 "전역 변수"로 오해하기 쉬워 정확한 동작이 필요해서 | [Globals section of the AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.html) |
| 변환 결과의 구체적 형태 | 23줄 SAM 템플릿이 200줄 넘는 CloudFormation으로 확장. `Function` → `AWS::Lambda::Function` + `AWS::IAM::Role` + `AWS::Lambda::Permission`, `HttpApi` → `AWS::ApiGatewayV2::Api` + `Stage`, `SimpleTable` → `AWS::DynamoDB::Table`(`id` 해시 키, `PAY_PER_REQUEST`). `SamResourceId` 메타데이터와 `lambda:createdBy=SAM` 태그 | "변환한다"만으로는 무엇이 얼마나 늘어나는지 감이 잡히지 않아서 | [How AWS SAM works](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam-overview.html) |
| Lambda 별칭 가중치 라우팅의 제약 | 별칭은 **최대 두 버전**, 두 버전의 실행 역할·데드 레터 큐 구성 일치, **`$LATEST` 불가**, 트래픽이 적을 때 **실제 비율 편차**. 호출된 버전은 `START` 로그의 `Version` · `x-amz-executed-version` 헤더 · `ExecutedVersion` 차원으로 확인 | 배포 구성이 실제로 무엇을 조작하는지와 제약을 알아야 해서 | [Implement Lambda canary deployments using a weighted alias](https://docs.aws.amazon.com/lambda/latest/dg/configuring-alias-routing.html) |
| AWS Serverless Application Repository | 서버리스 애플리케이션을 찾고 배포하고 게시. 공개·비공개 공유, 매니페스트 파일(AWS SAM 템플릿) 업로드, **AWS Lambda 콘솔과 깊이 통합.** `sam publish` 와 `AWS::Serverless::Application` 이 연결됨. 조회 시점에 정상 제공 중 | `sam publish` · `Application` 리소스와 이어지는데 다뤄지지 않아서 | [What Is the AWS Serverless Application Repository?](https://docs.aws.amazon.com/serverlessrepo/latest/devguide/what-is-serverlessrepo.html) |
| AWS Toolkit for VS Code | SAM 사전 요구 사항의 (선택) 3단계. 시각적 디버깅 · CodeLens 통합 · 간소화된 배포 워크플로. **Visual Studio Code 1.73.0 이상**과 YAML 언어 지원 확장 필요 | "다양한 도구 키트" 언급의 실제 대상이 이 도구라서 | [AWS SAM prerequisites](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/prerequisites.html) |
| 컴퓨팅 플랫폼 세 개와 Lambda 배포의 성격 | 플랫폼은 EC2/On-Premises · AWS Lambda · **Amazon ECS**. **Lambda와 ECS 배포는 현재 위치 배포를 쓸 수 없고 모두 블루/그린**입니다. 블루/그린의 롤백 이점, AppSpec 파일과 CodeDeploy 에이전트, 영역 구성은 EC2 현재 위치 배포 전용 | 두 갈래로만 보면 ECS가 빠지고 Lambda가 왜 블루/그린인지 놓쳐서 | [CodeDeploy primary components](https://docs.aws.amazon.com/codedeploy/latest/userguide/primary-components.html) |
| 로컬 호출의 주의와 도커 의존 | `sam local invoke` 는 **도커로 로컬 컨테이너를 만들어** 함수를 빌드·호출. 로그는 stderr, 결과는 stdout. `.aws-sam/build/<함수>` 를 컨테이너의 `/var/task` 에 마운트. **신뢰할 수 없는 코드에는 쓰지 말라**는 권고 | 도커가 사실상 필수인 시점과 안전 권고가 빠져 있어서 | [Introduction to testing with sam local invoke](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-local-invoke.html) |
| DevOps 방식의 '구성 관리' | `What is DevOps?` 페이지는 인프라 자동화 방식으로 코드형 인프라와 함께 **구성 관리(configuration management)** 를 제시합니다. 흔히 소개되는 6개 목록에는 없습니다 | 인프라 자동화 방식을 여섯 개로만 보면 하나가 빠져서 | [What is DevOps?](https://aws.amazon.com/devops/what-is-devops/) |
| IaC 도구 비교와 SAM 사용 시나리오 | CloudFormation 대신 SAM(템플릿 호환성 + 서버리스 단순화), AWS CDK 대신 SAM(선언적), **둘을 함께** 쓰기(SAM CLI 로컬 테스트로 CDK 보완). 시나리오는 서버리스 애플리케이션 · **CloudFormation 보강** · 로컬 개발·테스트 · 서버리스 CI/CD · **마이그레이션** | 언제 SAM·CloudFormation·CDK를 쓰는지 판단 기준이 없어서 | [What is the AWS Serverless Application Model (AWS SAM)?](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam.html) |
| 템플릿 필수·선택 섹션과 파라미터 우선순위 | **`Transform` 과 `Resources` 만 필수.** 섹션 순서는 자유이나 `AWS::LanguageExtensions` 는 서버리스 변환보다 앞. `--parameter-overrides` 와 구성 파일 항목이 **템플릿 항목보다 우선** | 어떤 섹션이 필수인지, 값이 어디서 우선하는지 구분이 없어서 | [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html) |
| `AWS::Serverless::Function` 의 기본값과 컨테이너 이미지 | `Timeout` 3초, `Architectures` `x86_64`, `PackageType` `Zip`(이때 `CodeUri` 또는 `InlineCode` 필요). `PackageType: Image` 면 **`ImageUri` 만 적용**되고 `Runtime` · `CodeUri` · `InlineCode` 는 무시. `Runtime` 에 `provided` 를 주면 사용자 지정 런타임 빌드 | `Timeout` 등 기본값과 이미지 패키징 규칙을 모르면 자주 걸려서 | [AWS::Serverless::Function](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-function.html) |
| `sam build` 의 옵션과 산출물 위치 | `--build-in-source`(Node.js 런타임 + Makefile·esbuild, `--use-container` 와 비호환) · `--parallel` · `--cached`(`.aws-sam/cache`) · `--exclude` · `--hook-name terraform` 등. 산출물은 `.aws-sam/build`, 빌드된 템플릿은 `.aws-sam/build/template.yaml`. **`sam deploy` 는 이 디렉터리를 배포** | `.aws-sam/build` 를 `sam deploy` 가 배포한다는 관계가 빠져 있어서 | [sam build](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-build.html) |
...
| 항목 | 상태 |
|---|---|
| AWS CodeCommit의 신규 고객 제공 종료 여부 | 개발자 안내서 · FAQ · 문서 검색을 모두 시도했으나 **신규 고객 제공 중단 안내를 찾지 못했습니다.** 추측으로 채우지 않았습니다. 참고로 DevOps용 도구 목록에 CodeCommit은 **등장하지 않습니다** ([3.4절](#34-소스-리포지토리가-목록에-없다)) |
| AWS CodeArtifact · AWS Config · AWS X-Ray · Amazon CloudWatch의 현재 정의 | 한 줄 설명을 대조할 **각 서비스 문서를 조회하지 않았습니다.** 이 문서의 [3.2절](#32-devops용-aws-도구) 표에서 이 네 항목은 교재 서술로만 실었습니다 |
| DevOps 이점 6개 | 참조 페이지는 응답했지만 **가져온 본문에 이점 절이 없어** 근거로 쓰지 않았습니다. [2.4절](#24-devops를-도입하는-이유)의 표는 교재 서술 그대로입니다 |
...
| `sam init` 프로젝트 디렉터리 구조 | 원본 자료가 제시하는 `events` / `hello_world` / `Tests` 트리를 대조할 **생성 결과의 디렉터리 구조**는 확인하지 못했습니다. 생성 결과 **요약**(Name · Architectures · Dependency Manager · Configuration file 등)만 확인했습니다 ([8.4절](#84-sam-init)) |
...
| 지속적 배포(continuous deployment)의 AWS 공식 정의 | 조회한 CodePipeline 문서는 **continuous integration과 continuous delivery만 정의**하므로, "지속적 배포(CD)는 구축에서 프로덕션 배포에 이르는 완전 자동화된 파이프라인입니다"라는 서술을 확인하지 못했습니다 ([2.3절](#23-devops-프로세스-세-가지)) |
| IDE용 AWS 도구 키트 전체 목록 | SAM 사전 요구 사항의 **AWS Toolkit for VS Code 하나만** 확인했습니다. "다양한 AWS 도구 키트"가 언급되지만 그 목록은 확인하지 못했습니다 ([8.1절](#81-사전-요구-사항)) |
