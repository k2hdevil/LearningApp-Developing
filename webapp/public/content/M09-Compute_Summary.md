...
> - 이 모듈은 이 과정에서 강의 당시와 현재의 차이가 가장 큰 모듈입니다. Lambda의 할당량, 런타임 목록, SnapStart 지원 범위, 비동기 페이로드 한도가 그동안 많이 바뀌었습니다.
...
| 섹션 | 이 문서 |
|---|---|
| 컴퓨팅 서비스 | [2장](#2-컴퓨팅-서비스) |
| AWS Lambda 작동 방식 | [3장](#3-aws-lambda-작동-방식) |
| AWS Lambda 호출 모델 | [4장](#4-aws-lambda-호출-모델) |
| 권한 | [5장](#5-권한) |
| 개발 | [6장](#6-개발) |
| 테스트 | [7장](#7-테스트) |
| 배포 | [8장](#8-배포) |
| 데모, 실습 4 | [9장](#9-데모와-실습-4) |
...
AWS는 세 가지 유형의 컴퓨팅을 제공합니다. 인스턴스에서 컨테이너, 서버리스로 갈수록 **추상화 수준**이 높아집니다.
...
3분류(인스턴스 / 컨테이너 / 서버리스)는 현재 결정 안내서의 분류와 부합합니다. 달라진 것은 **선택지의 폭**입니다.
...
🆕 현재 문서는 **Fargate를 Lambda와 나란히 서버리스 컴퓨팅으로 분류**합니다. 컨테이너는 직접 관리하는 EC2 인스턴스에서 실행하거나 AWS 관리형 컴퓨팅인 Fargate에서 실행할 수 있습니다.
...
AWS Fargate는 컨테이너를 서버리스 컴퓨팅 모델로 실행하는 ECS 및 EKS에서 지원됩니다. Amazon EKS 사용 설명서에도 Fargate로 Kubernetes 파드를 실행하는 항목이 있고, Fargate 프로필로 어떤 파드를 Fargate에서 시작할지 제어합니다. Fargate에서 실행되는 각 파드는 자체 컴퓨팅 경계를 가지며 커널·CPU·메모리·ENI를 다른 파드와 공유하지 않습니다.

> — 출처: [Simplify compute management with AWS Fargate (Amazon EKS)](https://docs.aws.amazon.com/eks/latest/userguide/fargate.html)

### 2.3 컴퓨팅 서비스 비교

세 컴퓨팅 서비스를 관점별로 비교하면 다음과 같습니다.
...
두 **서버리스** 옵션 Fargate와 Lambda 사이의 선택은 다음 비교로 판단합니다.
...
Lambda 요금은 "호출당, 실행 시간 및 메모리 사용량"으로 요약되지만, 실제 요금 페이지의 구조는 다음과 같습니다.
...
전체 흐름은 이렇습니다. 이벤트 소스가 Lambda를 호출하면 함수 코드가 실행되고, 코드는 다른 AWS 서비스나 인터넷과 상호 작용한 뒤 선택적으로 응답을 반환합니다.

```text
  ┌──────────────┐   호출   ┌────────────────────────┐  ──────▶  서비스
  │  이벤트 소스     │ ───────▶ │  AWS Lambda (함수 코드)   │  ──────▶  인터넷
  │  (S3, DynamoDB, │          │                        │  ◀──────  선택적 응답
  │   API GW ...)   │          └────────────────────────┘
  └──────────────┘
```
...
이벤트 소스는 다음과 같이 분류할 수 있습니다(전체 목록은 아닙니다).

| 범주 | 예 |
|---|---|
| 데이터 스토어 | Amazon S3, Amazon DynamoDB, Amazon Kinesis, Amazon Cognito |
| 엔드포인트 | Amazon API Gateway, AWS IoT, AWS Step Functions |
| 개발 및 관리 도구 | AWS CloudFormation, AWS CodePipeline, Amazon CloudWatch |
...
- 🔄 **Amazon Alexa**와 **AWS CloudTrail**은 현재 표에 없습니다. 표에 없다는 것이 호출이 불가능하다는 뜻은 아니므로 단정하지 않습니다(`AddPermission` API에는 여전히 "Alexa Smart Home 함수용" `EventSourceToken` 파라미터가 있습니다).
- 🔄 "Amazon CloudWatch"는 표에서 **Amazon CloudWatch Logs**로 구체화되었습니다.
- 🆕 표에 추가된 항목: Amazon DocumentDB, Amazon MQ, Amazon MSK, 셀프 매니지드 Apache Kafka, Application Load Balancer, Amazon EventBridge Scheduler, Amazon Data Firehose, Amazon Lex, Amazon S3 Batch, Amazon VPC Lattice, Amazon Connect Customer, AWS Config, AWS CodeCommit.
...
Lambda 함수는 **트리거 + 액세스 권한 + 코드(핸들러 함수, 기타) + 런타임 + 레이어 + 구성(동시성, 메모리, 시간제한)** 으로 구성됩니다.
...
런타임을 "Node.js, Java, Python, .NET Core, Go, Ruby"로 나열하기도 하지만 **이 목록은 현재와 다릅니다.** `.NET Core` 계열과 `go1.x`는 지원이 종료되었습니다.
...
런타임 폐기는 실무에서 가장 자주 부딪히는 주제이므로 정리합니다.
...
"Python 및 Node.js 런타임에는 SDK가 내장되어 있으므로 코드와 번들링할 필요가 없다"는 서술은 **전제는 맞지만 결론이 현재 권장과 반대입니다.**
...
🔄 `--payload`에 JSON 문자열을 그대로 넘길 때 **AWS CLI 버전 2에서는 `--cli-binary-format raw-in-base64-out` 옵션이 필요합니다.** 기본값으로 만들려면 `aws configure set cli-binary-format raw-in-base64-out`을 실행합니다. 출력 파일은 내용이 없지만 명령을 실행하면 생성되고, Lambda가 이벤트를 대기열에 추가할 수 없으면 오류 메시지가 명령 출력에 나타납니다.
...
🆕 **배치 동작**을 알아 둡니다. Lambda는 다음 세 조건 중 하나가 충족될 때 함수를 호출합니다.
...
Lambda는 새 레코드에 대해 스트림을 **기본 속도(base rate)로 초당 4회** 폴링합니다.
...
비동기 호출에서 **구성 가능한 것은 재시도 횟수와 이벤트 최대 수명이고, 재시도 간격 자체를 지정하는 파라미터는 없습니다.**
...
호출 레코드 대상은 **다섯 가지**이고, 비동기 실패 처리의 기본 경로는 DLQ가 아니라 온-실패 대상입니다(아래 참조).
...
실행 환경 수명 주기는 다음 단계로 흐릅니다. 앞의 두 단계가 **콜드 스타트**, 호출 단계가 **웜 스타트**입니다.
...
동시성을 제어하는 방법은 두 가지이며 **목적이 서로 다릅니다.**
...
이 항목은 한때 **"버스트 동시성"** 으로 불렸고 값은 "함수당 10초당 최대 1,000회 동시 실행"이었습니다. 값은 그대로이고 이름이 바뀌었습니다.
...
콜드 스타트를 줄이는 방법은 두 가지입니다.

| 방법 | 설명 |
|---|---|
| Lambda 함수 예약 | 함수를 특정 주기로 실행하는 규칙을 만들어 둡니다. EventBridge의 시간 기반 트리거로 함수를 미리 깨워 둡니다 |
| 프로비저닝된 동시성 | 지정한 개수만큼 Lambda 실행 환경을 미리 초기화해 웜 스타트 상태로 이벤트를 대기시킵니다 |

EventBridge로 함수를 일정에 따라 실행하는 방법은 다음 문서에 정리되어 있습니다.
...
콜드 스타트를 줄이는 두 번째 방법이 Lambda SnapStart입니다. 핵심은 다음과 같습니다.

- 시작 성능을 크게 향상
- 새 버전 출시 시 함수를 초기화
- 캐시된 스냅샷에서 실행 환경을 재개

동작 원리는 이렇습니다. 함수 버전을 게시할 때 Lambda가 함수를 초기화하고, 초기화된 실행 환경의 메모리·디스크 상태에 대한 **Firecracker MicroVM 스냅샷**을 촬영하고 암호화하고 짧은 지연 시간 액세스를 위해 캐시합니다. 처음 호출하거나 호출이 확장될 때 처음부터 초기화하는 대신 캐시된 스냅샷에서 실행 환경을 재개합니다. Lambda는 복원력을 위해 각 스냅샷의 사본 여러 개를 유지하고 최신 런타임·보안 업데이트를 자동으로 패치합니다.

SnapStart는 처음 Java 관리형 런타임 전용으로 출시된 뒤 지원 범위와 요금이 넓어졌습니다. 현재 기준은 다음과 같습니다.

| 항목 | 현재 |
|---|---|
| 지원 런타임 🔄 | **Java 11 이상, Python 3.12 이상, .NET 8 이상** |
| 성능 표현 🔄 | **"최대 1초 미만(as low as sub-second) 시작 성능"** |
| 요금 🔄 | **Java 관리형 런타임만 추가 비용 없음.** 그 밖에는 스냅샷 캐싱 요금(최소 3시간)과 복원 요금 |
...
Lambda 권한은 크게 두 가지로 나뉩니다.

| 권한 유형 | 무엇을 위한 것인가 |
|---|---|
| 호출 권한 | 이벤트 소스가 Lambda 함수와 통신하는 데 필요한 권한입니다. 호출 모델(Push 또는 Pull)에 따라 실행 역할 또는 리소스 기반 정책으로 부여합니다. 리소스 기반 정책은 `AddPermission` API로 문장을 추가하거나 전체 JSON 정책으로 관리합니다 |
| 실행 권한 | Lambda 함수가 계정 내 다른 AWS 리소스에 액세스하는 데 필요한 권한입니다. IAM 역할(**실행 역할**)을 생성해 부여합니다 |

같은 내용을 액세스 주체 기준으로 다시 정리하면 다음과 같습니다.
...
권한 모델 문서 경로는 현재 `lambda-permissions.html`입니다.

### 5.2 같은 계정에서도 호출 권한은 필요하다 🔄

호출 주체에게는 계정 관계와 무관하게 함수를 호출할 권한이 있어야 합니다. "같은 계정이면 명시적 권한이 필요 없다"는 흔한 오해와 달리, 같은 계정의 IAM 사용자·역할도 함수를 호출하려면 권한이 필요합니다. 호출 문제 해결 문서의 서술입니다.
...
같은 계정이면 리소스 기반 정책을 별도로 추가하지 않아도 되는 경우는 있지만, 자격 증명 기반 정책의 `lambda:InvokeFunction` 권한까지 생략할 수는 없습니다.

> — 출처: [Troubleshoot invocation issues in Lambda](https://docs.aws.amazon.com/lambda/latest/dg/troubleshooting-invocation.html)

### 5.3 푸시 또는 직접 호출 모델

푸시 모델에서는 이벤트 소스가 이벤트를 Lambda로 푸시하거나 함수를 직접 호출합니다. 이 경로에 관여하는 권한 요소는 세 가지입니다.

```text
이벤트 소스 ──(1. 이벤트 푸시 / 직접 호출)──▶ [리소스 기반 정책] ──▶ Lambda
                                                                    │
                                              (2. 이벤트 처리)      │
   서비스 ◀──── [IAM 실행 역할 + 권한 정책] ◀────────────────────────┘
```
...
| 요소 | 역할 |
|---|---|
| 리소스 기반 정책 | 다른 계정 및 AWS 서비스에 Lambda 리소스를 쓸 권한을 부여합니다 |
| IAM 실행 역할 | 함수에 AWS 서비스·리소스 액세스 권한을 부여합니다 |
| 권한 정책 | 실행 역할이 필요한 리소스 권한을 부여합니다 |

Amazon S3 트리거를 예로 들면, S3가 함수를 호출하려면 함수의 **리소스 기반 정책**에서 권한이 필요하며, Lambda 콘솔에서 S3 트리거를 구성하면 콘솔이 버킷 이름과 계정 ID가 일치할 때 S3가 함수를 호출할 수 있도록 리소스 기반 정책을 수정합니다. S3에서 알림을 구성하는 경우에는 Lambda API로 정책을 업데이트합니다. 함수가 SDK로 S3 리소스를 관리한다면 **실행 역할에도 S3 권한**이 필요합니다.
...
호출 권한을 부여하는 방법은 **두 가지**이며, 현재는 전체 JSON 정책 방식이 권장됩니다.
...
실행 역할은 함수에 다른 AWS 리소스 액세스 권한을 부여하는 IAM 역할입니다. 세부는 다음과 같습니다.
...
폴링(Pull) 모델에서는 Lambda가 이벤트 소스를 폴링해 이벤트를 가져오고, 처리 결과를 다른 서비스로 보냅니다. 이 두 방향 모두에 **IAM 실행 역할**이 관여합니다.

```text
Lambda ──(1. 이벤트 소스 폴링)──▶ 이벤트 소스
   │
   └──(2. 이벤트 처리)──▶ 서비스
   ▲
   └── IAM 실행 역할 (양쪽 모두에 필요)
...
이벤트 소스 매핑으로 함수를 호출할 때 Lambda는 **실행 역할을 사용해 이벤트 데이터를 읽습니다.** 따라서 스트림·큐에서 읽을 권한을 실행 역할에 부여해야 합니다.

🆕 이때 쓰는 AWS 관리형 정책 이름은 다음과 같습니다.
...
Lambda에는 **AWS 관리 콘솔 / AWS CLI / AWS SDK / 개발 도구**로 접근합니다. 현재 Lambda 개발 도구 문서가 제시하는 도구는 다음과 같습니다.
...
PyCharm용 툴킷은 현재 **AWS Toolkit for JetBrains**의 일부입니다. JetBrains 툴킷은 CLion(C·C++), GoLand(Go), IntelliJ(Java), WebStorm(Node.js), Rider(.NET), PhpStorm(PHP), PyCharm(Python), RubyMine(Ruby), DataGrip(데이터베이스)용 툴킷을 포함하고, Lambda 함수를 원격·로컬에서 생성·업데이트·실행·디버그할 수 있습니다.
...
코드는 Lambda 함수로 업로드하거나 Lambda 콘솔의 코드 편집기에서 직접 작성할 수 있습니다. "콘솔 인라인 편집은 3MB까지"라는 한도는 과거 이야기이며, **현재 할당량 표에 3MB 항목은 없습니다.** 코드 편집기 사용 조건은 다음 두 가지입니다.
...
함수를 계획할 때 고려하는 세 가지 축입니다.

| 항목 | 내용 |
|---|---|
| 프로그래밍 모델 | 프로세스, 스레드, `/tmp`, 소켓을 쓸 수 있고 AWS SDK를 사용합니다 |
| 스테이트리스(stateless) | 외부 스토리지에 데이터를 유지하고, 기본 인프라에 대한 선호도나 액세스 권한을 가정하지 않습니다 |
| 모니터링 및 로깅 | 요청·오류·스로틀 지표, Amazon CloudWatch Logs 내장 로그, AWS X-Ray 통합을 활용합니다 |

계획 단계의 권장 사항입니다.

- 일반 언어와 추가 스레드·프로세스 생성 같은 운영 체제 기능을 쓸 수 있습니다.
- 함수 코드 안에서 지표를 직접 만들지 말고 **Lambda 지표와 CloudWatch 경보**를 씁니다. 예상 런타임을 기준으로 경보를 구성할 수 있습니다.
- 로깅 라이브러리와 Lambda 지표·차원으로 애플리케이션 오류(ERR, ERROR, WARNING)를 파악합니다.

런타임 내장 SDK는 최신성 측면에서 주의가 필요하므로 [3.6절](#36-런타임에-포함된-aws-sdk)에서 따로 다뤘습니다.

### 6.4 AWS X-Ray 통합 🆕

X-Ray 통합의 세부 동작을 정리합니다.
...
핸들러는 세 요소로 구성됩니다. **핸들러 함수**(호출 시 실행할 함수), **이벤트 객체**(호출 시 전송되는 데이터), **컨텍스트 객체**(현재 런타임 환경 정보 제공).
...
Amazon S3가 함수로 보내는 이벤트의 JSON 구조입니다. 아래 예제에서 `principalId` 값만 자리표시자로 바꿨습니다. IAM 주체 고유 ID는 비밀값이 아니지만 액세스 키와 형태가 같아 비밀값 스캐너가 오탐하기 때문입니다. 나머지 값은 공식 문서 예제 그대로입니다.
...
🆕 함께 알아 둘 주의 사항: **함수를 트리거한 것과 같은 버킷에 함수가 쓰면 무한 루프가 발생할 수 있습니다.** 버킷을 두 개 쓰거나 수신 객체용 접두사로 트리거 범위를 제한합니다. 또 `s3.object.key`는 **URL 인코딩된** 객체 키 이름이므로 그대로 텍스트로 쓰면 공백이 있는 이름(`james beswick.jpg`)에서 `NoSuchKey` 오류가 납니다.

> — 출처: [Process Amazon S3 event notifications with Lambda](https://docs.aws.amazon.com/lambda/latest/dg/with-s3.html)

### 6.7 재귀 루프 감지 🆕

재귀 코드에 대한 전통적인 대응은 "코드를 고치는 동안 함수의 동시 실행 한도를 즉시 0으로 설정해 모든 호출을 막는 것"이었습니다. **현재 Lambda는 특정 유형의 재귀 루프를 자동으로 감지해 중지합니다.**
...
**앞의 수동 조치도 여전히 유효합니다.** DynamoDB가 포함된 루프처럼 감지되지 않는 조합이 있으므로, 예약된 동시성을 0으로 설정하는 조치와 CloudWatch 경보·청구 경보·AWS Cost Anomaly Detection을 병행하는 것이 문서의 권장입니다.
...
컨텍스트 객체는 세 언어 모두에서 "남은 시간"을 반환하는 항목을 제공합니다. 세 언어의 컨텍스트 문서는 이 항목을 "**시간 초과 전에 남은 밀리초 수를 반환한다**"고만 기술하고 **고정 기본값을 두지 않습니다.** 이 값은 함수에 구성된 시간 제한을 기준으로 계산되므로 언어별 기본값이 따로 있는 것이 아닙니다. 함수 시간 제한 자체의 기본값이 3초, 최대가 900초(15분)입니다.
...
🔄 Python 컨텍스트에도 **`identity`와 `client_context`가 있습니다.** `identity`는 `cognito_identity_id`·`cognito_identity_pool_id`를, `client_context`는 `client.installation_id`·`client.app_title`·`client.app_version_name`·`client.app_version_code`·`client.app_package_name`·`custom`·`env`를 포함합니다.
...
`com.amazonaws.services.lambda.runtime` 패키지는 AWS SDK for Java v1이 아니라 **`aws-lambda-java-core`** 라이브러리이며 **현재 공식 예제도 같은 임포트를 씁니다.**
...
**`DefaultLambdaJsonSerializer`(SystemTextJson)는 현재 공식 컨텍스트 문서 예제에서도 그대로 쓰입니다.**
...
이 코드는 세 가지 패턴을 보여 줍니다. 이벤트에서 정보를 끌어오고, 환경 변수에서 설정을 읽고, 핵심 로직에서 핸들러를 분리합니다.
...
| 항목 | 내용 |
|---|---|
| 형식 | 라이브러리·사용자 지정 런타임·기타 종속성이 포함된 **.zip 파일 아카이브** |
| 용도 | 같은 함수의 여러 버전 또는 여러 함수 간에 코드 공유. 스토리지 사용량이 줄고 배포 패키지가 작아져 콜드 스타트 동안 더 빠르게 다운로드됩니다 |
| 버전 | **배포된 레이어는 변경 불가능한 버전**입니다. 새 레이어를 만들면 버전 1이 생성되고 업데이트를 게시할 때마다 번호가 증가하며, 각 레이어 버전은 고유 ARN으로 식별되고 함수에 추가할 때 **정확한 레이어 버전을 지정**해야 합니다 |
| 개수 | **함수당 최대 5개**. 표준 Lambda 배포 크기 제한에 포함됩니다 |
| 공유 | 빌드-배포 과정에서 압축해 넣으면 다른 함수·계정·서드 파티와 손쉽게 공유하거나 변경 사항을 전파할 수 없습니다. 레이어로 분리하면 이 공유가 쉬워집니다 |

🆕 함께 알아 둘 사항:

- 레이어를 추가하면 Lambda가 실행 환경의 **`/opt` 디렉터리**로 레이어 내용을 추출합니다. 네이티브로 지원되는 모든 런타임에 `/opt` 하위 특정 디렉터리 경로가 포함되어 있습니다.
- **레이어는 .zip 파일 아카이브로 배포한 함수에만** 쓸 수 있습니다. 컨테이너 이미지로 정의한 함수는 레이어를 쓰지 않고 런타임과 모든 종속성을 이미지에 패키지합니다.
...
레이어에는 **AWS 관리형 레이어**와 **파트너 또는 서드 파티 레이어**가 있습니다. 구체적 이름은 확인하지 못한 범위이므로 추가하지 않았습니다([10.5절](#105-검증하지-못한-항목)).

> — 출처: [Managing Lambda dependencies with layers](https://docs.aws.amazon.com/lambda/latest/dg/chapter-layers.html)

### 6.12 환경 변수 🔄

환경 변수의 기본 성질과 설정 명령입니다.
...
🔄 **덮어쓰기 주의는 실습 시나리오에 직접 영향을 줍니다.** 함수 생성 시 `TABLE_NAME`을 설정해 두고, 이후 구성 업데이트에서 `MP3_BUCKET_NAME`·`Notes_Table`만 지정하면 `TABLE_NAME`은 **사라집니다.** 기존 값을 유지하려면 `get-function-configuration`으로 현재 값을 조회해 함께 넘기고, 조회 결과의 `RevisionId`를 `update-function-configuration`에 전달하면 읽고 쓰는 사이에 값이 바뀌는 것을 막을 수 있습니다.
...
함수를 만들 때 명령 세트 아키텍처를 선택할 수 있습니다. **현재는 모든 지원 런타임이 x86_64와 arm64를 함께 지원합니다.**
...
Lambda를 HTTP 엔드포인트로 호출하는 방법은 두 가지입니다. Amazon API Gateway를 앞에 두는 방법과, 함수에 직접 함수 URL을 붙이는 방법입니다.
...
함수가 결과를 조금씩 흘려보내는 응답 스트리밍의 세부입니다.
...
.zip 함수를 CLI로 만드는 명령을 볼 때 흔히 걸리는 두 가지 함정이 있습니다. 아래는 그 함정을 담은 형태입니다.
...
| 함정 | 내용 |
|---|---|
| 🔄 `-–role` | 하이픈 두 개가 아니라 **하이픈 하나 + en dash(`–`)** 로 조판되기 쉬워 그대로 붙여 쓰면 셸이 인식하지 못합니다. 모든 플래그는 하이픈 두 개여야 합니다 |
| 🔄 `python3.8` | **2024년 10월 14일 지원 종료.** 함수 생성 차단은 2027년 2월 1일, 함수 업데이트 차단은 2027년 3월 3일입니다 |

위 명령의 역할 ARN 계정 ID는 문서 예시용 `111122223333`으로 표기했습니다. `--role`·`--runtime` 함정과는 무관하며 명령의 동작에 영향이 없습니다.
...
AWS CLI에서 `fileb://` 문자열은 파일 데이터가 **바이너리 객체**임을 나타냅니다.
...
함수 생성 응답에 보이는 `"MemorySize": 128`은 현재도 기본 메모리 값입니다. 128MB는 **가능한 최솟값이자 기본값**이며 이벤트를 변환·라우팅하는 단순한 함수에만 쓰도록 권장됩니다([7.2절](#72-테스트-목표와-메모리cpu-관계)).
...
별칭과 버전을 함께 쓰면 API Gateway 단계 변수로 어느 버전을 호출할지 전환할 수 있습니다. 예를 들어 `Prod`·`Test` 두 별칭을 두고, API Gateway 쪽에서 `stageVariables.useAlias`로 `...:dictate-function:${stageVariables.useAlias}`를 구성하면 런타임에 단계 변수가 별칭 이름으로 대체됩니다.
...
🔄 표기 주의: 미게시 버전의 정확한 표기는 **`$LATEST`**(대문자)입니다.
...
별칭은 특정 버전 하나를 가리키는 데 그치지 않습니다. **하나의 별칭이 두 버전 사이로 트래픽을 나눌 수 있습니다.**
...
배포 패키지(함수 코드)에서 공유할 종속성을 분리해 레이어로 올리면, 함수 코드를 보다 효과적으로 테스트하고 디버그할 수 있습니다.
...
핵심은 **메모리를 늘리면 그에 비례해 CPU 용량도 늘어난다**는 점입니다.
...
콘솔에서 테스트를 실행하면 Lambda가 테스트 이벤트로 함수를 **동기식으로** 호출합니다. 입력이 필요 없으면 빈 문서 `{}`를 씁니다. 테스트 이벤트에는 두 종류가 있습니다.
...
AWS SAM을 AWS Toolkit·디버거와 함께 쓰면 함수를 로컬에서 테스트하고 디버그할 수 있습니다. `sam local`의 하위 명령은 네 개입니다.
...
🆕 놓치기 쉬운 중요한 사실: **함수 코드나 런타임이 오류를 반환해도 Lambda 응답의 상태 코드는 200 OK입니다.** 오류가 있다는 사실은 `X-Amz-Function-Error` 헤더로만 표시됩니다. 400·500 계열 상태 코드는 **호출 오류에 예약**되어 있습니다. 즉 상태 코드 200만 보고 성공했다고 판단하면 함수 오류를 놓칩니다.
...
시스템이 재시도에 의존한다면 코드가 **멱등적**이어야 합니다. ESM 문서는 "각 이벤트를 최소 한 번 처리하며 중복 처리가 발생할 수 있으므로 함수 코드를 멱등적으로 만들 것을 **강력히 권장**한다"고 기술합니다.
...
| `lambda:InvokeFunction not authorized` | 호출 주체에게 `lambda:InvokeFunction` 권한이 없습니다([5.2절](#52-같은-계정에서도-호출-권한은-필요하다)) |
...
🔄 위 표의 **".NET Core"** 는 현재 런타임 이름이 아닙니다([3.4절](#34-지원-런타임)). 지금은 `.NET 8`·`.NET 10`입니다.
...
컨테이너 이미지로 배포하는 흐름은 네 단계입니다.

```text
1. 프로젝트·코드 생성
2. AWS 기본 이미지로 컨테이너 빌드 (Lambda 런타임 인터페이스 클라이언트 포함)
3. 컨테이너를 Amazon ECR로 업로드 (최대 비압축 10GB)
4. 컨테이너 이미지로 Lambda 함수 생성
```
...
Lambda 할당량은 2021년 이후 여러 항목이 바뀌었습니다. 아래 표는 현재 값을, 이전에 흔히 인용되던 값과 나란히 둔 것입니다.

| 리소스 | 이전 값(2021년 9월경) | 현재 | 판정 |
|---|---|---|---|
| 메모리 할당 | 128~10,240MB | 128MB~10,240MB, **1MB 단위**. 1,769MB에서 1 vCPU | 그대로 |
| 최대 런타임(시간 제한) | 15분 | 900초(15분) | 그대로 |
...
| 동시 실행 | 1,000개 | 기본 할당량 1,000, **수만 단위까지 증액 가능**. 신규 계정은 축소된 값에서 시작 | 🔄 |
...
함수의 동시성은 특정 시점에 요청을 처리하는 인스턴스의 수이고, 필요한 인스턴스 수가 갑자기 늘어나는 것을 **버스트**라고 합니다.
...
### 9.1 데모: AWS 콘솔에서 Lambda 함수 생성

콘솔에서 함수를 만들어 보며 다음을 살펴봅니다.
...
### 9.2 실습 4 개요: AWS Lambda를 사용한 솔루션 개발

이 실습은 노트 애플리케이션을 소재로 합니다. 사용자가 Delete / Dictate / Search / List / Create·update 동작을 호출하면 Notes 테이블, MP3 호스팅 버킷, Amazon Polly가 함께 동작합니다.
...
실습 코드·명령을 그대로 따라 할 때 부딪히기 쉬운 지점을 모았습니다. 근거는 각 절에 있습니다.
...
| 호출 권한 | 같은 계정이어도 `lambda:InvokeFunction` 권한이 필요합니다 | [5.2절](#52-같은-계정에서도-호출-권한은-필요하다) |
...
이 모듈은 이 과정에서 delta가 가장 큰 모듈입니다. 교재 할당량 표가 **"2021년 9월 현재"** 기준이었고, 그 이후 런타임 목록이 크게 바뀌었으며 SnapStart·함수 URL·응답 스트리밍·재귀 루프 감지처럼 그 뒤에 추가된 기능이 많습니다.

### 10.1 교재와 다른 점

| 항목 | 교재의 서술 | 확인된 내용 | 근거 |
|---|---|---|---|
| 같은 계정의 호출 권한 | "사용자 정의 애플리케이션과 이 애플리케이션이 호출하는 Lambda 함수가 같은 AWS 계정에 속한 경우 명시적인 권한을 부여할 필요가 없습니다" | 계정 관계와 무관하게 호출 주체에게 **`lambda:InvokeFunction` 권한이 필요**합니다. 사용자가 Lambda 리소스에 액세스하면 Lambda는 자격 증명 기반 정책과 리소스 기반 정책을 **모두** 평가하고, AWS 서비스가 호출하면 리소스 기반 정책만 평가합니다. 권한이 없으면 "is not authorized to perform: lambda:InvokeFunction" 오류가 발생하며, 이 요구 사항은 함수를 호출하는 Lambda 함수와 다른 컴퓨팅 리소스에도 적용됩니다 | [Managing permissions in AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/lambda-permissions.html) |
| 컨텍스트 "남은 시간" 기본값 | Python `(기본값 = 3초)`, Java·.NET `(기본값 = 15분)` | 세 언어의 컨텍스트 문서는 모두 **"시간 초과 전에 남은 밀리초 수를 반환한다"** 고만 기술하고 기본값을 명시하지 않습니다. 이 값은 함수에 구성된 시간 제한을 기준으로 계산되므로 언어별 기본값이 따로 있는 것이 아닙니다. 함수 시간 제한 자체의 기본값이 3초, 최대가 900초이므로 Python은 기본값을, Java·.NET은 최대값을 옮긴 것으로 보이는 **내부 불일치**입니다 | [Python 컨텍스트 객체](https://docs.aws.amazon.com/lambda/latest/dg/python-context.html) |
| Python 컨텍스트 목록 | `function_name`~`log_stream_name` 7개 속성만 | 현재 문서는 Python 컨텍스트에도 **`identity`(모바일 앱)와 `client_context`(모바일 앱)** 가 있다고 기술합니다. `identity`는 `cognito_identity_id`·`cognito_identity_pool_id`를, `client_context`는 `client.installation_id`·`app_title`·`app_version_name`·`app_version_code`·`app_package_name`·`custom`·`env`를 포함합니다 | [Python 컨텍스트 객체](https://docs.aws.amazon.com/lambda/latest/dg/python-context.html) |
| CLI 플래그 `-–role` | 하이픈 하나 + en dash(`–`) | 셸이 인식하지 못합니다. 다른 플래그는 모두 하이픈 두 개입니다. 같은 코드 블록의 출력 JSON에도 `"State": "Active", -n:dictate-function",` 처럼 잘린 조각이 섞여 있습니다. 조판 오류이므로 AWS 문서 근거 없이 표기만 교정했습니다 | — ([10.5절](#105-검증하지-못한-항목)) |
| 콜드 스타트 최소화 제목 중복 | 두 절의 제목이 모두 "콜드 스타트 최소화하기 1/2" | 앞 절은 예약 트리거와 프로비저닝된 동시성, 뒤 절은 Lambda SnapStart를 다루므로 뒤 절이 **"2/2"** 여야 합니다. 이 자료는 두 절을 1/2·2/2로 구분했습니다 | — ([10.5절](#105-검증하지-못한-항목)) |

### 10.2 동작·기본값이 변경된 항목

| 항목 | 교재의 서술 | 현재 | 근거 |
|---|---|---|---|
| 지원 런타임 목록 | "Node.js, Java, Python, .NET Core, Go, Ruby" | 지원 런타임 표는 Node.js(22·24·26 프리뷰), Python(3.10~3.15 프리뷰), Java(8·11·17·21·25), **.NET(8·9·10)**, Ruby(3.3·3.4·4.0), OS 전용 런타임(`provided.al2023`)입니다. `.NET Core` 계열과 `go1.x`는 지원 종료되었고 Go·Rust는 OS 전용 런타임으로 실행합니다. Fargate·Lambda 결정 안내서는 네이티브 지원 언어에 **PowerShell**도 포함합니다 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| 비동기식 호출 페이로드 | 256KB | **1MB.** 동기식 6MB는 교재와 같고, 스트리밍 응답 200MB와 요청 라인+헤더 값 합계 1MB가 추가되었습니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| `/tmp` 스토리지 | 10GB (고정값처럼 제시) | **512MB~10,240MB를 1MB 단위로 구성**하는 설정입니다. 512MB까지 추가 비용이 없고 그 이상은 GB-초로 과금되며, SnapStart는 512MB 초과를 지원하지 않습니다 | [Configure ephemeral storage](https://docs.aws.amazon.com/lambda/latest/dg/configuration-ephemeral-storage.html) |
| 배포 패키지 "콘솔 인라인 편집 3MB" | 3MB | 현재 할당량 표에 이 항목이 **없습니다.** 표에는 Lambda API·SDK 업로드 50MB(압축), **콘솔 업로드 50MB**, 압축 해제 250MB가 있습니다. 코드 편집기 사용 조건은 **인터프리터 언어 런타임(Python·Node.js·Ruby) + 압축 해제 50MB 미만**이고, 컨테이너 이미지 함수는 콘솔에서 편집할 수 없습니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| "버스트 동시성" | 항목 이름 | 수치(함수·리전당 10초당 실행 환경 1,000개)는 같고 **이름이 바뀌었습니다.** 현재는 **동시성 조정 속도(concurrency scaling rate)** 이고 할당량 표에서는 동시성 조정 한도입니다. 쓰지 않은 조정 속도는 누적되지 않습니다 | [Lambda scaling behavior](https://docs.aws.amazon.com/lambda/latest/dg/scaling-behavior.html) |
| 동시 실행 한도 1,000 | 고정 한도처럼 서술 | **증액 가능한 기본 할당량**입니다(수만 단위까지). **신규 AWS 계정은 동시성·메모리 할당량이 축소된 상태로 시작**해 사용량에 따라 자동 상향됩니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| SnapStart 지원 런타임 | "Java 11 및 Java 17 관리형 런타임" | **Java 11 이상, Python 3.12 이상, .NET 8 이상.** 그 밖의 관리형 런타임, OS 전용 런타임, 컨테이너 이미지는 지원하지 않습니다 | [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) |
| SnapStart "추가 비용 없이" | 조건 없이 무료 | **Java 관리형 런타임만** 추가 비용이 없습니다. 그 밖에는 버전별 스냅샷 캐싱 요금(**최소 3시간** 과금)과 복원 요금이 발생하며 함수 메모리에 따라 달라집니다 | [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) |
| SnapStart "최대 10배 향상" | 배수 표현 | 현재 문서는 배수 대신 **"최대 1초 미만(as low as sub-second) 시작 성능"** 으로 기술합니다. 대규모 호출에서 가장 효과적이고 호출 빈도가 낮은 함수는 같은 개선을 보지 못할 수 있다고 명시합니다. 10배가 틀렸다고 단정할 근거는 찾지 못했고 **현재 문서에 그 표현이 없다는 것만** 확인했습니다 | [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) |
| 비동기 호출 레코드 대상 | AWS Lambda, SNS, SQS, EventBridge 네 개 | 이 네 개 **+ Amazon S3 버킷(실패 시에만)** 총 다섯 개. 대상별로 실행 역할에 `sqs:SendMessage`·`sns:Publish`·`s3:PutObject`+`s3:ListBucket`·`lambda:InvokeFunction`·`events:PutEvents` 권한이 필요합니다 | [Capturing records of asynchronous invocations](https://docs.aws.amazon.com/lambda/latest/dg/invocation-async-retain-records.html) |
| 비동기 재시도 구성 | "재시도 횟수 및 재시도 간격은 구성 가능합니다" | 구성 가능한 것은 **재시도 횟수(`MaximumRetryAttempts`, 0~2)와 이벤트 최대 수명(`MaximumEventAgeInSeconds`, 최대 6시간)** 이며 재시도 간격 자체를 지정하는 파라미터는 없습니다. 기본 재시도 간격은 첫 두 시도 사이 1분, 두 번째와 세 번째 사이 2분입니다 | [Configuring error handling settings](https://docs.aws.amazon.com/lambda/latest/dg/invocation-async-configuring.html) |
| 런타임 내장 SDK | "Python 및 Node.js용 런타임에는 SDK가 내장되어 있으므로 번들링할 필요가 없습니다" | 내장 사실은 맞고 **Ruby도 포함**하지만 결론이 반대입니다. 현재 문서는 종속성 제어와 자동 런타임 업데이트 중 하위 호환성을 위해 SDK 모듈을 **배포 패키지나 레이어에 항상 포함**하도록 권장하고, 내장 SDK는 콘솔 코드 편집기·CloudFormation 인라인 코드처럼 추가 패키지를 넣을 수 없을 때만 쓰라고 기술합니다 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| 코드 저장 위치 | "AWS Lambda는 Amazon S3에 코드를 저장하고 저장된 데이터를 암호화합니다" | 저장 중 암호화는 맞습니다. 저장 위치는 **Lambda 관리형 스토리지**(계정·리전당 300GB, 압축 해제 기준)가 기본이고 사용자 S3 버킷을 쓰는 **자체 관리형 S3 코드 스토리지**가 선택 옵션입니다. 자체 관리형을 쓰면 Lambda가 소스 코드 사본을 저장하지 않습니다 | [Data encryption at rest](https://docs.aws.amazon.com/lambda/latest/dg/security-encryption-at-rest.html) |
| 재귀 코드 대응 | "동시 런타임 한도를 즉시 0으로 설정" | Lambda가 **재귀 루프를 자동 감지해 중지**합니다(같은 요청 체인에서 약 16회, 기본 활성·무료). 단 감지 범위는 함수 자신·SQS·S3·SNS 사이의 루프와 Lambda 함수만으로 구성된 루프이고 **DynamoDB 등이 포함되면 감지하지 못하므로** 교재의 수동 조치도 여전히 유효합니다 | [Recursive loop detection](https://docs.aws.amazon.com/lambda/latest/dg/invocation-recursion.html) |
| `invoke --payload` | JSON 문자열을 그대로 전달 | AWS CLI **버전 2에서는 `--cli-binary-format raw-in-base64-out`** 이 필요합니다. `aws configure set cli-binary-format raw-in-base64-out`으로 기본값으로 만들 수 있습니다 | [Invoking a function asynchronously](https://docs.aws.amazon.com/lambda/latest/dg/invocation-async.html) |
| 이벤트 소스 목록 | Amazon Alexa, AWS CloudTrail, Amazon CloudWatch 포함 | 현재 "Lambda 함수를 호출할 수 있는 서비스" 표(28개)에 **Amazon Alexa와 AWS CloudTrail이 없습니다**(표에 없다는 것이 불가능하다는 뜻은 아니므로 단정하지 않습니다). "Amazon CloudWatch"는 **Amazon CloudWatch Logs**로 구체화되었습니다. 교재는 "이에 국한되지 않습니다"라는 단서를 붙였으므로 목록 자체가 틀린 것은 아닙니다 | [Invoking Lambda with events from other AWS services](https://docs.aws.amazon.com/lambda/latest/dg/lambda-services.html) |
| `.NET Core` 열 이름 | ".NET Core" | 현재 런타임 이름은 `.NET 8`·`.NET 9`(컨테이너 전용)·`.NET 10`입니다. `.NET Core` 계열 런타임은 모두 지원 종료되었습니다 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| `$Latest` 표기 | `$Latest` | 문서 표기는 **`$LATEST`** 입니다 | [Manage Lambda function versions](https://docs.aws.amazon.com/lambda/latest/dg/configuration-versions.html) |
| 옛 문서 경로 | `limits.html`, `intro-permission-model.html`, `dg/API_AddPermission.html`, `dg/API_CreateEventSourceMapping.html` | 모두 **리디렉션으로 살아 있습니다.** `limits.html`→`gettingstarted-limits.html`, `intro-permission-model.html`→`lambda-permissions.html`, `dg/API_*.html`→`api/API_*.html`. API 참조가 개발자 안내서에서 별도 문서로 분리되었으므로 새 경로를 직접 인용하는 것이 정확합니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| 호출 권한 부여 방법 | `AddPermission` API만 | 현재 문서는 **전체 JSON 정책(`PutResourcePolicy`)을 권장 경로**로 제시하고 `AddPermission`은 개별 `Allow` 문장 추가용으로 위치시킵니다. 전체 JSON 정책은 IAM 글로벌 조건 키 전체와 명시적 `Deny`를 쓸 수 있고 최대 20KB입니다. `put-resource-policy`는 기존 정책 전체를 **덮어씁니다** | [Working with resource-based policies](https://docs.aws.amazon.com/lambda/latest/dg/access-control-resource-based.html) |
| DLQ를 비동기 모범 사례로 제시 | "비동기식 호출에 대한 모범 사례는 DLQ를 생성 및 사용하는 것" | 현재 문서는 DLQ를 **온-실패 대상의 대안**으로 제시합니다. 온-실패 대상은 지원 대상이 더 많고 함수 응답 세부 정보를 호출 레코드에 포함하며 함수·버전·별칭 단위로 구성할 수 있습니다. DLQ는 함수 수준만 가능하고 이벤트 내용만 보냅니다. DLQ 자체는 여전히 지원됩니다 | [Capturing records of asynchronous invocations](https://docs.aws.amazon.com/lambda/latest/dg/invocation-async-retain-records.html) |
| 서버리스 컴퓨팅 분류 | 서버리스 항목에 Lambda만, Fargate는 보조 설명에만 | 현재 결정 안내서는 **Fargate를 Lambda와 나란히 서버리스 컴퓨팅으로 분류**합니다. 또 AWS Batch, Elastic Beanstalk, ECS/EKS Anywhere, Lightsail 같은 다른 컴퓨팅 선택지도 함께 제시합니다 | [Choosing an AWS compute service](https://docs.aws.amazon.com/decision-guides/latest/compute-on-aws-how-to-choose/choosing-aws-compute-service.html) |
| 개발 도구 | 서드 파티 플러그인(Eclipse, PyCharm, Visual Studio) | 현재 개발 도구 문서는 로컬 개발 도구로 **AWS Toolkit for VS Code**를 제시하고 Eclipse 툴킷은 언급하지 않습니다. PyCharm용 툴킷은 **AWS Toolkit for JetBrains**의 일부입니다. IaC(SAM·CDK·CloudFormation), GitHub Actions, Powertools for AWS Lambda도 함께 제시합니다 | [Development tools for Lambda](https://docs.aws.amazon.com/lambda/latest/dg/tools-to-develop-deploy-manage.html) |

### 10.3 비권장·지원 종료된 항목

| 항목 | 상태 | 대체 | 근거 |
|---|---|---|---|
| `python3.8` 런타임 | **2024년 10월 14일 지원 종료.** 함수 생성 차단 2027년 2월 1일, 업데이트 차단 2027년 3월 3일 | 현재 지원 런타임 표의 Python 런타임(`python3.12`·`python3.13`·`python3.14` 등). 표의 예상 폐기일은 계획 목적이므로 채택 전에 다시 확인 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| `.NET Core` 런타임 | 전 계열 지원 종료. `dotnetcore3.1` 2023-04-03, `dotnetcore2.1` 2022-01-05, `dotnetcore1.0` 2019-06-27, `dotnetcore2.0` 2019-05-30 | `.NET 8`(`dotnet8`), `.NET 10`(`dotnet10`). 컨테이너 전용으로는 `dotnet9` | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| Go 1.x 런타임 | **2024년 1월 8일 지원 종료**, 함수 생성 차단 2024년 2월 8일. Lambda는 Go 언어 자체는 계속 지원 | OS 전용 런타임(`provided.al2023`)에 Go 실행 파일 배포. 컨테이너는 AWS OS 전용 기본 이미지 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| AWS Toolkit for Eclipse 사용 설명서 링크 | 인용 URL이 **HTTP 404**. `/toolkit-for-eclipse/v1/user-guide/`는 AWS Toolkit for JetBrains 사용 설명서로 리디렉션. 현재 개발 도구 문서에 Eclipse 툴킷이 없습니다. 관련해 AWS SDK for Java 1.x는 2025년 12월 31일 지원 종료 | AWS Toolkit for VS Code. JetBrains IDE는 AWS Toolkit for JetBrains, Visual Studio는 AWS Toolkit for Visual Studio | [Development tools for Lambda](https://docs.aws.amazon.com/lambda/latest/dg/tools-to-develop-deploy-manage.html) |
| `csharp-package-toolkit.html` | 개별 페이지로 남아 있지 않고 개발자 안내서 루트로 리디렉션됩니다(상태 200이지만 도착 지점이 원래 항목이 아님) | Lambda 개발 도구 문서 또는 AWS Toolkit for Visual Studio 사용 설명서 | [Development tools for Lambda](https://docs.aws.amazon.com/lambda/latest/dg/tools-to-develop-deploy-manage.html) |
| boto3 리소스 인터페이스 | 비권장. AWS Python SDK 팀이 **새 기능을 추가할 계획이 없고** 새 서비스 기능은 client 인터페이스로만 제공됩니다. 기존 인터페이스는 boto3 수명 주기 동안 계속 동작 | `boto3.client('dynamodb')`. 리소스 인스턴스는 스레드 안전하지 않아 스레드마다 새로 만들어야 합니다 | [Boto3 Resources](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/resources.html) |

### 10.4 이 자료에서 더한 점

| 항목 | 요약 | 왜 더했는가 | 근거 |
|---|---|---|---|
| 런타임 폐기 정책·일정 | 폐기 예고 최소 180일 → 폐기일(콘솔 생성·업데이트 불가, CLI·SAM·CFN 가능) → 최소 30일 뒤 생성 차단 → 최소 60일 뒤 업데이트 차단. 폐기 후에도 호출은 무기한 가능. 컨테이너 이미지 함수에는 **폐기 알림이 제공되지 않습니다** | 런타임 지원 종료가 이 모듈 최대의 변경이므로, 언제 무엇이 차단되는지 알아야 대응 시점을 잡을 수 있습니다 | [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) |
| 함수 URL | `https://<url-id>.lambda-url.<region>.on.aws` 전용 HTTP(S) 엔드포인트. 리소스 기반 정책과 CORS, 인증 유형 `AWS_IAM`·`NONE`. 별칭 또는 `$LATEST`에만 적용. 퍼블릭 인터넷 전용(PrivateLink 미지원) | HTTP 호출 경로가 API Gateway 하나뿐이라는 인식을 바로잡는, 그 뒤에 나온 기능이기 때문입니다 | [Lambda function URLs](https://docs.aws.amazon.com/lambda/latest/dg/lambda-urls.html) |
| 응답 스트리밍 | 함수 URL·`InvokeWithResponseStream`·API Gateway 프록시 통합으로 최대 200MB 스트리밍. 첫 6MB 무제한, 이후 2MBps. 관리형 런타임 중 Node.js만 지원. 콘솔 테스트에서는 항상 버퍼링으로 보임 | 6MB 응답 상한을 넘는 방법으로 자주 찾게 되는 기능이라 함께 정리했습니다 | [Response streaming](https://docs.aws.amazon.com/lambda/latest/dg/configuration-response-streaming.html) |
| 재귀 루프 감지 | X-Ray 트레이싱 헤더로 요청 체인 추적. 같은 체인 약 16회에서 다음 호출 중지. 기본 활성·무료. 감지 범위는 함수 자신·SQS·S3·SNS. 최소 SDK 버전 필요 | 재귀 코드 대응이 "동시성 0"뿐이라는 옛 안내가 지금은 절반만 맞기 때문입니다 | [Recursive loop detection](https://docs.aws.amazon.com/lambda/latest/dg/invocation-recursion.html) |
| arm64(Graviton2) | 모든 지원 런타임이 x86_64·arm64를 함께 지원. 가격·성능 이점. 종속성·레이어·확장의 arm64 호환성 확인 필요. `Architectures` 미지정 시 기본 `x86_64` | 아키텍처 선택은 함수 생성 시의 기본 결정인데 기존 설명에는 없어 보완했습니다 | [Instruction set architecture](https://docs.aws.amazon.com/lambda/latest/dg/foundation-arch.html) |
| 가중치 별칭 카나리 배포 | 하나의 별칭이 최대 두 버전으로 트래픽 분할. 두 버전은 실행 역할·DLQ 구성이 같고 둘 다 게시되어야 함. `x-amz-executed-version` 헤더와 `START` 로그로 확인. CodeDeploy·SAM으로 롤링 배포(`AutoPublishAlias`·`DeploymentPreference`) | 별칭이 버전 하나만 가리킨다는 설명을 넘어, 실제 배포에서 쓰는 안전 장치이기 때문입니다 | [Weighted alias canary deployments](https://docs.aws.amazon.com/lambda/latest/dg/configuring-alias-routing.html) |
| 예약된 동시성의 정확한 의미 | 최대치와 **최소치를 함께** 설정하는 제어. 다운스트림 과부하 방지에도 씁니다. **추가 요금 없음**. 프로비저닝된 동시성은 유료이고 `$LATEST`에 쓸 수 없으며 상한은 미예약 동시성 − 100 | 예약·프로비저닝 두 동시성을 혼동하기 쉬워 목적·요금·제약을 구분했습니다 | [Understanding Lambda function scaling](https://docs.aws.amazon.com/lambda/latest/dg/lambda-concurrency.html) |
| 실행 환경 `Init` 10초 제한 | `Init` 단계는 10초 제한. 초과하면 첫 호출 시점에 구성된 시간 제한으로 재시도(suppressed init). 프로비저닝된 동시성·SnapStart·Managed Instances는 130초 또는 구성된 시간 제한 중 더 큰 값(최대 900초) | 초기화가 무거운 함수의 시간 초과 원인을 실습에서 직접 만나기 때문입니다 | [Execution environment lifecycle](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtime-environment.html) |
| 콜드 스타트 통계 | 전체 호출의 **1% 미만**에서 발생, 지속 시간 100ms 미만~1초 초과. 호출 빈도가 낮은 개발·테스트 함수에서 더 흔함 | 콜드 스타트의 정의만이 아니라 빈도·지속 시간을 알아야 실제 영향을 가늠할 수 있습니다 | [Execution environment lifecycle](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtime-environment.html) |
| ESM 배치 동작·프로비저닝 모드 | 배치 창(0~300초, Kinesis·DynamoDB·SQS 기본 0초, Kafka·MQ·DocumentDB 기본 500ms), 배치 크기, 페이로드 6MB 중 하나로 호출. 최소 한 번 처리이므로 멱등성 필수. 프로비저닝 모드는 MSK·Kafka·SQS에서 폴러 최소·최대치 지정 | 폴링 호출 모델을 실제로 튜닝할 때 필요한 값들이기 때문입니다 | [Event source mappings](https://docs.aws.amazon.com/lambda/latest/dg/invocation-eventsourcemapping.html) |
| DynamoDB ESM 세부 사항 | 초당 4회 폴링은 **기본 속도**. `ParallelizationFactor` 1~10(기본 1)으로 샤드당 동시 처리, 항목 수준 순서 유지. 시작 위치는 `TRIM_HORIZON` 권장. 단일 리전 테이블은 샤드당 최대 2개 함수, 글로벌 테이블은 1개 권장 | 스트림 처리량을 늘릴 때 가장 먼저 만지는 설정이라 구체값을 남겼습니다 | [Using Lambda with DynamoDB](https://docs.aws.amazon.com/lambda/latest/dg/with-ddb.html) |
| 공유 가능 테스트 이벤트 | 비공개(함수당 10개)와 공유 가능 두 종류. 공유 가능은 `lambda-testevent-schemas` EventBridge 스키마 레지스트리에 저장되고 관련 API 권한이 모두 필요. `sam remote test-event`로도 호출 | 팀에서 테스트 이벤트를 공유하려다 권한 문제로 막히는 일이 잦기 때문입니다 | [Testing Lambda functions in the console](https://docs.aws.amazon.com/lambda/latest/dg/testing-functions.html) |
| `sam local` 하위 명령 | `generate-event`, `invoke`, `start-api`, `start-lambda` | 로컬 테스트를 실제로 돌리려면 하위 명령의 구분이 필요하기 때문입니다 | [Testing with sam local](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-local.html) |
| 환경 변수 한도·주의 | 총 4KB, 키는 문자 시작·2자 이상·문자·숫자·`_` 만. **`update-function-configuration`은 `Variables` 구조 전체를 대체**. 민감 정보는 Secrets Manager 권장. 기본 AWS 관리형 키(무료) 또는 고객 관리형 키, 전송 중 암호화 헬퍼 | 전체 대체 동작을 모르면 실습에서 기존 변수를 잃기 때문입니다 | [Environment variables](https://docs.aws.amazon.com/lambda/latest/dg/configuration-envvars.html) |
| 레이어 세부 사항 | `/opt` 디렉터리로 추출. **.zip 함수에만** 사용 가능. 내장 SDK 버전 고정 용도. Go·Rust에는 비권장 | 레이어의 제약과 용도를 알아야 언제 쓸지 판단할 수 있기 때문입니다 | [Lambda layers](https://docs.aws.amazon.com/lambda/latest/dg/chapter-layers.html) |
| 컨테이너 이미지 기본 이미지 세 갈래 | AWS 기본 이미지(런타임 인터페이스 에뮬레이터 포함), AWS OS 전용 기본 이미지, AWS가 아닌 기본 이미지. **패키지 유형(.zip ↔ 이미지)은 기존 함수에서 변경 불가.** 다중 아키텍처 이미지 미지원. AL2023 전환 | 컨테이너 배포를 고를 때의 선택지와 되돌릴 수 없는 제약이기 때문입니다 | [Container images](https://docs.aws.amazon.com/lambda/latest/dg/images-create.html) |
| 버전 게시를 유발하는 변경 목록 | 코드·환경 변수·런타임·핸들러·레이어·메모리·시간 제한·VPC·DLQ·IAM 역할·설명·아키텍처·임시 스토리지·패키지 유형·코드 스토리지 모드·로깅·파일 시스템·SnapStart·트레이싱. **예약된 동시성은 유발하지 않음.** 버전 번호는 재사용되지 않음 | 무엇이 새 버전을 만드는지 알아야 버전 관리가 예측 가능해지기 때문입니다 | [Manage function versions](https://docs.aws.amazon.com/lambda/latest/dg/configuration-versions.html) |
| X-Ray 세부 사항 | `Active`·`PassThrough` 모드. 샘플링 초당 1건 + 5%(구성 불가). 트레이스당 2개 세그먼트(`AWS::Lambda`, `AWS::Lambda::Function`). MSK·Kafka·MQ·DocumentDB ESM 미지원 | 항목 이름만 있던 X-Ray 통합을 실제로 읽을 수 있게 세부를 채웠습니다 | [Visualize invocations using AWS X-Ray](https://docs.aws.amazon.com/lambda/latest/dg/services-xray.html) |
| Lambda API 요청 할당량 | 동기 호출은 인스턴스당 초당 10건(총 호출 한도 = 동시성 × 10). `GetFunction` 초당 100, `GetPolicy` 초당 15, 그 밖의 제어 영역 API 전체 합쳐 초당 15(모두 증액 불가) | 스로틀링 오류를 만났을 때 어느 한도에 걸렸는지 판단할 근거이기 때문입니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| 그 밖의 새 할당량 | 환경 변수 4KB, 리소스 기반 정책 20KB, 스트리밍 대역폭 2MBps, 실행 환경당 네트워크 625Mbps, 컨테이너 이미지 설정 16KB, 테스트 이벤트 10개, 파일 설명자·스레드 1,024, Lambda 관리형 코드 스토리지 300GB, VPC당 ENI 500 | 옛 할당량 표에 없던 항목들이라 현재 값으로 채웠습니다 | [Lambda quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) |
| 요금 구조 세분화 | x86·Arm 별도 요금 표와 GB-초 계층 요금, 프리 티어 월 100만 요청 + 400,000 GB-초, 임시 스토리지 512MB 초과분 과금, 프로비저닝된 동시성·SnapStart 별도 과금, Lambda MicroVMs 인스턴스-초 과금 | 콜드 스타트·프로비저닝 동시성의 비용 영향을 이해하려면 과금 구조가 필요하기 때문입니다 | [AWS Lambda pricing](https://aws.amazon.com/lambda/pricing/) |
| Lambda durable functions | 체크포인트·재생 실행으로 **최대 1년** 실행되는 워크플로. 대기 기간에는 컴퓨팅 요금이 발생하지 않습니다. SDK는 JavaScript·TypeScript·Python·Java·C#(.NET) | 15분 실행 한도라는 통념을 넘는, 그 뒤에 나온 실행 모델이기 때문입니다 | [AWS Fargate or AWS Lambda?](https://docs.aws.amazon.com/decision-guides/latest/decision-guides/fargate-or-lambda.html) |
| Lambda Managed Instances / MicroVMs | Managed Instances는 다양한 EC2 인스턴스 유형에서 함수를 실행하면서 Lambda의 운영 단순성을 유지합니다. MicroVMs는 신뢰할 수 없는 코드를 실행하는 세션 기반 격리 샌드박스로 별도 컴퓨팅 프리미티브입니다 | 컴퓨팅 선택지가 넓어진 최신 변화라 존재와 위치를 남겼습니다 | [AWS Fargate or AWS Lambda?](https://docs.aws.amazon.com/decision-guides/latest/decision-guides/fargate-or-lambda.html) |
| ABAC로 함수 액세스 제어 | 함수에 태그를 붙이고 API 요청이나 IAM 주체에 같은 태그를 지정해 IAM 정책의 조건 요소로 함수 액세스를 제어 | 자격 증명·리소스 기반 정책 외에 현재 문서가 제시하는 세 번째 액세스 제어이기 때문입니다 | [Managing permissions in AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/lambda-permissions.html) |
...
| AWS 관리형 레이어의 구체적 이름 | 레이어에는 "AWS 관리형 레이어"와 "파트너 또는 서드 파티 레이어"가 있지만 이름을 밝힌 자료를 확인하지 못했습니다. 레이어 문서의 개요·버전·패키징 항목은 확인했지만 **AWS가 제공하는 관리형 레이어의 목록이나 대표 예를 명시한 페이지는 조회하지 않았습니다.** 이름을 추측해 넣지 않았습니다 |
...
| CLI `-–role` 조판 오류 | 셸이 인식하지 못하는 표기 오류입니다. AWS 문서로 확인할 성질의 사실이 아니라 교재 자체의 조판 문제이므로 표기만 교정하고 근거 인용은 붙이지 않았습니다. 같은 코드 블록의 출력 JSON에 섞인 `"State": "Active", -n:dictate-function",` 조각도 같습니다 |
| 콜드 스타트 최소화 제목 중복 | 같은 이유로 문서 검증 대상이 아닙니다. 두 절을 1/2·2/2로 구분했습니다 |
| 함수 생성 응답 JSON의 나머지 필드 | `LastUpdateStatus`·`RevisionId`·`State`·`PackageType` 같은 필드가 현재 `CreateFunction` 응답에도 있는지는 `CreateFunction` **요청** 문서까지만 확인했고 **응답 요소 전체 목록은 조회하지 않았습니다.** `State`·`StateReason`·`StateReasonCode`가 `GetFunctionConfiguration` 응답에 있다는 것은 확인했습니다 |
...
| 실습 4의 상세 절차 | 원본 실습 자료에는 아키텍처 다이어그램과 목표만 있고 단계별 절차 텍스트가 없습니다. 요약할 원문이 없어 이 자료에서는 목표와 [9.3절](#93-실습에서-걸릴-수-있는-지점)의 주의 지점만 다뤘습니다 |
