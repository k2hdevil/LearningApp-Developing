...
> - 예시 액세스 키 ID 는 `AKIA####ODNN7EXAMPLE` 처럼 **5~8번째 글자를 `#` 로 가렸습니다.** 자격 증명 스캐너가 실제 키로 오인하는 것을 막기 위한 것입니다.
...
이 모듈은 Amazon S3와 그 기본 구성 요소를 다룹니다. AWS 스토리지 유형 중 S3의 자리를 잡고, S3의 핵심 개념(버킷·객체·키·스토리지 클래스), 데이터 보호와 액세스 제어, 그리고 AWS SDK·AWS CLI로 S3에서 작업하기 위한 개발 환경 구성까지 이어집니다.

### 이 모듈로 할 수 있게 되는 것

- Amazon Simple Storage Service(Amazon S3)의 기본 개념을 설명한다
- Amazon S3로 데이터를 보호하는 옵션을 나열한다
- 코드의 SDK 종속성을 정의한다
- Amazon S3 서비스에 연결하는 방법을 설명한다
- 요청 및 응답 객체를 설명한다

스토리지를 프로그래밍 방식으로 사용하는 방법과 정적 웹 사이트 배포는 모듈 6이 이어서 다룹니다.
...
버킷 이름의 고유성 범위는 **파티션 단위**입니다. 버킷 이름은 한 파티션 내 모든 AWS 계정의 모든 리전에서 고유해야 합니다. AWS 파티션은 현재 4개입니다.
...
범용 버킷 이름 규칙은 다음과 같습니다. 소문자·숫자·하이픈뿐 아니라 **마침표(.)도 허용됩니다.**
...
SQS·SNS·Lambda 대상으로 게시할 수 있는 이벤트 유형은 다음과 같습니다. 객체 생성·제거·복원·복제 외에도 수명 주기, 태그, ACL 관련 유형이 있습니다.
...
이제 기본 설정 자체가 ACL을 끕니다. S3 Object Ownership은 **버킷 수준 설정**으로, 업로드된 객체의 소유권을 제어하고 ACL을 활성화·비활성화합니다.
...
신규 코드에서는 `addressing_style = path` 같은 경로 스타일 설정 대신 SDK 기본값(가상 호스팅 스타일)을 사용하세요.
...
웹사이트 엔드포인트는 리전에 따라 두 형식 중 하나를 따릅니다.
...
버킷을 만들 때 리전을 지정하지 않으면 기본 리전에 생성됩니다. **`LocationConstraint`를 지정하지 않으면 버킷은 US East(N. Virginia) 리전(`us-east-1`)에 생성됩니다.** 위 출력에서 `notes-bucket`의 `LocationConstraint`가 `null`인 것이 `us-east-1`에 있다는 뜻입니다.
...
**설정 값을 지정하는 방법의 우선순위**(작업별 파라미터 → 환경 변수 → 공유 자격 증명 파일 → 구성 파일)와, 자격 증명을 찾는 **자격 증명 공급자 체인**은 다른 개념입니다. 후자가 더 넓습니다.
...
- 보안 인증: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`
- 리전: `AWS_REGION`
...
> 🔄 `addressing_style = path` 같은 경로 스타일 설정은 [5.2절](#52-경로-스타일-url)의 이유로 넣지 않았습니다. SDK 기본값인 가상 호스팅 스타일을 사용하세요.
...
> 아래는 AWS SDK for Java 2.x 문법입니다. 요청 객체도 빌더로 생성합니다.
...
각 항목은 S3를 실무 수준으로 이해하는 데 필요해서 더했습니다.

| 더한 항목 | 왜 더했는가 | 근거 |
|---|---|---|
| S3 Glacier Instant Retrieval | 스토리지 클래스 선택지가 늘어 아카이브 밀리초 검색이라는 새 선택을 알아야 하므로 더했습니다. 분기 1회 액세스 아카이브용 | [스토리지 클래스](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html) |
| S3 Express One Zone | 지연에 민감한 워크로드의 새 선택지라 더했습니다. 단일 AZ 한 자리 밀리초, 디렉터리 버킷 전용 | [스토리지 클래스](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html) |
| 계정 리전 네임스페이스 | 버킷 이름 스쿼팅을 막는 최신 권장 방식이라 더했습니다. 내 계정만 버킷을 만들 수 있는 예약 네임스페이스(`-an` 접미사) | [범용 버킷 네임스페이스](https://docs.aws.amazon.com/AmazonS3/latest/userguide/gpbucketnamespaces.html) |
| 조건부 요청 | last-writer-wins 경합을 애플리케이션 코드 없이 다루는 방법이라 더했습니다. 조건부 읽기·쓰기·삭제, 추가 요금 없음 | [조건부 요청](https://docs.aws.amazon.com/AmazonS3/latest/userguide/conditional-requests.html) |
| 이벤트 알림 유형 확장 | 이벤트 유형을 4종으로만 알면 수명 주기 삭제 등을 놓치므로 더했습니다. 수명 주기·Intelligent-Tiering·태그·주석·ACL 이벤트 | [이벤트 유형](https://docs.aws.amazon.com/AmazonS3/latest/userguide/notification-how-to-event-types-and-destinations.html) |
| 액세스 포인트 연결 대상 확장 | 액세스 포인트가 버킷 외 대상에도 붙으므로 더했습니다. FSx for NetApp ONTAP·OpenZFS 볼륨, AWS Backup의 S3 복구 지점 | [액세스 포인트](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points.html) |
| 버킷 생성 시 태그 지정 | 생성 시점 태깅과 그 권한·조건을 실무에서 알아야 하므로 더했습니다. `s3:TagResource` 권한 필요, 태그 조건은 ABAC 활성화 후 적용 | [CreateBucket API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateBucket.html) |
...
| 항목 | 상태 |
|---|---|
| 파일 스토리지 세부 목록 | 교재는 파일 스토리지 세부 목록을 두 곳에서 서로 다르게 적습니다(한쪽은 EFS Standard·EFS Infrequent Access·FSx for Lustre·FSx for NetApp ONTAP·FSx for OpenZFS, 다른 쪽은 EFS·FSx for Windows File Server·FSx for Lustre). 이 문서에서는 서비스명만 남기고 세부 목록은 제외했습니다. EFS·FSx 문서로 별도 검증이 필요합니다 |
