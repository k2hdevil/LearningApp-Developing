...
> - 🔄 강의 당시와 달라져 교정한 항목입니다. 무엇이 어떻게 달라졌는지는 [11장](#11-교재-대비-변경-사항)에 모아 두었습니다.
> - 검증일: 2026년 9월 1일. 이후 문서가 갱신될 수 있으니 시험·실무 적용 전에는 링크된 원문을 확인하세요.
> - 용어를 통일했습니다. `authorization` 은 **권한 부여**, `identity pool` 은 **자격 증명 풀**, `issue` 는 **발급**입니다. 같은 대상을 여러 이름으로 부르는 경우가 있어, 토큰을 나열할 때는 항상 **ID → 액세스 → 갱신** 순서를 씁니다 ([11.1절](#111-교재와-다른-점)).
...
세 번째 항목인 **구현 모범 사례**는 이름만 제시될 뿐 대응하는 설명이 원본 강의 자료에 없습니다. 이 문서는 그 자리를 [9장](#9-구현-모범-사례)에 공식 문서로 확인한 모범 사례로 채웠습니다.

### 이 모듈의 위치

이 모듈은 3일차 두 번째 모듈입니다. 앞의 모듈에서 만든 API와 애플리케이션에 사용자를 붙이는 자리이고, 그대로 실습 6(캡스톤)으로 이어집니다.
...
이 모듈은 열 개 주제로 나뉩니다. 이 문서도 같은 순서를 따릅니다.

| 주제 | 이 문서 |
|---|---|
| 표지 · 어젠다 · 모듈 목표 | [1장](#1-모듈-개요) |
| 인증 및 권한 부여의 복잡성 | [2장](#2-인증과-권한-부여) |
| Amazon Cognito | [3장](#3-amazon-cognito) |
| 사용자 액세스 관리 | [4장](#4-사용자-풀-만들기) · [5장](#5-속성-그룹-범위) |
| 사용자 풀을 통해 애플리케이션에 대한 액세스 권한 부여 | [6장](#6-토큰) |
| 자격 증명 풀을 통해 애플리케이션에 대한 액세스 권한 부여 | [7장](#7-자격-증명-풀) |
| API 액세스 보안 | [8장](#8-api-액세스-보안) |
| 데모 | [10.4절](#104-데모-노트) |
| 실습 6: 캡스톤 - 애플리케이션 구축 완료 | [10장](#10-애플리케이션과-실습-6) |
| 요약 | [2.4절](#24-용어-정리) |

이 모듈의 원본 자료는 **다이어그램 중심**이라 코드 예시가 거의 없습니다. 이 문서는 인증 흐름 이름, 기능 요금제, 토큰 폐기, JWT 검증, 할당량처럼 원본이 다루지 않는 영역을 공식 문서로 채웠습니다.

### 1.1 이 모듈에서 가장 크게 달라진 것 🆕

강의 당시 이후 Amazon Cognito에 추가·변경된 것이 많습니다. 가장 먼저 마주칠 여섯 가지를 미리 짚어 둡니다. 각 항목의 근거는 해당 절에 있습니다.
...
| 인증 흐름 | "인증 흐름 지원" 한 줄. 흐름 이름을 하나도 제시하지 않음 | 앱 클라이언트의 `ExplicitAuthFlows` 로 허용 흐름을 골라야 하고, 나중에 추가된 **선택 기반 로그인(`USER_AUTH`)** 이 암호 없는 로그인·패스키의 유일한 진입 흐름입니다 ([4.7절](#47-인증-흐름)) |
| JWT 페이로드 | "암호화된 정보" | ID·액세스 토큰의 페이로드는 **암호화되지 않고 base64url 로 인코딩되어 서명**만 붙습니다. 암호화되는 것은 refresh 토큰입니다 ([6.3절](#63-jwt-구조와-교재-예시-교정)) |
...
두 개념을 나란히 놓고 봅니다. `authorization` 은 이 문서 전체에서 **권한 부여**로 통일합니다.
...
권한 부여의 예시 동작은 노트 게시 · 노트 편집 · 노트 삭제입니다. 이 세 동작이 그대로 [10장](#10-애플리케이션과-실습-6)의 Pollynotes API가 됩니다. 정리하면 사용자는 자신의 자격 증명을 증명해야 원하는 작업을 수행할 수 있고, 사용자 자격 증명은 애플리케이션에 통합됩니다.
...
개발자가 직접 떠안아야 하는 세 영역이 있고, 각 영역은 설계 시 답해야 하는 질문으로 되돌아옵니다.
...
이 세 영역은 솔루션을 구현할 때 개발자가 직면하는 보안 책임이자 당면 과제이고, Amazon Cognito가 바로 이 인증 및 권한 부여를 해결하는 솔루션입니다.
...
SAML 2.0 · OAuth 2.0 · OIDC를 이름만 나열하면 사용자 풀과 자격 증명 풀이 각 표준에서 무슨 역할을 하는지 구분되지 않습니다. 공식 문서는 역할을 명시합니다. 이 구분을 놓치면 "왜 사용자 풀은 토큰을 발급하는데 자격 증명 풀은 자격 증명을 발급하는가"가 계속 흐려집니다.
...
OAuth 2.0 · OIDC · JWT 세 용어를 공식 문서 기준으로 정리합니다. 일부 원본 자료의 서술과 다른 두 곳을 교정했습니다.

| 용어 | 정의 |
|---|---|
| OAuth 2.0 | 표기는 **OAuth 2.0**. 웹사이트나 애플리케이션이 암호를 공유하지 않고도 다른 웹 사이트나 애플리케이션을 통해 정보에 액세스할 수 있게 함. OAuth 2.0(소셜) 공급자는 JWT 액세스·refresh 토큰을 제공하는 IdP |
| OpenID Connect(OIDC) | OAuth 위에 있는 자격 증명 계층이며 SSO 시나리오를 활성화하고 ID · 액세스 · refresh 토큰을 사용. OIDC 공급자는 **ID 토큰을 제공하기 위해 OAuth 명세를 확장하는** IdP |
| JSON 웹 토큰(JWT) | **인증된 사용자에 관한 클레임을 담은 JSON 형식 문서.** ID 토큰이 사용자를 인증하고, 액세스 토큰이 사용자에게 권한을 부여하고, refresh 토큰이 자격 증명을 갱신 |

일부 원본 자료는 JWT를 "보안 정보 **고유**를 위한 JSON 객체"로 적지만, 이는 "보안 암호 **공유**를 위한 JSON 객체"의 오타로 보입니다. 클레임을 "암호화된 사용자 정보가 있는 페이로드"로 설명하는 서술도 있는데, 이 부분은 [6.3절](#63-jwt-구조와-교재-예시-교정)에서 교정합니다.
...
Amazon Cognito는 흔히 "웹 및 모바일 앱에 대한 인증, 권한 부여 및 사용자 관리를 제공합니다"로 요약됩니다. 조금 더 정확하게 규정하면 웹·모바일 앱을 위한 **자격 증명 플랫폼(identity platform)** 이며, 사용자 디렉터리이자 인증 서버이고 OAuth 2.0 액세스 토큰과 AWS 자격 증명에 대한 권한 부여 서비스입니다.
...
대표 기능으로 흔히 제시되는 세 가지는 다음과 같습니다. 세 항목 자체는 현재 문서와 방향이 같고, 규정 준수 목록과 "고급 보안 기능"이라는 이름만 교정이 필요합니다([3.7절](#37-규정-준수) · [3.8절](#38-위협-보호)).
...
두 구성 요소는 질문으로 가를 수 있고, 이 접근은 지금도 유효합니다.
...
> 일부 원본 자료는 두 번째 질문을 "사용자에게 **AWS 액세스 키**를 부여해야 하는가?"로 씁니다. 문서가 기술하는 것은 **임시 AWS 자격 증명(temporary AWS credentials)** 입니다. "액세스 키"는 IAM 사용자의 장기 액세스 키와 혼동될 수 있어 이 문서는 표현을 바꿨습니다.
...
두 구성 요소를 특징 세 개씩으로 요약하면 문서 서술과 이렇게 대응합니다.

| 구성 요소 | 요약 | 문서 기준으로 읽으면 |
|---|---|---|
...
두 구성 요소를 함께 쓰는 흐름에서 **어떤 토큰을 어떤 API에 넘기는지**가 자주 빠집니다. 문서 기준으로 채우면 세 단계입니다.
...
같은 흐름을 그림으로 보면 이렇습니다.

```text
사용자
  │  (1) 로그인
  v
사용자 풀 ---- ID / Access / Refresh 토큰 발급
  │
  │  (2) ID 토큰 제시  (GetId -> GetCredentialsForIdentity)
  v
자격 증명 풀 ---- 임시 AWS 자격 증명 발급 (1시간 유효)
  │
  │  (3) 자격 증명으로 호출
  v
AWS 서비스  (Amazon S3 / Amazon DynamoDB 등)
```
...
사용자 풀의 로그인 화면은 흔히 "호스트된 UI"로 알려져 있지만, 현재 브랜딩 버전은 **두 개** 입니다.
...
이 모듈에서 다루는 기능 중 **여러 개가 요금제에 따라 쓸 수 있는지 갈립니다.** 사용자 풀을 만들기 전에 알아야 하는 선택입니다.
...
문서는 예전에 일부 사용자 풀 기능이 `advanced security features` 요금 구조에 포함되어 있었고, 그 기능들이 지금은 Essentials 또는 Plus 요금제로 편입되었다고 알립니다. "고급 보안 기능"이라는 이전 표현이 여기서 나옵니다.

> — 출처: [User pool feature plans](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-sign-in-feature-plans.html)

### 3.6 저장 중 전송 중 암호화 🔄

데이터는 저장 중과 전송 중에 암호화되며 모든 데이터가 서버 측에서 암호화됩니다. 여기에 흔히 빠지는 것이 **키 종류와 TLS 요구 사항**입니다.
...
규정 준수 표준을 `PCI DSS / SOC / ISO 9001 / 표준 기반 인증(OAuth 2.0, SAML 2.0, OIDC) / MFA` 로 나열하는 자료가 있는데, 두 가지를 교정해야 합니다.

| 항목 | 확인된 내용 |
|---|---|
| ISO 9001 | Amazon Cognito 문서가 명시하는 ISO 표준은 **ISO 27001**(정보 보안 관리 체계)입니다. ISO 9001은 품질 경영 시스템이고 Cognito 문서에는 나오지 않습니다 |
| 표준 기반 인증과 MFA를 규정 준수 목록에 포함 | 이 둘은 규정 준수 프로그램이 아니라 **서비스 기능**입니다. 서로 다른 범주를 한 목록에 섞은 것입니다 |
...
흔히 "고급 보안 기능으로 사용자를 보호합니다"로 소개되던 것이 이 기능입니다. 문서는 이 기능을 **위협 보호(threat protection)** 로 부르며 "이전에 고급 보안 기능(advanced security features)이라고 불렸다"고 명시합니다. **Plus 기능 요금제 전용**입니다.
...
Amazon Cognito는 "수백만 사용자 수준으로 확장 가능"하다고 흔히 소개됩니다. 실제 기준값과 조정 가능 여부를 확인했습니다. 할당량은 **계정·리전 단위**로 적용됩니다.
...
이 모듈의 원본 자료에는 등장하지 않습니다. 그런데 자격 증명 풀 콘솔의 Identity browser 탭과 데이터 세트 기능이 여전히 존재하고, 오래된 자료가 자격 증명 풀과 함께 이 서비스를 소개하므로 상태를 명시해 둡니다.
...
오른쪽 열의 여섯 항목 중 **세 항목은 이름만으로는 내용이 드러나지 않습니다.** 이 문서는 다음 절들에서 채웁니다.
...
사용자 풀로 할 수 있는 아홉 가지 일입니다. 오른쪽에 이 문서의 해당 절을 붙였습니다.
...
| 7 | 사용자 풀을 분류하고 관리할 수 있도록 태그를 추가합니다 | 원문 그대로 |
...
"사용자 풀 및 앱 클라이언트 ID 생성"을 콘솔 조작으로 전제하던 데모가 있는데, 현재 콘솔의 사용자 풀 생성은 **애플리케이션 중심 흐름**으로 바뀌었습니다.
...
암호 정책은 흔히 "정책(암호 강도, 관리자에게 가입 요청, 임시 암호 만료 기간 설정 등)을 지정합니다" 수준으로만 소개되어 **구체적인 값이 드러나지 않습니다.** 콘솔의 `Authentication methods` 메뉴 `Password policy` 에서 설정하고, 모드는 권장 최소 설정을 적용하는 `Cognito defaults` 와 `Custom` 두 가지입니다.
...
MFA는 흔히 "다중 인증(MFA)과 사용자 디바이스 기억을 활성화합니다" 수준으로만 소개되어 **두 번째 요소의 종류가 드러나지 않습니다.** MFA는 보통 사용자 이름·암호인 최초의 "아는 것(something you know)" 요소에 "가진 것(something you have)" 요소를 추가합니다.
...
"사용자 디바이스 기억"은 이름만으로는 동작이 드러나지 않습니다. 콘솔의 `Sign-in` 메뉴 `Device tracking` 에서 설정하고 옵션이 **세 개**입니다.
...
보안 요구 사항으로 "인증 흐름 지원"이라고만 소개하면 **흐름 이름이 드러나지 않습니다.** 실제로는 앱 클라이언트에서 허용할 흐름을 명시적으로 골라야 합니다.
...
| `ALLOW_USER_AUTH` | **선택 기반 로그인.** 나중에 추가된 값 |
...
보안 요구 사항으로 흔히 "보안 암호 처리 / 다중 인증 / 암호 정책 적용"만 소개되고 이 두 기능은 다루지 않습니다. 두 기능 모두 나중에 추가되었고 **선택 기반 로그인(`USER_AUTH`) 흐름을 통해서만** 쓸 수 있습니다.
...
앱 클라이언트가 "사용자 풀에 액세스할 수 있는 고유 ID와 **선택적 비밀 키**를 수신한다"는 서술은 문서의 클라이언트 ID + 선택적 클라이언트 보안 암호와 일치합니다. 빠지기 쉬운 것은 **클라이언트 유형 구분**입니다.
...
"SMS와 사용자 지정 이메일 전송 옵션을 설정합니다" 수준의 소개 뒤에는, 실습에서 사용자를 여러 명 만들면 바로 부딪히는 한도가 있습니다.
...
트리거의 예로 흔히 "**사전 가입, 후 인증**" 두 개만 언급되지만 실제 목록은 훨씬 깁니다. 사용자 풀을 구성해 최초 가입 전, 인증 완료 후, 그 사이 여러 단계에서 Lambda 함수를 자동 호출할 수 있습니다.
...
| 가입 | **Pre sign-up** | 가입 요청 수락·거부 검증 ("사전 가입"이라고도 함) |
...
> 흔히 예로 드는 "후 인증"은 위 표의 `Post authentication` 입니다.
...
"사용자 풀은 Facebook, Google, Login with Amazon, Sign in with Apple을 통한 소셜 가입에서 반환된 토큰의 처리 작업을 관리하고, OIDC와 SAML IdP에서 보내는 토큰 처리도 관리한다"는 서술은 문서와 일치합니다. 빠지기 쉬운 것은 **그 결과 무엇이 생기는가**입니다.
...
> 일부 원본 자료는 이 공급자를 "Login **for** Amazon"으로 씁니다. 공식 이름은 **Login with Amazon** 입니다([11.1절](#111-교재와-다른-점)).
...
일부 원본 자료는 이 세 가지를 "속성, 범위 및 그룹" 순서로 제목에 적지만 본문과 설명은 모두 속성 → 그룹 → 범위 순서입니다. 이 문서는 본문 순서를 따라 **속성 → 그룹 → 범위**로 정리합니다([11.1절](#111-교재와-다른-점)).
...
🔄 **사용자 지정 속성의 예로 흔히 드는 것이 실제로는 표준 속성입니다.** 속성을 `표준 속성(이메일, 이름) / 사용자 지정(닉네임, 그림)` 으로 나누는 자료가 있는데, `nickname` 과 `picture` 는 위 18개 표준 속성에 포함되어 있으므로 사용자 지정 속성이 아닙니다([11.1절](#111-교재와-다른-점)).
...
🔄 **`dev:` 접두사를 쓰는 개발자 속성(developer attributes)은 레거시 기능입니다.** 속성을 표준과 사용자 지정 둘로만 나누는 설명이 흔하지만, 문서는 `CreateUserPool` 의 `SchemaAttributes` 로 `dev:` 속성도 추가할 수 있으며 이것이 **앱 클라이언트 읽기·쓰기 권한으로 대체된 레거시 기능**이라고 명시합니다. 개발자 속성은 AWS 자격 증명으로만 수정할 수 있습니다([11.3절](#113-비권장지원-종료된-항목)).

### 5.2 필수 속성과 확인 가능한 속성

"어떤 속성이 필수이고 확인이 필요한지 정의할 수 있습니다" 수준의 소개 뒤에는 **사용자 풀을 만든 뒤에는 되돌릴 수 없는 결정**이 세 개 숨어 있습니다.
...
그룹의 기본 개념은 현재 문서와 일치합니다. 그룹으로 사용자 모음을 만들어 권한을 관리하거나 여러 유형의 사용자를 나타낼 수 있고, **그룹에 IAM 역할을 할당해 그룹 멤버의 권한을 정의**할 수 있습니다.
...
마지막 두 줄은 자주 간과됩니다. 우선순위를 같게 두면 역할이 조용히 사라질 수 있으므로 짚어 둘 가치가 있습니다.
...
"자체 OAuth 2.0 리소스 서버를 만들고 서버에서 사용자 지정 범위를 정의할 수 있다"는 맞는 서술이지만, **전제 조건 하나와 제약 하나가 빠지기 쉽습니다.**
...
🆕 사용자 풀이 발급하는 범위는 **세 종류**입니다. 흔히 사용자 지정 범위만 다뤄집니다.
...
범위를 요청할 수 있는 흐름으로 "OAuth 2.0 권한 부여 코드 부여 흐름, 암묵적 흐름, 클라이언트 보안 인증 정보 흐름"을 **아무런 우열 없이 나란히** 놓는 설명이 흔합니다. 현재 문서는 이 세 가지를 동급으로 다루지 않습니다.
...
**PKCE(Proof Key for Code Exchange)** 는 퍼블릭 클라이언트를 위한 OAuth 2.0 권한 부여 코드 그랜트의 확장이며 **가로채인 권한 부여 코드가 토큰으로 교환되는 것을 막습니다.** 브라우저·모바일 앱에서 권한 부여 코드 그랜트를 쓸 때 표준 보강 수단입니다.
...
이 모듈의 핵심이고, **교정할 것이 가장 많이 몰려 있는 자리**이기도 합니다.

### 6.1 사용자 풀 로그인 흐름

사용자 풀 로그인은 네 단계로 진행됩니다.
...
- `cognito:groups` 클레임은 **액세스 토큰과 ID 토큰 양쪽에** 들어갑니다. 이 클레임을 Access 토큰 전용으로 소개하는 자료가 있는데 이는 부정확합니다([11.1절](#111-교재와-다른-점)).
- 사용자 이름 클레임의 **이름이 두 토큰에서 다릅니다.** ID 토큰은 `cognito:username`, 액세스 토큰은 `username` 입니다.

> — 출처: [Understanding user pool JSON web tokens (JWTs)](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.html)

### 6.3 JWT 구조와 교재 예시 교정 🔄

JWT는 점(`.`)으로 구분된 **헤더 · 페이로드 · 서명** 세 섹션입니다. 흔히 쓰이는 ID 토큰 예시에는 **교정할 것이 다섯 개** 있습니다.
...
| 클레임 이름 표기 | `Sub` · `Aud` · `Jti` · `Scope` 로 **첫 글자를 대문자**로 표기 | 모두 소문자 |
| `token_use` 값 표기 | 값을 `ID` · `액세스` 로 표기 | `id` · `access` (소문자 문자열) |
...
🔄 **서명 알고리즘은 `RS256` 입니다.** 사용자 풀은 **SHA-256을 쓰는 RSA 서명**을 사용합니다. `HMACSHA256` 은 공유 비밀 키를 쓰는 **대칭 HMAC** 이고 `RS256` 은 **비대칭 서명**입니다. 일부 원본 자료는 한 예시 안에서 두 알고리즘을 섞어 놓았습니다. 검증은 공유 비밀이 아니라 사용자 풀의 **공개 JWKS** 로 합니다([11.1절](#111-교재와-다른-점)).
...
페이로드 예시는 그대로 유효합니다. 계정처럼 보이는 값만 문서 예시 형태로 두었습니다.
...
일부 원본 자료는 페이로드를 여러 곳에서 암호화된 정보라고 기술합니다. "키의 클레임에 대한 **암호화된** 정보", "클레임: **암호화된** 사용자 정보가 있는 페이로드" 같은 표현입니다.
...
위에 실은 페이로드 예시 자체가 평문 JSON이므로 **원본 자료는 자신의 예시와도 어긋납니다.** 실무에서 이 차이가 만드는 결과는 분명합니다.
...
따라서 이 페이로드가 "암호화되어 있다"는 서술은 거짓입니다.

> — 출처: [Understanding user pool JSON web tokens (JWTs)](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.html)

### 6.4 ID 토큰의 클레임

ID 토큰의 기본 페이로드 클레임입니다. 원본 자료에서 다루는 것과 다루지 않는 것을 표시했습니다.
...
🆕 **Amazon Cognito는 사용자 풀마다 RSA 키 페어를 두 쌍 만들어 액세스 토큰과 ID 토큰을 각각 다른 프라이빗 키로 서명합니다.** 따라서 같은 사용자 세션의 두 토큰에서 **`kid` 값이 일치하지 않고, 앱 코드에서 두 토큰을 독립적으로 검증해야 합니다.** 이 사실은 자주 간과됩니다.
...
🔄 **액세스 토큰의 목적을 넓게 봐야 합니다.** "사용자 풀에 있는 사용자의 API 작업에 권한을 부여한다"로 좁게 보는 설명이 흔합니다. 문서는 사용자 풀 자체 서비스 작업(속성 추가·변경·삭제 등)과 함께 **외부 API 권한 부여**를 제시하고, Amazon API Gateway가 Amazon Cognito 액세스 토큰으로 권한 부여를 지원한다고 명시합니다([8장](#8-api-액세스-보안)).
...
🔄 **기준점 하나가 흔히 잘못 설명됩니다.** refresh 토큰 기본 만료를 "애플리케이션 사용자가 사용자 풀에 **가입**한 후 30일"로 적는 자료가 있습니다. 문서 표현은 "사용자가 사용자 풀에 **로그인한 후**(signs into your user pool) 30일"이므로 기준은 가입이 아니라 로그인입니다([11.1절](#111-교재와-다른-점)).
...
refresh 토큰은 흔히 "새 ID와 Access 토큰을 검색할 수 있다" 수준으로만 소개됩니다. 문서는 **refresh 토큰 교체(rotation)를 보안 모범 사례로 권장**합니다.
...
검증 절차는 원본 자료에서 다뤄지지 않습니다. "서명은 토큰의 헤더와 페이로드를 바탕으로 계산되며 키 원본과 그 변조 여부를 확인한다" 정도가 서명에 관한 서술의 전부입니다.
...
토큰 폐기는 원본 자료에서 다뤄지지 않고, `origin_jti` 도 "인증이 발생한 지점을 표시하는 JWT 식별자"로만 설명되곤 합니다. **`origin_jti` 는 토큰 폐기 식별자**입니다.
...
ID·액세스 토큰의 클레임은 흔히 **고정된 목록**으로만 소개됩니다. 실제로는 앱에 전달되는 토큰을 사용자 지정할 수 있습니다.
...
사용자 풀이 "누구인지"를 다루는 곳이라면 자격 증명 풀은 **"AWS 리소스에 무엇을 할 수 있는지"** 를 다루는 곳입니다.
...
🔄 **"AWS 액세스 키"는 임시 AWS 자격 증명입니다.** 자격 증명 풀을 선택하는 질문을 "사용자에게 **AWS 액세스 키**를 부여해야 하는가?"로 쓰는 자료가 있습니다. 문서는 자격 증명 풀이 **임시 AWS 자격 증명(temporary AWS credentials)** 을 발급한다고 기술합니다. "액세스 키"라는 표현은 IAM 사용자의 **장기 액세스 키**와 혼동될 수 있습니다([11.1절](#111-교재와-다른-점)).
...
"자격 증명 풀은 어떤 사용자 프로필도 저장하지 않습니다"는 **맞는 서술**입니다. 문서의 비교 표에서 `User directory — Store user profiles for authentication` 은 **사용자 풀에만** 체크되고 자격 증명 풀에는 체크되지 않습니다.

자격 증명 풀이 저장하는 것은 **identity** 입니다. identity는 앱 사용자와 그 사용자 자격 증명을 **자격 증명 풀과 신뢰 관계가 있는 외부 사용자 디렉터리의 프로필에 연결하는 UUID** 입니다.

함께 알아 둘 특성도 문서와 일치합니다. 두 애플리케이션에 서로 다른 두 자격 증명 풀을 쓰면 동일한 최종 사용자가 각 자격 증명 풀에서 다른 고유 식별자를 가집니다. 문서 표현으로는 **Amazon Cognito UUID가 사용자 풀 또는 자격 증명 풀 단위로 고유**합니다.

🆕 자주 다뤄지지 않는 주의가 셋 있습니다.
...
"인증되지 않은 사용자는 기본값으로 IAM 역할·정책을 따르며 인증되지 않은 게스트로 다뤄진다"는 맞는 서술입니다. 자격 증명 풀이 미인증 사용자에게 AWS 보안 인증 정보를 제공할 수 있다는 것도 문서와 일치합니다.
...
🆕 **게스트 액세스는 별도로 활성화해야 합니다.** 이 선택은 자주 다뤄지지 않습니다.
...
이 구분 자체가 자주 다뤄지지 않습니다. 그런데 오래된 자료가 흔히 제시하는 API가 **둘 중 비권장 쪽**입니다.

자격 증명 풀은 퍼블릭 공급자 인증에 **두 흐름**을 제공합니다. 두 흐름의 호출 순서를 그림으로 보면 이렇습니다.

```text
enhanced (단순화) 흐름:
  GetId  ->  GetCredentialsForIdentity  ->  임시 AWS 자격 증명 (1시간)

basic (classic) 흐름:
  GetId  ->  GetOpenIdToken  ->  AssumeRoleWithWebIdentity  ->  임시 AWS 자격 증명
```
...
🔄 **일부 오래된 자료는 설명하는 흐름과 참조 문서가 어긋납니다.** 사용자가 `GetOpenIdToken` 과 `AssumeRoleWithWebIdentity` 요청을 발행한다고 서술하면서, 참조 링크로는 [Understanding Amazon Cognito Authentication Part 4: Enhanced Flow](https://aws.amazon.com/blogs/mobile/understanding-amazon-cognito-authentication-part-4-enhanced-flow/) 블로그를 제시하는 식입니다. 두 API는 **basic(classic) 흐름**의 API이므로 설명하는 흐름과 참조하는 문서가 서로 다른 흐름입니다. 참조 URL 자체는 살아 있으나 이 문서는 흐름에 관한 사실을 개발자 안내서에서 확인했습니다([11.1절](#111-교재와-다른-점)).
...
자격 증명 풀의 역할 선택은 흔히 "인증된 사용자면 사용자 풀·서드 파티 페더레이션으로 권한을 부여하고 인증되지 않은 사용자면 기본값으로 IAM 역할·정책을 따른다" 수준으로만 소개됩니다. 실제로는 **네 갈래**입니다.
...
"자격 증명 풀은 IAM 역할과 결합된 AWS 보안 인증 정보를 반환한다"는 소개만으로는 **그 역할이 어떤 신뢰 정책을 가져야 하는지**가 드러나지 않습니다. 여기에 **빠뜨리면 IAM이 저장을 거부하는 필수 조건**이 있습니다.
...
자격 증명 풀의 페더레이션은 흔히 소셜·SAML·OIDC IdP 기반으로만 다뤄집니다. **자체 백엔드 인증 시스템을 쓰면서 자격 증명 풀로 AWS 자격 증명을 발급하는 경로**가 따로 있습니다.
...
앞에서 만든 토큰과 자격 증명을 **실제로 API 앞에 세우는** 자리입니다.

### 8.1 REST API 사용자 풀 권한 부여자

REST API 통합의 5단계는 현재 문서와 일치합니다.
...
🔄 **헤더 이름이 번역에서 흐려지기 쉽습니다.** "토큰 소스에서 권한 부여를 헤더 이름으로 사용하여"로 번역된 자료가 있는데, 문서가 명시하는 헤더는 **`Authorization`** 입니다.

🔄 **ID 토큰과 액세스 토큰의 용도가 다릅니다.** "자격 증명이나 액세스 토큰"으로 뭉개는 설명이 흔합니다. 문서는 구분합니다.
...
**REST API와의 통합만** 소개되는 경우가 많습니다. HTTP API에서는 `COGNITO_USER_POOLS` 권한 부여자가 아니라 **JWT 권한 부여자**로 Amazon Cognito를 씁니다. 모듈 2·10에서 확인한 "REST는 Cognito를 직접 지원하고 HTTP는 JWT 권한 부여자를 통해 쓴다"는 서술의 구현 수준 근거입니다.
...
IAM 자격 증명 기반 정책용 인증 워크플로는 흔히 "API Gateway가 요청한 리소스에 대해 자격 증명 관리 정책을 평가하고 요청을 허용하거나 거부한다" 수준으로만 소개됩니다. **여기에 빠뜨리면 API가 열려 버리는 설정이 있습니다.**
...
Lambda 권한 부여자의 5단계 워크플로는 문서와 **정확히 일치합니다.**
...
🔄 **유형 이름과 권장 사항을 교정합니다.** 두 유형을 "토큰 기반(토큰 권한 부여자)"과 "요청 파라미터 기반(요청 권한 부여자)"으로만 부르고 **어느 쪽이 권장인지 밝히지 않는** 설명이 흔합니다.
...
문서는 **여러 자격 증명 소스를 쓸 수 있고 캐시 키를 분리할 수 있다는 이유로 `REQUEST` 권한 부여자를 권장**합니다. Lambda 권한 부여자의 이전 명칭이 **사용자 지정 권한 부여자(custom authorizer)** 라는 사실도 자주 다뤄지지 않습니다([11.3절](#113-비권장지원-종료된-항목)).
...
애플리케이션에 Amazon Cognito를 통합하는 방법으로는 SDK 사용이 흔히 안내되며 [Integrating Amazon Cognito authentication and authorization with web and mobile apps](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-integrate-apps.html) 가 참고 자료입니다. 이 링크는 현재도 유효하고, 문서는 Amazon Cognito 구현이 **콘솔·SDK 관리 도구와 애플리케이션의 SDK 라이브러리의 조합**이라고 설명합니다. 가장 노력이 적은 통합은 **managed login** 이며 이때 OIDC 라이브러리로 호스팅 로그인 페이지로 사용자를 보내야 한다고 기술합니다([10.3절](#103-애플리케이션에-통합하는-방법)).
...
모듈 목표 세 번째 항목은 **"Amazon Cognito 구현 모범 사례 관찰"** 입니다. 그런데 **원본 자료에 이 목표에 대응하는 내용이 없습니다.** 모범 사례를 제목으로 하는 부분도, 모범 사례를 열거하는 대목도 없습니다([11.1절](#111-교재와-다른-점)).
...
이 모듈이 과정 전체의 마지막 조각을 끼우는 자리이고, 그대로 캡스톤 실습으로 이어집니다.
...
🔄 **경로 파라미터 표기에 흔한 오류가 있습니다.** 삭제·업데이트 경로를 **`/notes/(id)`** 처럼 **소괄호**로 표기하는 자료가 있는데, API Gateway 경로 파라미터 표기는 **중괄호**입니다. 문서의 PetStore 샘플 API에서 개별 리소스 경로는 `/pets/{petId}` 이고 `get-resources` 응답의 `pathPart` 도 `{petId}` 입니다([11.1절](#111-교재와-다른-점)).
...
🔄 **같은 위치가 자료마다 다르게 표기됩니다.** 데이터 저장소를 한쪽은 `DynamoDB` 로, 다른 쪽은 같은 위치를 **"노트"** 로 표기하는 경우가 있습니다. API 경로 표는 동일합니다. 이 문서는 **서비스 이름은 Amazon DynamoDB, 그 안의 대상은 노트 테이블**로 구분해 표기합니다([11.1절](#111-교재와-다른-점)).

### 10.2 실습 6: 캡스톤

실습 6은 **Amazon Cognito를 통한 사용자 인증**을 붙여 애플리케이션 구축을 완료합니다.

🔄 **캡스톤 주제 목록이 불완전한 경우가 있습니다.** 캡스톤인데 앞선 모듈의 주제를 다섯 개만 나열하기도 합니다.

| 흔히 나열되는 것 | 빠진 것 |
|---|---|
| IDE 설정 / 애플리케이션 호스팅 / 데이터베이스 솔루션 / Lambda를 사용한 컴퓨팅 / Amazon API Gateway | **이 모듈의 주제인 Amazon Cognito**, 그리고 **모듈 11(모던 애플리케이션)** |

외부 문서로 검증할 성질의 것이 아닙니다([11.1절](#111-교재와-다른-점)). 실습에서 학습한 것을 정리할 때 이 두 항목을 함께 넣어야 캡스톤이 실제로 덮은 범위와 맞습니다.

실습 6이 전제하는 기능 요금제는 확인하지 못했습니다. 실습 가이드가 이 자료에 없습니다([11.5절](#115-검증하지-못한-항목)).

### 10.3 애플리케이션에 통합하는 방법

애플리케이션 통합은 흔히 "선택한 프로그래밍 언어에 맞는 AWS SDK를 사용하여 Amazon Cognito API를 애플리케이션에 바로 통합하십시오"로 안내됩니다. 문서는 선택을 세 갈래로 제시합니다.
...
🆕 **AWS Amplify는 원본 자료에 없습니다.** "클라이언트별 프레임워크를 사용하여 배포된 API Gateway API를 호출"이라는 표현이 무엇을 가리키는지 모호한데, 문서는 그 자리에 Amplify와 AWS SDK를 명시합니다.
...
데모 노트는 샘플 앱 참조로 [Amazon Cognito 개발자 리소스](https://aws.amazon.com/cognito/dev-resources/) 를 제시합니다. 이 링크는 현재도 유효합니다.
...
앞의 열 항목은 외부 문서로 확인한 것이고, 뒤의 열 항목은 **교재 안에서 서로 다른 서술이 어긋나거나 추출 과정에서 표기가 깨진 내부 불일치**여서 외부 문서로 검증할 대상이 아닙니다. 후자에는 근거 칸에 `—` 를 두었습니다.

| 항목 | 교재의 서술 | 확인된 내용 | 근거 |
|---|---|---|---|
| JWT 서명 알고리즘 | 헤더에 `alg: RS256` 을 적으면서 서명 줄은 `HMACSHA256(base64UrlEncode(header) + "." + base64UrlEncode(payload), {secret})` | `HMACSHA256` 은 공유 비밀 키를 쓰는 **대칭 HMAC**, `RS256` 은 SHA-256을 쓰는 **RSA 비대칭 서명**입니다. 한 예시 안에서 두 알고리즘이 섞여 있습니다. 문서는 사용자 풀이 **`RS256`** 을 사용한다고 명시하며, Amazon Cognito는 사용자 풀마다 RSA 키 페어를 **두 쌍** 만들어 액세스 토큰과 ID 토큰을 각각 다른 프라이빗 키로 서명합니다. 검증은 공유 비밀이 아니라 **공개 JWKS** 로 합니다 ([6.3절](#63-jwt-구조와-교재-예시-교정)) | [Understanding the identity (ID) token](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-the-id-token.html) |
| JWT 페이로드를 "암호화된 정보"로 기술 | 원본 자료가 여러 곳에서 "키의 클레임에 대한 암호화된 정보", "클레임: 암호화된 사용자 정보가 있는 페이로드"라고 서술 | Amazon Cognito는 토큰을 **base64url 로 인코딩한 문자열**로 발급하며 ID·액세스 토큰은 **평문 JSON 으로 디코딩할 수 있습니다.** 암호화되는 것은 **refresh 토큰**이며 사용자 풀만 읽을 수 있습니다. 원본 자료의 페이로드 예시 자체가 평문 JSON이므로 원본 자료 안에서도 어긋납니다 ([6.3절](#63-jwt-구조와-교재-예시-교정)) | [Understanding user pool JSON web tokens (JWTs)](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.html) |
| 규정 준수 표준 | `PCI DSS / SOC / ISO 9001 / 표준 기반 인증(OAuth 2.0, SAML 2.0, OIDC) / 다중 인증(MFA)` | Amazon Cognito 문서가 명시하는 ISO 표준은 **ISO 27001**(정보 보안 관리 체계)이며 ISO 9001(품질 경영)이 아닙니다. 공동 책임 모델의 "클라우드의 보안"은 SOC 1-3 · PCI DSS · ISO 27001 을 준수하고 HIPAA-BAA 대상이며, 고객이 설계하는 "클라우드에서의 보안"은 SOC 1-3 · ISO 27001 · HIPAA-BAA 로는 설계할 수 있으나 **PCI DSS 로는 그럴 수 없습니다.** 또 표준 기반 인증과 MFA는 규정 준수 프로그램이 아니라 **서비스 기능**이므로 서로 다른 범주가 한 목록에 섞였습니다 ([3.7절](#37-규정-준수)) | [What is Amazon Cognito?](https://docs.aws.amazon.com/cognito/latest/developerguide/what-is-amazon-cognito.html) |
| 소셜 IdP 이름 | `Login for Amazon` | 공식 이름은 **`Login with Amazon`** 입니다. 같은 원본 자료의 다른 곳에서는 올바르게 표기하므로 원본 자료 안에서도 표기가 갈립니다 ([4.12절](#412-서드-파티-idp-페더레이션)) | [Using social identity providers with a user pool](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-social-idp.html) |
| API 경로 파라미터 표기 | `/notes/(id)` — **소괄호** | API Gateway 경로 파라미터 표기는 **중괄호**입니다. 문서의 PetStore 샘플 API에서 개별 리소스 경로는 `/pets/{petId}` 이고 `get-resources` 응답의 `pathPart` 도 `{petId}` 입니다 ([10.1절](#101-애플리케이션-아키텍처)) | [Set up a method request in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-method-settings-method-request.html) |
| 클레임 이름 표기 | 원본 자료가 `Sub` · `Aud` · `Jti` · `Scope` 로 **첫 글자를 대문자**로 표기 | JWT 클레임 이름은 **대소문자를 구분**하고 문서 표기는 모두 소문자(`sub` · `aud` · `jti` · `scope`)입니다. 원본 자료의 다른 줄과 본문 코드 예시는 소문자를 쓰므로 원본 자료 안에서도 갈립니다 ([6.4절](#64-id-토큰의-클레임)) | [Understanding the identity (ID) token](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-the-id-token.html) |
| refresh 토큰 기본 만료의 기준점 | "애플리케이션 사용자가 사용자 풀에 **가입**한 후 30일 후에 만료" | 문서 표현은 "사용자가 사용자 풀에 **로그인한 후**(signs into your user pool) 30일"이므로 기준은 가입이 아니라 **로그인**입니다. 설정 범위 60분~10년은 교재 서술과 일치합니다 ([6.6절](#66-토큰-유효-기간)) | [Refresh tokens](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-the-refresh-token.html) |
| 사용자 지정 속성의 예 | `표준 속성(이메일, 이름) / 사용자 지정(닉네임, 그림)` | `nickname` 과 `picture` 는 OpenID Connect 명세에 근거한 **표준 속성 18개에 포함**되므로 사용자 지정 속성이 아닙니다. 실제 사용자 지정 속성은 이름에 **`custom:` 접두사**가 붙고 사용자 풀당 최대 **50개**이며 추가 후 제거·변경할 수 없습니다 ([5.1절](#51-속성)) | [Working with user attributes](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-settings-attributes.html) |
| 자격 증명 풀 흐름과 참조 문서의 불일치 | 원본 자료가 3단계로 `GetOpenIdToken` + `AssumeRoleWithWebIdentity` 를 제시하면서 참조 링크로 **Enhanced Flow** 블로그를 제시 | 두 API는 **basic(classic) 흐름**의 API이고 enhanced 흐름은 `GetId` → `GetCredentialsForIdentity` 두 단계입니다. 설명하는 흐름과 참조하는 문서가 서로 다른 흐름입니다. 참조 URL 자체는 살아 있으나 이 문서는 흐름에 관한 사실을 개발자 안내서에서 확인했습니다 ([7.4절](#74-enhanced-흐름과-basic-classic-흐름)) | [Identity pools authentication flow](https://docs.aws.amazon.com/cognito/latest/developerguide/authentication-flow.html) |
| 자격 증명 풀이 발급하는 것 | "사용자에게 **AWS 액세스 키**를 부여해야 하는가?" | 문서는 자격 증명 풀이 **임시 AWS 자격 증명(temporary AWS credentials)** 을 발급한다고 기술하며, 원본 자료의 다른 곳에서도 "임시 보안 인증 정보"로 씁니다. "액세스 키"는 IAM 사용자의 **장기 액세스 키**와 혼동될 수 있습니다 ([7.1절](#71-자격-증명-풀이-하는-일)) | [What is Amazon Cognito?](https://docs.aws.amazon.com/cognito/latest/developerguide/what-is-amazon-cognito.html) |
| ID 토큰 헤더의 닫는 인용부호 | `{ "kid" : "1234example=“, "alg" : "RS256" }` — 닫는 인용부호가 **왼쪽 큰따옴표**(`“`) | 그대로는 JSON으로 파싱되지 않습니다. 원본 자료의 헤더 코드 블록도 `kid` 줄 끝의 쉼표가 보이지 않고 마지막 `alg` 줄 뒤에는 후행 쉼표가 남아 유효한 JSON이 아닙니다. 이 문서는 **파싱 가능한 JSON으로 교정**했습니다 ([6.3절](#63-jwt-구조와-교재-예시-교정)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 두 화면의 본문이 글자 단위로 동일 | 원본 자료에서 두 화면의 본문 텍스트가 같고 한쪽에만 "둘을 함께 사용할 수 있음" 상자가 추가됨 | 애니메이션 단계를 둘로 쪼갠 것으로 보입니다. 이 문서는 **한 절로 합치고** "함께 사용" 부분만 별도로 다뤘습니다 ([3.2절](#32-두-구성-요소-사용자-풀과-자격-증명-풀) · [3.3절](#33-둘을-함께-쓰는-흐름)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 제목과 본문의 항목 순서 | 제목은 "속성, **범위** 및 **그룹**" 순서인데 본문 열 배치와 서술은 모두 속성 → **그룹** → **범위** 순서 | 이 문서는 본문 순서를 따라 **속성 → 그룹 → 범위**로 정리하고 장 제목도 그 순서로 맞췄습니다 ([5장](#5-속성-그룹-범위)) | — ([11.5절](#115-검증하지-못한-항목)) |
| JWT 정의의 오타 | "보안 정보 **고유**를 위한 JSON 객체를 정의합니다" | 원본 자료의 다른 곳은 같은 대상을 "보안 암호 **공유**를 위한 JSON 객체"라고 쓰므로 "고유"는 "공유"의 오타로 보입니다. OAuth 표기도 `OAuth2` 와 `OAuth 2.0` 으로 갈립니다. 이 문서는 JWT 정의를 공식 문서 서술로 교체하고 표기를 `OAuth 2.0` 으로 통일했습니다 ([2.4절](#24-용어-정리)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 모듈 목표 3번에 대응하는 내용이 없음 | 원본 자료가 "Amazon Cognito 구현 모범 사례 관찰"을 모듈 목표로 제시 | 원본 자료에 모범 사례를 제목으로 하는 부분이 없고 모범 사례를 열거하는 대목도 없습니다. 모듈 요약이 모듈 목표의 네 항목을 그대로 반복할 뿐입니다. 이 문서는 그 자리를 **공식 문서에서 명시적으로 모범 사례·권장으로 기술된 항목**으로 채웠습니다 ([9장](#9-구현-모범-사례)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 캡스톤 주제 목록이 불완전 | IDE 설정 / 애플리케이션 호스팅 / 데이터베이스 솔루션 / Lambda를 사용한 컴퓨팅 / Amazon API Gateway **다섯 개만** | 캡스톤인데 **이 모듈의 주제인 Amazon Cognito 와 모듈 11(모던 애플리케이션)** 이 빠져 있습니다 ([10.2절](#102-실습-6-캡스톤)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 용어 번역이 갈림 | 어떤 곳은 `authorization` 을 "승인"으로, 다른 곳은 "권한 부여"로 번역 | 같은 유형의 분기가 원본 자료 전체에 있습니다. 한 다이어그램은 "아이덴티티 풀", 다른 곳은 "자격 증명 풀"로 씁니다. 토큰을 "발행"이라고 하는 곳과 "발급"이라고 하는 곳이 갈리고, 토큰 3종 나열 순서도 "ID, Access 및 Refresh"와 "Refresh, Access 및 ID"로 뒤바뀝니다. 이 문서는 **권한 부여 · 자격 증명 풀 · 발급**으로 통일하고 토큰 순서를 **ID → Access → Refresh** 로 고정했습니다 | — ([11.5절](#115-검증하지-못한-항목)) |
| 다이어그램 레이블의 공백 누락 | `Amazon Cognito사용자 풀`, `Amazon Cognito토큰`, `Amazon APIGateway`, `애플리케이션API 호출`, `서버 측리소스`, `OpenID Connect(OIDC)사용자의` 처럼 공백이 빠짐 | 원본 다이어그램에서 줄바꿈이나 별개 텍스트 조각으로 나뉘어 있던 것이 합쳐진 결과로 보입니다. 정확한 서비스 이름은 **Amazon Cognito 사용자 풀**, **Amazon API Gateway** 입니다 | — ([11.5절](#115-검증하지-못한-항목)) |
| `token_use` 값 표기 | 원본 자료가 값을 `ID`(ID 토큰)·`액세스`(Access 토큰)로 표기 | 원본 자료의 코드 블록은 `"token_use": "id"` 로 올바르게 표기합니다. `token_use` 는 **문자열 값을 대소문자 구분해 비교**하는 클레임이므로 값 자체를 한글이나 대문자로 적으면 수강생이 그대로 코드에 옮길 위험이 있습니다. 값은 **`id`** 와 **`access`** 입니다 ([6.2절](#62-토큰-세-가지-비교)) | — ([11.5절](#115-검증하지-못한-항목)) |
| 같은 위치의 데이터 저장소를 다르게 표기 | 한 곳은 `DynamoDB`, 다른 곳은 같은 위치를 `노트` 로 표기 | 두 곳의 API 경로 표는 동일합니다. 이 문서는 **서비스 이름은 Amazon DynamoDB, 그 안의 대상은 노트 테이블**로 구분해 표기했습니다 ([10.1절](#101-애플리케이션-아키텍처)) | — ([11.5절](#115-검증하지-못한-항목)) |

### 11.2 동작·기본값이 변경된 항목

| 항목 | 교재의 서술 | 현재 | 근거 |
|---|---|---|---|
| 고급 보안 기능 | "**고급 보안 기능**으로 사용자를 보호합니다" | **위협 보호(threat protection)** 로 이름이 바뀌었고 문서가 "이전에 advanced security features 라고 불렸다"고 명시합니다. **Plus 기능 요금제**에서 제공되며 구성 요소는 침해된 자격 증명 감지 · 적응형 인증 · IP 허용·차단 목록 · 로그 내보내기입니다. 예전 advanced security features 요금 구조의 기능들은 **Essentials 또는 Plus 로 편입**되었습니다. `USER_SRP_AUTH` 에서는 적응형 인증만 켤 수 있고 **페더레이션 로그인에는 쓸 수 없습니다** ([3.8절](#38-위협-보호)) | [Advanced security with threat protection](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-threat-protection.html) |
| 로그인 화면 | "**호스트된 UI**" | 브랜딩 버전이 **managed login** 과 **hosted UI (classic)** 두 개이고, 문서는 classic을 managed login의 "더 얇고 덜 사용자 지정 가능한 선행 버전"으로 기술합니다. **managed login은 자체 서비스 프로필 관리를 지원하지 않아** 그 부분은 앱 코드로 구현합니다. 두 버전 모두 사용자 지정 CORS 오리진 정책을 지원하지 않고, managed login은 사용자 지정 도메인과 접두사 도메인 모두에 **TLS 1.2** 를 요구합니다. 브랜딩 버전을 바꾸면 사용자 세션이 유지되지 않습니다 ([3.4절](#34-로그인-화면-managed-login-과-hosted-ui-classic)) | [Managed login and the hosted UI](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-managed-login.html) |
| 기능 요금제 | Amazon Cognito를 **사용자 풀·자격 증명 풀 두 구성 요소로만** 소개하고 요금제 개념이 없음 | 사용자 풀에 **Lite · Essentials · Plus** 기능 요금제가 있고 신규 사용자 풀의 기본값은 **Essentials** 입니다. 요금제는 **사용자 풀 단위**로 적용되며 앱 클라이언트별로 다르게 둘 수 없습니다. `CreateUserPool` · `UpdateUserPool` 의 **`UserPoolTier`**(CLI는 `--user-pool-tier`)로 설정하고, `AdvancedSecurityMode` 를 `AUDIT` 또는 `ENFORCED` 로 두면 요금제가 `PLUS` 여야 합니다. 여러 기능이 요금제로 갈립니다: 패스키(Lite 제외), 이메일 MFA·이메일 OTP 와 암호 이력(Essentials 이상), 위협 보호(Plus), 액세스 토큰 사용자 지정(Lite 아닌 요금제 + 이벤트 버전 2) ([3.5절](#35-기능-요금제-lite-essentials-plus)) | [User pool feature plans](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-sign-in-feature-plans.html) |
| SMS 전송 경로 | "단문 메시지 서비스(SMS)와 사용자 지정 이메일 전송 옵션을 설정합니다" | 문서는 **2024년 11월에 AWS가 Amazon SNS SMS 메시징을 AWS End User Messaging SMS 로 대체**했다고 명시하고 현재 구성 경로가 **두 갈래**라고 기술합니다. Amazon SNS 경로는 `sns:Publish` 권한이 필요하고, AWS End User Messaging SMS 직접 경로는 `sms-voice:SendTextMessage` 권한이 필요합니다. **두 구성은 상호 배타적**이어서 한쪽을 설정하면 다른 쪽이 지워집니다. 처음 문자 메시지를 보내면 계정이 **샌드박스 환경**에 놓입니다 ([4.10절](#410-sms-와-이메일-전송)) | [SMS message settings](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-sms-settings.html) |
| 콘솔 사용자 풀 생성 흐름 | 데모가 "Cognito UI 설명", "사용자 풀 및 앱 클라이언트 ID 생성"을 콘솔 조작으로 전제 | 현재 콘솔은 **애플리케이션 중심 흐름**입니다. `Define your application`(애플리케이션 유형 선택) → 이름 입력 → `Configure options`(로그인 식별자와 가입 필수 속성) → 반환 URL 입력 → `Create your application` 이고, Amazon Cognito가 해당 유형의 기본 설정으로 **사용자 풀과 앱 클라이언트를 함께** 만듭니다. 외부 IdP·MFA 같은 옵션은 생성 후 구성합니다. 이 과정에서 **되돌릴 수 없는 기본 구성 세 가지**가 생깁니다: 클라이언트 보안 암호, `preferred_username` 별칭 미허용, 사용자 이름 대소문자 구분 없음 ([4.3절](#43-콘솔-생성-흐름)) | [Create a new application in the Amazon Cognito console](https://docs.aws.amazon.com/cognito/latest/developerguide/getting-started-user-pools-application.html) |
| 인증 흐름 | "**인증 흐름 지원**" 한 줄. 흐름 이름을 하나도 제시하지 않음 | 앱 클라이언트의 **`ExplicitAuthFlows`** 에 허용할 흐름을 명시적으로 골라야 하고 값은 `ALLOW_USER_PASSWORD_AUTH` · `ALLOW_ADMIN_USER_PASSWORD_AUTH` · `ALLOW_USER_SRP_AUTH` · **`ALLOW_USER_AUTH`** 입니다. 교재 이후 추가된 **선택 기반 로그인(`USER_AUTH`)** 은 사용자 풀의 `SignInPolicy.AllowedFirstAuthFactors` 에 `PASSWORD` · `EMAIL_OTP` · `SMS_OTP` · `WEB_AUTHN` 을 지정해 사용자가 로그인 방법을 고르게 하며, **암호 없는 로그인(OTP)과 패스키(WebAuthn)는 이 흐름을 통해서만** 쓸 수 있습니다 ([4.7절](#47-인증-흐름) · [4.8절](#48-암호-없는-로그인과-패스키)) | [Authentication flows](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-authentication-flow-methods.html) |
...
| 항목 | 요약 | 왜 더했는가 | 근거 |
|---|---|---|
| 기능 요금제 | Lite · Essentials(신규 기본값) · Plus. `UserPoolTier` 파라미터로 설정. 사용자 풀 단위 적용이며 앱 클라이언트별로 다르게 둘 수 없음 | 어떤 기능을 쓸 수 있는지가 요금제에 묶여 있어, 설계 전에 요금제를 먼저 정해야 하기 때문입니다 | [User pool feature plans](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-sign-in-feature-plans.html) |
| 선택 기반 로그인(`USER_AUTH`) | `SignInPolicy.AllowedFirstAuthFactors` 에 `PASSWORD` · `EMAIL_OTP` · `SMS_OTP` · `WEB_AUTHN` 지정. `PREFERRED_CHALLENGE` 를 넣거나 생략해 `AvailableChallenges` 목록을 받은 뒤 `SELECT_CHALLENGE` 로 진행. `PASSWORD` 값은 평문 암호와 SRP 두 변형을 모두 포함 | 암호 외의 인증 요소를 조합하는 최신 로그인 방식이라, 실습 첫 화면에서 바로 만나기 때문입니다 | [Authentication flows](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-authentication-flow-methods.html) |
| 암호 없는 로그인과 패스키 | OTP 로그인은 인증과 동시에 이메일·전화번호 속성을 확인됨으로 표시하고 사용자 상태를 `UNCONFIRMED` 에서 `CONFIRMED` 로 바꿈. **MFA 필수와 상호 배타.** 패스키는 W3C·FIDO Alliance 의 WebAuthn·CTAP2 기반, Lite 제외 요금제, `ES256`(-7)·`RS256`(-257) 인식, attestation 강제 미지원, 사용자당 최대 20개, RP ID는 퍼블릭 서픽스 목록에 없는 도메인이어야 하고 변경 시 재등록 필요 | OTP·패스키는 강의 이후 추가된 로그인 수단이고 MFA와의 상호 배타 같은 제약이 실무에서 걸리기 때문입니다 | [Authentication flows](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-authentication-flow-methods.html) |
| 토큰 폐기 | `RevokeToken` / `/oauth2/revoke` / `GlobalSignOut` / `AdminUserGlobalSignOut` 네 가지. `origin_jti` 가 폐기 식별자. 새 앱 클라이언트는 **기본 활성화.** **폐기된 토큰도 서명·만료만 검증하는 라이브러리에는 유효하게 보임** | 로그아웃이 실제로 무엇을 무효화하는지, 그리고 검증 라이브러리가 놓칠 수 있는 지점을 알아야 하기 때문입니다 | [Ending user sessions with token revocation](https://docs.aws.amazon.com/cognito/latest/developerguide/token-revocation.html) |
| refresh 토큰 교체 | 문서가 보안 모범 사례로 권장. 갱신마다 원래 토큰 무효화, 새 토큰은 원래 토큰의 남은 기간 동안 유효, 유예 기간 최대 60초. **`REFRESH_TOKEN_AUTH` 와 비호환**이므로 `GetTokensFromRefreshToken` 사용 | 보안 모범 사례로 권장되지만 기존 인증 흐름과 비호환이라 별도 API가 필요하기 때문입니다 | [Refresh tokens](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-the-refresh-token.html) |
| JWKS 기반 JWT 검증 | `jwks_uri` 는 `https://cognito-idp.<Region>.amazonaws.com/<userPoolId>/.well-known/jwks.json`. JWK 필드는 `kid` · `alg` · `kty` · `e` · `n` · `use`. `exp` · `aud`/`client_id` · `iss` · `token_use` 확인. `kid` 를 캐시 키로 삼아 공개 키 캐시·갱신. Node.js는 `aws-jwt-verify` 권장 | 토큰을 백엔드에서 실제로 검증하려면 JWKS 조회와 확인 항목을 알아야 하기 때문입니다 | [Verifying a JSON web token](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-verifying-a-jwt.html) |
| 액세스 토큰과 ID 토큰의 서명 키가 다름 | 사용자 풀마다 RSA 키 페어를 두 쌍 만들어 각각 다른 프라이빗 키로 서명. 같은 세션의 두 토큰에서 `kid` 가 일치하지 않으므로 **독립적으로 검증** | 두 토큰을 같은 키로 검증하려다 실패하는 일이 잦기 때문입니다 | [Understanding the access token](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-the-access-token.html) |
| 토큰 클레임 사용자 지정 | Pre token generation Lambda 트리거로 클레임 추가·수정·억제. 액세스 토큰 사용자 지정은 Lite 아닌 요금제 + 이벤트 버전 2 필요, **추가 비용 발생** | 클레임을 바꾸려면 Lambda 트리거와 요금제·비용 조건을 알아야 하기 때문입니다 | [Understanding user pool JSON web tokens (JWTs)](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.html) |
| PKCE | `code_verifier` → SHA256 → base64 → `code_challenge`(`code_challenge_method=S256`). 토큰 교환 시 `code_verifier` 를 평문으로 전송. 도메인과 퍼블릭 앱 클라이언트 필요 | 퍼블릭 클라이언트의 인가 코드 탈취를 막는 현재 표준 절차이기 때문입니다 | [Using PKCE in authorization code grants](https://docs.aws.amazon.com/cognito/latest/developerguide/using-pkce-in-authorization-code.html) |
| 퍼블릭 클라이언트와 기밀 클라이언트 | 퍼블릭 클라이언트는 **클라이언트 보안 암호를 갖지 않음.** 앱 클라이언트당 보안 암호 최대 2개로 무중단 교체(`AddUserPoolClientSecret` / `DeleteUserPoolClientSecret`), 유일한 보안 암호는 삭제 불가 | 클라이언트 종류에 따라 보안 암호 유무와 흐름이 달라지기 때문입니다 | [App client types](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-settings-client-apps.html) |
| 암호 정책의 구체적 값 | 최소 길이 6~99(사용자는 최대 256자), `TemporaryPasswordValidityDays` 기본 7일·범위 0~365, `PasswordHistorySize` 0~24(**Essentials 이상**). Amazon Cognito 로컬 사용자 암호는 **자동으로 만료되지 않음** | 암호 정책을 실제로 설정하려면 길이·이력·만료의 구체값이 필요하기 때문입니다 | [Adding user pool password requirements](https://docs.aws.amazon.com/cognito/latest/developerguide/managing-users-passwords.html) |
| MFA 두 번째 요소 세 가지 | SMS · **이메일** · TOTP. **페더레이션 사용자에게는 제공되지 않고** IdP에 위임. MFA 코드 5회 오류 시 지수 백오프 잠금. MFA와 계정 복구를 같은 채널로 받을 수 없음 | 이메일 MFA는 강의 이후 추가되었고 페더레이션 사용자 제외 같은 제약이 있기 때문입니다 | [Adding MFA to a user pool](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-settings-mfa.html) |
| 디바이스 기억 세 옵션 | `Don't remember` / `Always remember` / `User opt-in`. 기억된 디바이스는 **MFA가 활성화된 사용자 풀에서만** MFA를 대체. 디바이스 키 형식은 `Region_UUID`. **신뢰 기간 만료는 애플리케이션이 직접 구현** | 기억된 디바이스가 MFA를 대체하는 조건과 신뢰 만료 구현 책임을 알아야 하기 때문입니다 | [Working with user devices in your user pool](https://docs.aws.amazon.com/cognito/latest/developerguide/amazon-cognito-user-pools-device-tracking.html) |
| Lambda 트리거 전체 | 사용자 지정 인증 3종(Define/Create/Verify Auth Challenge), 인증 이벤트 3종(Pre/Post authentication, Pre token generation), 인바운드 페더레이션, 가입 3종(Pre sign-up, Post confirmation, Migrate user), Custom message, Custom sender. **Custom sender 를 제외하면 동기 호출이며 5초 안에 응답해야 하고 이 값은 변경 불가** | 어느 지점에 로직을 끼울 수 있는지와 5초 동기 응답 제약을 알아야 하기 때문입니다 | [Customizing user pool workflows with Lambda triggers](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-identity-pools-working-with-aws-lambda-triggers.html) |
| 표준 속성 18개와 사용자 지정 속성 제약 | 표준 18개, 확인 가능한 것은 `email`·`phone_number` 둘뿐. 사용자 지정 속성 최대 50개·2048자·추가 후 변경 불가·필수 불가·`custom:` 접두사·ID 토큰에는 문자열로만 기록. 필수 속성과 별칭·사용자 이름 속성은 **사용자 풀 생성 후 변경 불가** | 속성은 생성 후 바꿀 수 없는 것이 많아 설계 시점에 알아야 하기 때문입니다 | [Working with user attributes](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-settings-attributes.html) |
| 그룹 우선순위 동률 처리와 제한 | 같은 우선순위에 역할 ARN이 다르면 **`cognito:preferred_role` 이 설정되지 않음.** IdP별 자동 생성 그룹 `[사용자 풀 ID]_[IdP 이름]`. 중첩 불가, 그룹 내 사용자 검색 불가, 이름으로 그룹 검색 불가. 추가 비용 없음 | 동률일 때 역할이 정해지지 않는 함정과 그룹 검색 제약이 실무에서 걸리기 때문입니다 | [Adding groups to a user pool](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-user-groups.html) |
| 범위 세 종류와 리소스 서버 | 예약 `aws.cognito.signin.user.admin` / 사용자 지정(`식별자/범위이름`) / OIDC(`openid`·`profile`·`email`·`phone`). 리소스 서버는 **도메인 필요.** **`InitiateAuth` 로 받은 액세스 토큰에는 예약 범위만 담기므로 사용자 지정 범위 기반 API 권한 부여를 못 함.** 삭제한 범위는 비활성으로 남음 | 사용자 지정 범위로 API 권한을 부여하려는데 InitiateAuth 토큰에는 담기지 않는 함정이 있기 때문입니다 | [Scopes, M2M, and resource servers](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-define-resource-servers.html) |
| enhanced 흐름과 basic 흐름의 구분 | enhanced는 `GetId` → `GetCredentialsForIdentity`(자격 증명 1시간), basic은 `GetId` → `GetOpenIdToken` → `AssumeRoleWithWebIdentity`. 공급자별 아티팩트(사용자 풀·OIDC는 ID 토큰, SAML은 어설션, 소셜은 액세스 토큰). enhanced는 같은 계정 역할만, basic은 다른 계정 역할도 가능 | 자격 증명 풀의 두 흐름은 호출 API와 교차 계정 역할 가능 여부가 달라 구분이 필요하기 때문입니다 | [Identity pools authentication flow](https://docs.aws.amazon.com/cognito/latest/developerguide/authentication-flow.html) |
| 자격 증명 풀 역할 선택 네 갈래 | 기본 역할 / 토큰(`cognito:preferred_role`·`cognito:roles`) / 규칙 기반 매핑(`Equals`·`NotEqual`·`StartsWith`·`Contains`, 첫 일치 규칙 적용) / ABAC. `GetCredentialsForIdentity` 의 **`CustomRoleArn` 이 최우선**이고 `cognito:roles` 와 일치하지 않으면 액세스 거부. `iam:PassRole` 이 필요한 경우 있음 | 어떤 역할이 적용되는지 우선순위를 모르면 액세스 거부의 원인을 찾을 수 없기 때문입니다 | [Using role-based access control](https://docs.aws.amazon.com/cognito/latest/developerguide/role-based-access-control.html) |
| 속성 기반 액세스 제어(ABAC) | 토큰·어설션의 속성을 프린시펄 태그로 매핑하고 IAM 정책에서 `${aws:PrincipalTag/tagkey}` 조건으로 평가. 콘솔 옵션은 `Inactive` / `Use default mappings`(`sub`·`aud`) / `Use custom mappings` | 토큰 속성으로 세분화된 권한을 주는 방식이라 매핑 옵션을 알아야 하기 때문입니다 | [Using attributes for access control](https://docs.aws.amazon.com/cognito/latest/developerguide/attributes-for-access-control.html) |
| 신뢰 정책 조건 키 | `cognito-identity.amazonaws.com:aud` **필수**(없으면 IAM이 저장 거부), `amr`(`authenticated`/`unauthenticated`/공급자), `sub`(identity ID, IdP의 `sub` 아님). **enhanced 흐름 역할에 `aws:SourceIp` 를 넣으면 절대 충족되지 않음** | `aud` 누락 시 저장 거부나 enhanced 흐름의 SourceIp 함정처럼 흔한 실수를 막기 위해서입니다 | [Role trust and permissions](https://docs.aws.amazon.com/cognito/latest/developerguide/iam-roles.html) |
| 개발자 인증 자격 증명 | `GetOpenIdTokenForDeveloperIdentity` 로 시작하고 관리 자격 증명으로 요청을 인증. `Logins` 맵 키가 토큰의 `iss` 와 정확히 일치해야 하며 불일치 시 `NotAuthorizedException`. **개발자 공급자 이름은 추가 후 변경·삭제 불가** | 자체 인증 시스템을 자격 증명 풀에 연결하는 경로이고 공급자 이름 변경 불가 제약이 있기 때문입니다 | [Developer-authenticated identities](https://docs.aws.amazon.com/cognito/latest/developerguide/developer-authenticated-identities.html) |
| 게스트 액세스 활성화 | 게스트 액세스를 지원하지 않으면 콘솔 상태가 `Inactive`. 활성화에는 게스트용 **기본 IAM 역할 지정**이 필요하고 기존 역할을 쓰려면 신뢰 정책에 `cognito-identity.amazonaws.com` 포함. `GetId` 로 받은 identity ID를 앱이 캐시하는 것이 기대됨 | 비인증 사용자에게 자격 증명을 주는 설정과 필요한 역할·신뢰 정책을 알아야 하기 때문입니다 | [Identity pools](https://docs.aws.amazon.com/cognito/latest/developerguide/identity-pools.html) |
| HTTP API JWT 권한 부여자 | **RSA 기반 알고리즘만 지원**, 공개 키 2시간 캐시(키 교체 시 유예 기간 권장), `aud` 우선(없을 때만 `client_id`), `nbf`·`iat` 는 현재 UTC 이전, 라우트별 `authorizationScopes`. Lambda 통합에서 `$event.requestContext.authorizer.jwt.claims.<클레임>` 으로 접근 | Cognito 토큰으로 API를 보호하는 실제 경로이고 RSA 전용·캐시 같은 제약이 있기 때문입니다 | [Control access to HTTP APIs with JWT authorizers](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-jwt-authorizer.html) |
| API Gateway `AWS_IAM` 권한 부여 | SigV4·SigV4a 서명. `Action` 은 `execute-api:Invoke`(그 밖에 `*`·`InvalidateCache`), `Resource` 는 `api-id/스테이지/HTTP동사/경로` 형식 ARN. **`authorizationType` 을 `AWS_IAM` 으로 설정하지 않으면 그 메서드가 퍼블릭으로 열림** | SigV4로 API를 보호하는 경로이고 설정을 빠뜨리면 메서드가 퍼블릭으로 열리기 때문입니다 | [Control access for invoking an API](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-control-access-using-iam-policies-to-invoke-api.html) |
| 서비스 할당량과 과금 단위 | 사용자 풀당 사용자 **40,000,000명**(조정 가능), 리전당 사용자 풀 1,000(최대 10,000), 사용자 풀당 앱 클라이언트 1,000(최대 10,000)·자격 증명 공급자 300(최대 1,000)·리소스 서버 25(최대 300)·사용자 지정 속성 **50(조정 불가)**·그룹 **10,000(조정 불가)**. 기본 이메일 **AWS 계정당 하루 50통(조정 불가)**. 과금은 **MAU** 기준이며 `AdminGetUser` 도 MAU에 기여, CSV 가져오기와 `AdminResetUserPassword` 는 기여하지 않음 | MAU 과금과 조정 불가 할당량은 설계·비용 판단에 직접 영향을 주기 때문입니다 | [Quotas in Amazon Cognito](https://docs.aws.amazon.com/cognito/latest/developerguide/quotas.html) |
| 저장 중·전송 중 암호화의 세부 | 자격 증명 풀은 **AWS 소유 키 고정**(변경 불가), 사용자 풀은 기본 AWS 소유 키에 **고객 관리 키 선택 가능**(같은 리전 대칭 KMS 키만, ARN으로만 구성). 검색 가능 암호화로 `sub`·`email`·`phone_number` 등의 평문과 암호문을 매핑. **TLS 1.2 요구·1.3 권장**, PFS 암호 그룹 요구 | 자격 증명 풀은 키를 못 바꾸고 사용자 풀만 고객 관리 키가 되는 차이를 알아야 하기 때문입니다 | [Data protection in Amazon Cognito](https://docs.aws.amazon.com/cognito/latest/developerguide/data-protection.html) |
| AWS Amplify 통합 경로 | 백엔드에 Amazon Cognito 인증을 두고 풀스택 애플리케이션을 만드는 AWS 서비스. **`Authenticator`** 같은 연결된 UI 구성 요소 제공. 자격 증명 풀에는 사용자 풀과 같은 관리형 인증 옵션이 없어 SDK로 직접 구현 필요 | Cognito 인증을 프런트엔드에 붙이는 관리형 경로라 구현 부담을 줄여 주기 때문입니다 | [Integrating Amazon Cognito with web and mobile apps](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-integrate-apps.html) |
| 페더레이션 사용자 프로필 생성 | 사용자 풀 디렉터리에 페더레이션 사용자의 **프로필이 실제로 만들어지고** IdP 클레임과 퍼블릭 userinfo 엔드포인트를 근거로 속성이 채워짐. 매핑된 IdP 속성이 바뀌면 사용자 속성도 바뀜. `username` 은 `MyIDP_bob@example.com` 형태. **`InitiateAuth` 로는 페더레이션 사용자를 로그인시킬 수 없고** Login·Authorize 엔드포인트만 가능 | 페더레이션 사용자도 실제 프로필이 생기고 InitiateAuth로는 로그인할 수 없다는 점을 알아야 하기 때문입니다 | [User pool sign-in with third party identity providers](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-identity-federation.html) |
...
| 원본 자료의 `iss` 예시 값이 사용자 풀 ID 형식 규칙에 맞는지 | 교재 예시는 `us-east-1_example` 이고 문서 예시는 `us-west-2_example` · `us-east-1_EXAMPLE` 형태입니다. **사용자 풀 ID의 형식 자체를 규정한 페이지를 찾지 못했습니다.** `sub` 에 대해서는 "형식을 엄격히 검증하지 말라"는 지침이 있으나 사용자 풀 ID 규칙은 미확인입니다 |
| 교재 자체의 표기 오류 열 건 | 왼쪽 큰따옴표와 `token_use` 값 한글 표기, 두 화면 본문 중복, 제목 순서, JWT 정의 오타, 모듈 목표 3번 대응 내용 부재, 캡스톤 목록 누락, 용어 번역 분기, 다이어그램 레이블 공백 누락, 데이터 저장소 레이블은 모두 **교재 안에서 서로 다른 서술이 어긋나거나 추출 과정에서 표기가 깨진 것**입니다. AWS 문서로 확인할 성질의 사실이 아니므로 근거 인용을 붙이지 않고 표기만 교정했습니다 |
