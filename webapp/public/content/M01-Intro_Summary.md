...
> - 교재 안에서 표기가 서로 어긋나는 항목은 AWS 문서로 판별할 대상이 아니므로 🆕·🔄 없이 본문에서 짚고 [8.1절](#81-교재-기술이-사실과-다른-항목)에 모았습니다.
...
이 모듈은 기술 모듈이 아니라 **과정 운영 모듈**입니다. 무엇을 배우는지(과정 목표), 무엇을 알고 와야 하는지(수강 전 권장 사항), 3일 동안 어떤 순서로 진행하는지(어젠다), 실습 환경과 가이드에 어떻게 접근하는지를 다룹니다. 따라서 이 문서도 짧습니다. API·할당량 같은 검증 대상 사실이 원래 적은 모듈입니다.

### 과정 목표

교재 슬라이드 4의 세 항목을 그대로 옮깁니다.
...
이 모듈에는 **모듈 목표 슬라이드가 따로 없습니다.** 다른 모듈 덱과 달리 슬라이드 4의 *과정* 목표가 그 역할을 합니다.

### 모듈 제목의 출처

슬라이드 1에는 `Developing on AWS (한국어)` 만 있고 모듈 제목이 없습니다. 모듈 제목은 **슬라이드 2의 부제** `모듈 1: 과정 개요` 에서 옵니다. 슬라이드 7의 어젠다 강사 노트는 같은 모듈을 `모듈 1 - 과정 소개` 라고 적어 교재 안에서 이름이 두 가지로 나타납니다. 이 문서는 슬라이드 2를 따릅니다([8.1절](#81-교재-기술이-사실과-다른-항목)).
...
| 항목 | 내용 |
|---|---|
| 지식 확인 문제 | 다른 모듈 덱과 달리 참/거짓 문제 슬라이드가 없습니다. [9장](#9-지식-확인-및-핵심-정리)은 교재 문제 대신 최신화 내용 확인용 보충 문제만 싣습니다 |
| 코드 | 코드 블록으로 옮길 원문이 한 줄도 없습니다. 이 문서에도 코드가 없습니다 |
| 모듈 목표 슬라이드 | 위에 적은 대로 슬라이드 4의 과정 목표가 그 역할을 합니다 |

---

## 2. 수강 전 권장 사항

슬라이드 5의 내용입니다. 강사 노트는 없습니다.
...
슬라이드 15의 자기 소개 질문은 같은 항목을 `Java, Python, C#` 로 묻습니다. `.NET` 은 플랫폼 이름이고 `C#` 은 그 플랫폼의 언어라서 표기 축이 다릅니다. 실습 IDE·SDK 선택과 직결되므로 강의 시작 시점에 어느 축으로 물을지 정리해 두는 편이 낫습니다([8.1절](#81-교재-기술이-사실과-다른-항목)).
...
슬라이드 6과 슬라이드 12는 같은 아키텍처 그림을 씁니다. 그림의 텍스트 상자에서 뽑은 요소는 다음과 같습니다.
...
서비스 레이블 중 `Amazon APIGateway` 는 공백이 빠진 표기입니다. 정확한 이름은 `Amazon API Gateway` 이고, 같은 교재의 슬라이드 10·11 다이어그램은 올바르게 적고 있습니다([8.1절](#81-교재-기술이-사실과-다른-항목)).

### 3.3 아키텍처에 등장하는 서비스의 현재 표기 🆕

교재 다이어그램(슬라이드 6·7·12)에 나오는 서비스를 현재 공식 문서와 하나씩 대조했습니다. **이름이 바뀌거나 지원이 종료된 서비스는 없습니다.** 다만 세 곳에서 문서상 위치가 달라졌습니다(→ [3.4절](#34-교재-이후-달라진-세-지점)).
...
슬라이드 7~12입니다. 각 슬라이드는 왼쪽에 모듈·실습 상자를, 오른쪽에 그 반나절에 다루는 아키텍처 조각을 그립니다. 아래 표의 **학습 목표** 열은 같은 슬라이드의 강사 노트에서 옮긴 것입니다.
...
**실습 환경 접속과 IDE.** 강사 노트는 실습에 **세 가지 IDE**가 있고 수강생이 선호도 또는 프로그래밍 언어에 따라 고를 수 있다고 적습니다(IDE 이름은 적혀 있지 않습니다). 접속 방법은 슬라이드 상자와 강사 노트가 어긋납니다. 상자는 `Guacamole 또는 원격 데스크톱` 두 가지, 노트는 `Guacamole, 원격 데스크톱 또는 브라우저 기반 옵션` 세 가지입니다. Apache Guacamole은 AWS가 운영하는 문서 대상이 아니고 실습 환경 접속 방법은 강의 운영 사항이라 이 문서에서 확정할 수 없습니다([8.5절](#85-검증하지-못한-항목)).

또 이 슬라이드의 강사 노트는 한국어인데 **실습 1 설명 한 줄만 영어**입니다(`실습 1 – Configure and test IAM permissions in a development environment.`). 위 표에는 한국어로 옮겨 적었습니다([8.1절](#81-교재-기술이-사실과-다른-항목)).
...
실습 2만 상자 제목과 강사 노트가 서로 다른 실습을 가리킵니다. 같은 슬라이드의 다이어그램이 개발자 → AWS SDK → Amazon S3 흐름을 그리고 있어 **상자 제목 쪽**이 이 반나절의 실습으로 보입니다. 다른 실습(3·4·6)은 상자 제목과 노트가 대응합니다([8.1절](#81-교재-기술이-사실과-다른-항목)).
...
**모듈 제목이 개별 덱과 다릅니다.** 어젠다 상자는 `현대적 애플리케이션 구축` 인데 M11 덱 표지는 `모던 애플리케이션(Modern Application) 구축` 이고, 상자는 `애플리케이션 사용자에게 액세스 권한 부여` 인데 M12 덱 표지는 `내 애플리케이션의 사용자에게 액세스 권한 부여하기` 입니다. 수강생이 어젠다와 모듈 표지를 대조하면 바로 눈에 띕니다([8.1절](#81-교재-기술이-사실과-다른-항목)).
...
다이어그램 조각: 슬라이드 6의 Pollynotes 전체 아키텍처와 같은 레이블 집합입니다.

**실습 7의 화면 경로가 교재와 다를 수 있습니다.** 문서는 X-Ray 콘솔을 더 이상 개발하지 않는다고 명시하고, X-Ray Service map과 CloudWatch ServiceLens map이 **Amazon CloudWatch 콘솔의 X-Ray trace map**으로 통합되었다고 기술합니다. CloudWatch 콘솔 왼쪽 탐색 창의 `X-Ray traces` → `Trace Map` 에서 엽니다. 자세한 내용은 [3.4절](#34-교재-이후-달라진-세-지점)에 있습니다.
...
슬라이드 13~15입니다. 강의 운영에 관한 내용이라 AWS 문서로 검증할 사실이 없습니다. 교재 그대로 옮깁니다.

### 5.1 대면 강의 안내 사항 (슬라이드 13)
...
강사 노트: 자기 소개는 수강생의 숙련도 수준과 수강생 각자가 수업에서 얻고자 하는 것을 이해하는 데 도움이 됩니다.
...
5번 질문의 언어 목록은 슬라이드 5(`Python / .NET / Java`)와 표기 축이 다릅니다([2장](#2-수강-전-권장-사항) 참조).

---

## 6. 실습 및 가이드 액세스

슬라이드 16~19입니다. 슬라이드 16은 섹션 표지(`과정 실습 및 가이드에 액세스하는 방법` / 부제 `AWS Builder Labs 지침`)입니다.

### 6.1 등록 (슬라이드 17)

슬라이드 본문은 두 줄입니다. **AWS Builder Labs에 등록해야 하며, 등록 정보는 환영 이메일에서 참조합니다.**

수강생용 강사 노트를 정리하면 절차는 이렇습니다.
...
교재가 적어 둔 이 Skill Builder 주소는 **강사 전용 내부 경로**입니다. 이 문서에서는 내용을 확인하지 못했습니다([8.5절](#85-검증하지-못한-항목)).

### 6.2 가이드 위치 (슬라이드 18)

슬라이드 본문은 없고 그래픽만 있습니다. 강사 노트를 정리한 내용입니다.
...
| 주소 | 슬라이드는 참가자 가이드 주소로 `https://evantage.gilmoreglobal.com/` 을 적고 "강사에게 코드 받기"를 덧붙인다 |
| 가이드 구성 | 실습이 없는 강의는 수강생 가이드만, 실습이 있는 강의는 수강생 가이드와 실습 가이드가 있다 |

강사용 노트의 **중요 참고 사항**: 이 슬라이드의 내용은 실습이 없는 강의이거나, 수강생이 AWS Builder Labs 포털의 고유 URL을 쓰지 않고 기존 eVantage Bookshelf(VitalSource) 계정에 로그인하는 경우에만 적용됩니다. **실습이 포함된 과정은 환영 이메일의 등록 URL을 쓰도록 권합니다.** 강사는 Gilmore를 통해 라이선스 코드를 구매하고 수강생에게 다운로드 코드를 보냅니다.

eVantage Bookshelf(VitalSource)와 Gilmore Global은 AWS가 운영하는 도메인이 아니어서 이 문서의 인용 가능 출처 목록에 없습니다. 그래서 위 내용은 **교재가 기재한 대로만** 옮기고 출처를 붙이지 않았습니다([8.5절](#85-검증하지-못한-항목)).

### 6.4 AWS Builder Labs의 현재 위치 🔄

강의 실습에 접근하는 경로(환영 이메일의 고유 등록 URL → AWS Builder Labs 대시보드)는 교재와 같습니다. 달라진 것은 **AWS가 AWS Builder Labs를 소개하는 자리**입니다.
...
이 목록은 **강의 운영 로지스틱스**입니다. 특정 AWS 서비스의 지원 운영 체제·브라우저 요구 사항이 아니라 실습 환경 접속 조건이라서 AWS 공식 문서로 검증할 성질의 사실이 아닙니다. 그대로 옮기고 출처를 붙이지 않았습니다([8.5절](#85-검증하지-못한-항목)).
...
| 실습 1 설명 (슬라이드 7 강사 노트) | 한국어 노트 중 이 한 줄만 영어 (`Configure and test IAM permissions in a development environment.`) | 번역 누락으로 보입니다. [4.1절](#41-1일-차-오전-슬라이드-7) 표에는 한국어로 옮겨 적었습니다 | — ([8.5절](#85-검증하지-못한-항목)) |
...
**이 덱에는 지식 확인 문제 슬라이드가 없습니다.** 다른 모듈 덱과 달리 참/거짓 문제가 실려 있지 않아 옮길 교재 문제가 없습니다. 대신 위에서 최신화한 내용을 확인하는 보충 문제만 싣습니다.

### 🆕 보충 문제 (최신화 내용 확인)

**문제 1**: Amazon Polly에서 고를 수 있는 음성 엔진은 Neural과 Standard 두 가지입니다.

- ❌ **정답: 거짓** — **Generative, Long-form, Neural, Standard** 네 가지입니다. Generative와 Long-form은 교재 이후 추가되었습니다. ([3.4절](#34-교재-이후-달라진-세-지점))

> — 출처: [Amazon Polly voice engines](https://docs.aws.amazon.com/polly/latest/dg/voice-engines-polly.html)

**문제 2**: 실습 7에서 애플리케이션 추적을 보려면 X-Ray 콘솔을 써야 합니다. AWS는 X-Ray 콘솔을 계속 개선하고 있습니다.

- ❌ **정답: 거짓** — 문서는 **"AWS는 X-Ray 콘솔을 더 이상 개발하지 않는다"** 고 명시합니다. X-Ray Service map과 CloudWatch ServiceLens map은 **Amazon CloudWatch 콘솔의 X-Ray trace map**으로 통합되었습니다. X-Ray 서비스가 종료된 것은 아니고 화면 경로가 달라진 것입니다. ([8.3절](#83-비권장지원-종료된-항목))

> — 출처: [Use a console (X-Ray)](https://docs.aws.amazon.com/xray/latest/devguide/aws-xray-interface-console.html)

**문제 3**: 선수 과정인 AWS Cloud Practitioner Essentials는 이름이 바뀌어 더 이상 존재하지 않습니다.

- ❌ **정답: 거짓** — 같은 이름으로 존재하며 클라우드 개념 중심의 **13개 모듈**로 개편되었습니다. AWS Skill Builder에서 수강하고 Coursera·edX에서도 제공됩니다. `aws.amazon.com` 의 강의실 과정 주소로 들어가면 Skill Builder로 이동합니다. ([2.1절](#21-두-선수-과정의-현재-이름과-위치))
...
| # | 요점 |
|---|---|
| 1 | 과정 목표는 세 가지입니다. **IAM 권한 구성 / AWS SDK로 클라우드 네이티브 애플리케이션 설계·구축·배포 / AWS 리소스로 모니터링·유지 관리** |
| 2 | 3일 동안 하나의 애플리케이션 **Pollynotes**를 처음부터 끝까지 만듭니다. 모듈마다 이 애플리케이션의 한 조각을 붙이고, 슬라이드 6과 슬라이드 12의 같은 그림이 시작과 끝을 이어 줍니다 |
| 3 | 아키텍처에 등장하는 서비스 13개의 이름은 **하나도 바뀌지 않았습니다.** 달라진 것은 위치입니다. X-Ray 콘솔은 더 이상 개발되지 않고 CloudWatch로, Polly의 음성 엔진은 네 가지로, Lambda는 두 컴퓨팅 프리미티브로 문서가 재편되었습니다 |
| 4 | 실습 환경과 가이드는 **환영 이메일의 고유한 수강생 등록 URL**에서 시작합니다. 이 경로는 교재와 같습니다. 이 URL로 들어가면 Gilmore에서 라이선스 코드를 따로 구매할 필요가 없습니다 |
| 5 | 교재 안에서 표기가 어긋나는 지점이 여섯 곳 있습니다(모듈 이름, 실습 1 설명, 실습 2 제목, 어젠다 모듈 제목, 접속 방법 개수, 언어 목록). 모두 [8.1절](#81-교재-기술이-사실과-다른-항목)에 정리했습니다. 수강생이 교재와 대조하다 물어볼 지점입니다 |
