...
| 항목 | 내용 |
|---|---|
| 지식 확인 문제 | 이 덱에는 참/거짓 문제 슬라이드가 없습니다. [10장](#10-지식-확인-및-핵심-정리)은 교재 문제 대신 최신화 내용 확인용 보충 문제만 싣습니다 |
...
**이 덱에는 지식 확인 문제 슬라이드가 없습니다.** 슬라이드 13이 마무리 다이어그램이고 슬라이드 14가 `감사합니다.` 표지라서 옮길 교재 문제가 없습니다. 대신 위에서 최신화한 내용을 확인하는 보충 문제만 싣습니다.

### 🆕 보충 문제 (최신화 내용 확인)

**문제 1**: Amazon API Gateway로 만들 수 있는 API는 REST API 하나입니다.

- ❌ **정답: 거짓** — **REST, HTTP, WebSocket** 세 가지입니다. REST와 HTTP는 무상태 RESTful API이고 WebSocket은 상태 저장 양방향 통신용입니다. AWS는 기능·가격 기준으로 고르라고 안내하며, API 키·클라이언트별 스로틀링·요청 검증·AWS WAF 통합·프라이빗 엔드포인트가 필요하면 REST, 필요 없으면 더 저렴한 HTTP를 선택합니다. ([7.2절](#72-api-gateway가-만드는-api는-세-가지입니다))

> — 출처: [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html)

**문제 2**: EC2는 지속적인 유지 관리가 필요하고 Lambda는 비용이 더 저렴하므로, AWS는 애플리케이션 컴퓨팅에 항상 Lambda를 권장합니다.

- ❌ **정답: 거짓** — 결정 안내서는 컴퓨팅 선택을 **열 가지 요소의 균형** 문제로 제시하고, EC2 관리 오버헤드에 대해 "컴퓨팅 환경에 대한 **세밀한 제어가 필요한 사용 사례에서는 그 오버헤드가 정당화되는 경우가 많다**"고 서술합니다. 비용 최적화도 Lambda로 옮기는 문제가 아니라 인스턴스 선택(Graviton 최대 40% 가격 대비 성능)·구매 플랜(Savings Plans 최대 72%, Spot 최대 90%)·적정 크기 조정(Compute Optimizer 최대 25%)으로 접근하고, **단일 워크로드에서 여러 컴퓨팅 솔루션을 함께 쓸 수 있다**고 기술합니다. 이 과정에서 Lambda를 쓰는 근거는 비용이 아니라 요청·이벤트 구동 CRUD라는 워크로드 특성입니다. ([6.5절](#65-ec2와-lambda를-대비시킨-서술))

> — 출처: [Choosing an AWS compute service](https://docs.aws.amazon.com/decision-guides/latest/decision-guides/choosing-aws-compute-service.html)

**문제 3**: DynamoDB 테이블을 기본 설정으로 만들면 프로비저닝된 용량 모드가 됩니다.

- ❌ **정답: 거짓** — **온디맨드 모드가 기본이자 권장 처리량 옵션**입니다. 온디맨드는 용량 계획을 없애고 요청당 과금하며, 트래픽이 0이면 처리량 요금이 부과되지 않고 프로비저닝 모드와 같은 한 자릿수 밀리초 지연 시간·SLA·보안을 제공합니다. 실습 화면이 예전 캡처와 다를 수 있는 지점입니다. ([6.3절](#63-용량-크기-조정의-현재-기본값))

> — 출처: [DynamoDB on-demand capacity mode](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/on-demand-capacity-mode.html)

**문제 4**: Amazon Cognito 사용자 풀을 만들 때는 사용자 풀 이름과 로그인 옵션만 정하면 되고, 요금제를 선택하는 단계는 없습니다.

- ❌ **정답: 거짓** — 현재 사용자 풀에는 기능 요금제 **Lite · Essentials · Plus**가 있고 신규 사용자 풀의 **기본값은 Essentials**입니다. 요금제는 사용자 풀 단위로 적용되며 사용자 풀 안의 앱 클라이언트별로 다르게 둘 수 없습니다. API에서는 `UserPoolTier`, AWS CLI에서는 `--user-pool-tier`로 지정하고, 생략하면 Essentials가 됩니다. ([7.5절](#75-가입로그인-화면과-요금제가-달라졌습니다))

> — 출처: [User pool feature plans](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-sign-in-feature-plans.html)

**문제 5**: Pollynotes의 프런트엔드를 S3에 호스팅하려면 버킷의 정적 웹 사이트 호스팅을 켜는 것이 현재 문서의 1순위 권장 방식입니다.

- ❌ **정답: 거짓** — 현재 문서의 1순위 권장은 **AWS Amplify Hosting**입니다. CloudFront 기반 전역 CDN에 배포하는 완전 관리형 서비스로 공개 HTTPS URL을 생성합니다. 버킷이 **SSE-KMS로 암호화된 경우에는 CloudFront 배포가 필수**이고, 오리진 보호에는 OAI가 아니라 **OAC**를 써야 합니다. ([5.3절](#53-프런트엔드-호스팅의-권장-경로))

> — 출처: [Hosting a static website using Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/WebsiteHosting.html)

**문제 6**: 실습 7에서 애플리케이션 추적을 보려면 X-Ray 콘솔을 써야 하고, AWS는 X-Ray 콘솔을 계속 개선하고 있습니다.

- ❌ **정답: 거짓** — 문서는 **"AWS는 X-Ray 콘솔을 더 이상 개발하지 않는다"** 고 명시합니다. X-Ray Service map과 CloudWatch ServiceLens map은 **Amazon CloudWatch 콘솔의 X-Ray trace map**으로 통합되었습니다. X-Ray 서비스가 종료된 것은 아니고 화면 경로가 달라진 것입니다. ([8.3절](#83-x-ray-콘솔의-현재-상태))
...
| # | 요점 |
|---|---|
| 1 | 이 모듈은 **아키텍처 워크스루**입니다. 슬라이드 4~13이 같은 그림을 두고 조각을 하나씩 강조합니다. 그림 한 장을 외우면 과정 전체의 지도가 됩니다([2장](#2-반복되는-아키텍처-다이어그램)) |
| 2 | 역할 분담은 이렇습니다. **S3**가 프런트엔드와 MP3를, **DynamoDB**가 노트 데이터를, **Lambda**가 CRUD 비즈니스 로직을, **API Gateway**가 그 사이를, **Cognito**가 사용자 인증을, **CloudWatch·X-Ray**가 관측을 맡습니다. **Polly**가 노트를 음성으로, **SAM**이 배포를, **IAM**이 액세스 제어를 담당합니다 |
| 3 | 서비스 **이름이 바뀐 것은 없습니다.** 달라진 것은 프레이밍과 위치입니다. API Gateway의 API는 세 종류가 되었고, DynamoDB의 기본 용량 모드는 온디맨드이며, 프런트엔드 호스팅의 1순위 권장은 Amplify Hosting이고, X-Ray 콘솔은 CloudWatch로 옮겨졌으며, Cognito 사용자 풀에는 기능 요금제가 생겼습니다([9장](#9-교재-대비-변경-사항)) |
| 4 | 교재가 **단정한 두 문장은 그대로 받아들이지 마세요.** "콘솔이 필요한 모든 기능을 지원한다"는 서술은 문서에서 범위가 IaaS로 한정되고 방향이 반대이며, "EC2는 유지 관리가 필요하니 비용 절감을 위해 Lambda"라는 논리는 결정 안내서가 지지하지 않습니다. Lambda 선택의 근거는 워크로드 특성입니다([4.2절](#42-콘솔이-모든-기능을-지원한다는-서술) · [6.5절](#65-ec2와-lambda를-대비시킨-서술)) |
| 5 | 교재 안에서 표기가 어긋나는 지점이 다섯 곳 있습니다(슬라이드 2 제목 누락, 모듈 2 제목 `구축`/`빌드`, 모듈 4 제목 `권한`/`권한 부여`, 접속 방법 개수, 다이어그램 레이블 언어 혼용). 모두 [9.1절](#91-교재-기술이-사실과-다른-항목)에 정리했습니다. 수강생이 교재와 대조하다 물어볼 지점입니다 |
| 6 | 이 모듈은 **깊이를 의도적으로 얕게 둡니다.** 각 서비스의 할당량·API·요금은 해당 모듈의 소관입니다. 여기서 얻어야 할 것은 "무엇이 무엇을 맡는가"이고, 그 대응표가 [2.2절](#22-요소와-서비스의-대응)입니다 |
