...
| 데모 / 학습 내용 확인 / 실습 5 / 요약 | 37–45 | [7.7절](#77-데모-주제) · [10장](#10-지식-확인-및-핵심-정리) |
...
교재 슬라이드 40의 문제를 그대로 싣습니다.

**문제 1**: Amazon API Gateway를 사용하면 개발자는 API를 생성, 게시, 유지 관리, 모니터링 및 보호할 수 있습니다.

- ✅ **정답: 참**

**문제 2**: canary 릴리스 배포에서는 새 소프트웨어 기능을 부분적으로 배포하고 API 트래픽의 일정 비율을 새 버전의 애플리케이션으로 이동합니다.

- ✅ **정답: 참**

**문제 3**: Mock 통합은 API 메서드용 200 상태 코드에만 응답합니다.

- ❌ **정답: 거짓** — API 개발자는 상태 코드를 구성하고 API Gateway가 모의 통합 요청에 응답하는 방법을 결정할 수 있습니다.

**문제 4**: 리소스는 애플리케이션이 리소스 경로를 통해 액세스할 수 있는 논리적 엔터티입니다.

- ✅ **정답: 참**

**문제 5**: 스테이지 변수는 REST API의 배포 스테이지와 관련된 구성 특성으로 정의할 수 있는 이름-값 쌍입니다.

- ✅ **정답: 참**

**문제 6**: 다양한 기능의 API 호출 목록을 지능적으로 처리하려면, AWS Lambda 함수를 CRUD 백엔드로 사용하면 됩니다.
...
**문제 7**: API Gateway REST API의 기본 엔드포인트 유형은 리전(Regional)입니다.

- ❌ **정답: 거짓** — 엔드포인트 유형 문서는 엣지 최적화 엔드포인트를 설명하면서 **"이것이 API Gateway REST API의 기본 엔드포인트 유형"** 이라고 명시합니다. 같은 리전 클라이언트가 대부분이면 리전 유형을 명시적으로 선택하세요. ([3.3절](#33-엔드포인트-유형과-기본값))

> — 출처: [API endpoint types for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-endpoint-types.html)

**문제 8**: 메서드 권한 부여에 쓸 수 있는 값은 `NONE`과 `AWS_IAM` 두 개입니다.

- ❌ **정답: 거짓** — `authorizationType`은 **필수 파라미터**이고 유효값은 `NONE`, `AWS_IAM`, `CUSTOM`(Lambda 권한 부여자), `COGNITO_USER_POOLS` **네 개**입니다. 교재 목록에는 Lambda 권한 부여자와 Cognito 사용자 풀이 빠져 있습니다. ([3.4절](#34-메서드-권한-부여-유효값))

> — 출처: [PutMethod](https://docs.aws.amazon.com/apigateway/latest/api/API_PutMethod.html)

**문제 9**: HTTP API도 Mock 통합을 지원하므로 백엔드 없이 응답을 만들 수 있습니다.

- ❌ **정답: 거짓** — REST·HTTP 비교 문서의 통합 표는 **Mock 통합을 REST API 전용**으로 표시합니다. 테스트 호출, 캐싱, 요청 검증, 카나리 릴리스 배포, 요청 본문 변환, 실행 로그, X-Ray 추적도 REST API 전용입니다. ([2.3절](#23-rest-api와-http-api를-고르는-기준))

> — 출처: [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html)

**문제 10**: Lambda 함수와 통합할 때는 Lambda 프록시 통합과 Lambda 사용자 지정 통합 중 무엇을 골라도 문서상 차이가 없습니다.

- ❌ **정답: 거짓** — 문서는 Lambda 프록시 통합을 **"Lambda 함수를 호출하는 데 선호되는 통합 유형"** 으로 강력히 권장합니다. 설정이 간소하고 기존 설정을 해체하지 않고도 백엔드와 함께 발전할 수 있기 때문입니다. Lambda 사용자 지정 통합은 매핑 템플릿을 여러 엔드포인트에서 재사용하는 고급 시나리오용입니다. ([3.5절](#35-통합-유형))

> — 출처: [Choose an API Gateway API integration type](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-integration-types.html)

**문제 11**: Lambda 프록시 통합에서 함수가 아무 JSON이나 반환하면 API Gateway가 그대로 클라이언트에 전달합니다.

- ❌ **정답: 거짓** — REST API의 Lambda 프록시 통합은 `isBase64Encoded`·`statusCode`·`headers`·`multiValueHeaders`·`body` 형식을 요구하며, **형식이 다르면 `502 Bad Gateway`** 를 반환합니다. HTTP API의 페이로드 형식 `2.0`에서만 `statusCode` 없는 유효 JSON을 200 응답으로 추론합니다. ([3.6절](#36-lambda-프록시-통합의-입력과-출력-형식))

> — 출처: [Lambda proxy integrations in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-lambda-proxy-integrations.html)

**문제 12**: 통합 패스스루 동작은 두 가지 조건으로 결정됩니다.

- ❌ **정답: 거짓** — 세 가지 선택지가 있는 **설정**입니다. `WHEN_NO_MATCH`, `WHEN_NO_TEMPLATES`(**권장**), `NEVER` 이며 뒤의 두 값은 매핑되지 않은 콘텐츠 유형을 **`415 Unsupported Media Type`** 으로 거부합니다. 프록시 통합에서 패스스루 동작을 수정할 수 없다는 교재 서술은 맞습니다. ([3.7절](#37-통합-패스스루-동작))

> — 출처: [Method request behavior for payloads without mapping templates for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/integration-passthrough-behaviors.html)

**문제 13**: 데이터를 변환해야 하면 매핑 템플릿을 쓰는 것이 문서가 권장하는 첫 번째 방법입니다.

- ❌ **정답: 거짓** — 권장 순서는 **① 프록시 통합 → ② 파라미터 매핑(VTL 불필요) → ③ 매핑 템플릿 변환** 입니다. 파라미터 매핑은 경로·쿼리 문자열·헤더 값을 수정하지만 **본문은 수정할 수 없습니다.** 본문을 바꿔야 하거나 조건부 재정의가 필요하고 프록시 통합을 쓸 수 없을 때 매핑 템플릿을 씁니다. ([4.2절](#42-데이터-변환의-세-가지-선택))

> — 출처: [Data transformations for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/rest-api-data-transformations.html)

**문제 14**: 요청 검증을 켜면 필수 요청 파라미터의 유형과 형식까지 검사합니다.

- ❌ **정답: 거짓** — API Gateway는 파라미터의 **존재만 확인하고 유형이나 형식은 확인하지 않습니다.** 페이로드는 콘텐츠 유형별로 구성된 JSON 스키마로 검증하고, 일치하는 콘텐츠 유형이 없으면 검증이 수행되지 않습니다. 실패 시 `400`을 반환하고 결과를 CloudWatch Logs에 게시합니다. ([4.4절](#44-요청-검증))

> — 출처: [Request validation for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-method-request-validation.html)

**문제 15**: 요청 검증자 구성에서 `validateRequestBody`와 `validateRequestParameters` 값은 문자열입니다.

- ❌ **정답: 거짓** — 두 필드는 **Boolean** 플래그입니다. 교재 슬라이드 18의 `"false"`·`"true"` 표기는 따옴표를 빼고 `false`·`true`로 써야 합니다. 문자열 `"false"`는 JSON에서 참으로 취급될 수 있습니다. ([4.4절](#44-요청-검증))

> — 출처: [RequestValidator](https://docs.aws.amazon.com/apigateway/latest/api/API_RequestValidator.html)

**문제 16**: 매핑 템플릿에서 배열을 순회하려면 `$elem` 변수를 그냥 쓰면 됩니다.

- ❌ **정답: 거짓** — 공식 예제에서 컬렉션 순회는 **`#foreach($elem in $inputRoot.Notes)` ... `#end`** 로 표현하고 `$foreach.hasNext`로 구분자를 넣습니다. 반복 지시자 없이 참조한 변수는 값이 채워지지 않습니다. 다만 AWS 문서 예제에도 같은 표기가 있어 문서가 이를 오류로 명시하지는 않습니다([9.5절](#95-검증하지-못한-항목)). ([4.5절](#45-매핑-템플릿-요청-방향))

> — 출처: [Examples using variables for mapping template transformations for API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-mapping-variable-examples.html)

**문제 17**: 카나리 릴리스를 만들려면 새 스테이지를 만들고 그 스테이지에서 카나리를 활성화합니다.

- ❌ **정답: 거짓** — 카나리는 별도 스테이지가 아니라 **프로덕션 릴리스가 배포된 같은 스테이지의 `canarySettings`** 입니다. 스테이지는 최초 배포와, 카나리는 이후 배포와 연결됩니다. 카나리를 활성화하면 비활성화하고 설정을 제거할 때까지 그 스테이지를 다른 비카나리 배포에 연결할 수 없습니다. ([7.4절](#74-canary-릴리스-단계))

> — 출처: [Set up an API Gateway canary release deployment](https://docs.aws.amazon.com/apigateway/latest/developerguide/canary-release.html)

**문제 18**: 카나리를 승격하면 카나리가 비활성화되고 배포가 일반 프로덕션 배포로 돌아갑니다.

- ❌ **정답: 거짓** — **승격은 스테이지의 카나리를 비활성화하지 않습니다.** 승격은 배포 ID와 스테이지 변수를 스테이지로 복사하고 `percentTraffic`을 **0.0** 으로 내리는 것이며, 일반 프로덕션 배포로 되돌리려면 `canarySettings`를 제거해야 합니다. ([7.3절](#73-canary-릴리스))

> — 출처: [Promote a canary release](https://docs.aws.amazon.com/apigateway/latest/developerguide/promote-canary-deployment.html)

**문제 19**: API 캐싱을 켜면 스테이지의 모든 메서드가 캐싱됩니다.

- ❌ **정답: 거짓** — API 안전성·가용성을 위해 **`GET` 메서드만** 기본으로 캐싱이 활성화되고, 다른 메서드는 메서드 설정 재정의로 켭니다. 기본 TTL은 300초, 최대 3600초이고 `TTL=0`은 비활성화입니다. 캐시 가능한 응답의 최대 크기는 1,048,576바이트입니다. ([7.5절](#75-응답-캐싱))

> — 출처: [Cache settings for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html)

**문제 20**: 캐시 용량을 늘리면 기존 캐시 데이터는 그대로 유지됩니다.

- ❌ **정답: 거짓** — 용량을 바꾸면 API Gateway가 **기존 캐시 인스턴스를 제거하고 새로 만들기 때문에 기존 캐시 데이터가 모두 삭제됩니다.** 캐시 인스턴스 생성·삭제에는 약 4분이 걸립니다. ([7.5절](#75-응답-캐싱))

> — 출처: [Cache settings for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html)

**문제 21**: API Gateway의 제한 설정은 서버 측 제한과 클라이언트당 제한 두 가지입니다.

- ❌ **정답: 거짓** — **네 가지**입니다: AWS 제한 한도, 계정별 한도, API별·스테이지별 한도, 클라이언트별 한도. 적용 순서는 클라이언트별·메서드별 → 스테이지 메서드별 → 계정 수준 → AWS 리전 순입니다. 계정·리전당 기본 스로틀 할당량은 10,000 RPS이고 최대 버킷 용량은 5,000 요청입니다. ([7.6절](#76-요청-제한과-사용량-계획))

> — 출처: [Throttle requests to your REST APIs for better throughput in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-request-throttling.html)

**문제 22**: API 키를 요구하면 API 액세스를 인증·권한 부여할 수 있습니다.

- ❌ **정답: 거짓** — 문서의 모범 사례는 **"API 액세스를 제어하는 인증·권한 부여에 API 키를 쓰지 말라"** 입니다. 한 사용량 계획에 여러 API가 있으면 그중 한 API의 유효한 API 키를 가진 사용자가 그 계획의 모든 API에 액세스할 수 있습니다. 액세스 제어에는 IAM 역할, Lambda 권한 부여자, Amazon Cognito 사용자 풀을 씁니다. ([7.6절](#76-요청-제한과-사용량-계획))

> — 출처: [Usage plans and API keys for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html)

**문제 23**: 사용량 계획의 할당량을 설정하면 API 비용의 상한을 보장할 수 있습니다.

- ❌ **정답: 거짓** — 제한과 할당량은 하드 한도가 아니라 **best-effort 로 적용되므로 클라이언트가 초과할 수 있습니다.** 비용 통제나 API 액세스 차단에 의존하지 말고 비용 모니터링에는 **AWS Budgets**, 요청 관리에는 **AWS WAF** 를 쓰라고 문서가 안내합니다. ([7.6절](#76-요청-제한과-사용량-계획))

> — 출처: [Usage plans and API keys for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html)

**문제 24**: Lambda 통합의 시간 초과는 Lambda 함수의 최대 실행 시간까지 늘릴 수 있습니다.

- ❌ **정답: 거짓** — REST API의 **통합 시간 초과 기본 범위는 50밀리초~29초**이며 모든 통합 유형에 적용됩니다. 리전·프라이빗 API는 29초를 넘겨 상향할 수 있지만 계정의 리전 수준 스로틀 할당량을 낮춰야 할 수 있고, **엣지 최적화 API는 상향할 수 없습니다.** 페이로드 크기 상한 10MB도 상향 불가입니다. ([3.8절](#38-rest-api-할당량))

> — 출처: [Quotas for configuring and running a REST API in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-execution-service-limits-table.html)

**문제 25**: WebSocket API에서는 각 경로마다 권한 부여를 다르게 구성할 수 있습니다.

- ❌ **정답: 거짓** — WebSocket 연결은 상태 저장이므로 권한 부여는 **`$connect` 경로에만** 구성할 수 있고 인증·권한 부여는 연결 시점에만 수행됩니다. 이 설정은 `$connect` 경로만이 아니라 **API 전체**에 적용됩니다. `$connect`가 다른 경로를 보호하는 구조입니다. ([2.4절](#24-websocket-api-작업))

> — 출처: [Create routes for WebSocket APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/websocket-api-develop-routes.html)

**문제 26**: API Gateway 콘솔의 Test 기능으로 메서드를 호출하면 실제 리소스는 변경되지 않습니다.

- ❌ **정답: 거짓** — **콘솔에서의 메서드 테스트는 콘솔 외부에서 메서드를 호출하는 것과 같습니다.** API 리소스를 삭제하는 메서드를 콘솔로 호출해 성공하면 그 리소스는 실제로 삭제됩니다. 되돌릴 수 없는 변경이 발생할 수 있습니다. 결과의 Logs는 시뮬레이션이지만 호출 결과는 실제입니다. ([6.2절](#62-api-gateway-콘솔을-사용한-테스트))

> — 출처: [Use the API Gateway console to test a REST API method](https://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-test-method.html)

**문제 27**: API Gateway는 Swagger 2.0 정의 파일만 가져올 수 있습니다.

- ❌ **정답: 거짓** — **OpenAPI v2.0(Swagger 2.0에 해당)과 OpenAPI v3.0** 두 규격을 지원합니다. 내보내기 경로도 OpenAPI 3.0은 `exports/oas30`, OpenAPI 2.0은 `exports/swagger`로 나뉩니다. 문서는 이 절을 "OpenAPI extensions for API Gateway"로 부릅니다. ([5.1절](#51-swagger에서-openapi로))

> — 출처: [Develop REST APIs using OpenAPI in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-import-api.html)

**문제 28**: AWS WAF를 API에 연결하면 리소스 정책이 먼저 평가되고 그다음 WAF 규칙이 평가됩니다.

- ❌ **정답: 거짓** — **AWS WAF 규칙이 리소스 정책, IAM 정책, Lambda 권한 부여자, Amazon Cognito 권한 부여자보다 먼저 평가됩니다.** 리소스 정책이 허용하는 CIDR 블록을 AWS WAF가 차단하면 AWS WAF가 우선하고 리소스 정책은 평가되지 않습니다. 웹 ACL은 API 스테이지에 연결합니다. ([2.7절](#27-프라이빗-rest-api와-aws-waf))

> — 출처: [Use AWS WAF to protect your REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-control-access-aws-waf.html)

**문제 29**: 메서드 수준 CloudWatch 지표는 기본으로 전송되며 추가 요금이 없습니다.

- ❌ **정답: 거짓** — `ApiName, Method, Resource, Stage` 차원의 지표는 **상세 CloudWatch 지표를 명시적으로 활성화해야** 전송되고, 활성화하면 계정에 **추가 요금이 발생합니다.** API 수준·스테이지 수준 지표에는 요금이 부과되지 않습니다. ([8.2절](#82-cloudwatch-지표))

> — 출처: [Amazon API Gateway dimensions and metrics](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-metrics-and-dimensions.html)

**문제 30**: 프라이빗 REST API는 TLS 1.0 이상 모든 버전을 지원합니다.

- ❌ **정답: 거짓** — 프라이빗 API는 **TLS 1.2만 지원**하며 이전 버전은 지원하지 않습니다. HTTP/2 프로토콜로 요청하면 HTTP/1.1로 강제되고, IP 주소 유형은 dualstack만 지원합니다. 프라이빗 API를 엣지 최적화 API로 변환할 수도 없습니다. ([2.7절](#27-프라이빗-rest-api와-aws-waf))
...
| 주제 | 기억할 것 |
|---|---|
| API 유형 | REST / HTTP / WebSocket. **기능과 가격으로 고릅니다.** 요청 검증·매핑 템플릿·캐싱·카나리·사용량 계획·테스트 호출·실행 로그·X-Ray·Mock 통합은 REST 전용. 자동 배포·JWT 권한 부여자는 HTTP 전용 |
| 엔드포인트 | `{api-id}.execute-api.{region}.amazonaws.com`. 유형은 엣지 최적화(**기본**) / 리전 / 프라이빗. 프라이빗은 인터페이스 VPC 엔드포인트로만, TLS 1.2만 |
| 권한 부여 | `authorizationType`은 **필수**이고 값은 `NONE` · `AWS_IAM`(SigV4) · `CUSTOM`(Lambda 권한 부여자, `REQUEST` 권장) · `COGNITO_USER_POOLS`. AWS WAF가 이들보다 **먼저** 평가됩니다 |
| 통합 | 프록시(`AWS_PROXY`·`HTTP_PROXY`)는 요청·응답 설정 없음, 비프록시(`AWS`·`HTTP`)는 둘 다 구성, `MOCK`은 백엔드 미호출. **Lambda 프록시가 권장** 이고 형식 위반은 `502`. 프라이빗 통합은 `HTTP_PROXY` + `VPC_LINK` |
| 데이터 변환 | 권장 순서는 **프록시 통합 → 파라미터 매핑 → 매핑 템플릿**. 매핑 템플릿은 VTL + JSONPath, `Content-Type` 기준. 순회는 `#foreach` ... `#end` |
| 검증과 모델 | 모델은 JSON schema draft 4. 파라미터 검증은 **존재만** 확인. `validateRequestBody`·`validateRequestParameters`는 **Boolean**. 실패는 `400` |
| 스테이지 | 스테이지는 배포(스냅샷)에 대한 **명명된 참조**. 캐싱·제한·로깅·스테이지 변수·카나리를 설정하고 변경 후 **재배포** 필요 |
| 스테이지 변수 | `$stageVariables.이름`. Lambda 별칭은 `function:이름:${stageVariables.변수}`. 같은 계정만, 민감한 데이터 금지 |
| Canary | **같은 스테이지의 `canarySettings`.** `percentTraffic`은 Double(0.0~100.0), `useStageCache`는 Boolean. 승격은 배포 ID·변수 복사 + 비율 0.0이며 카나리는 계속 남습니다 |
| 캐싱 | 스테이지 단위. TTL 기본 300초 / 최대 3600초 / `0`은 비활성화. **`GET`만 기본**, 캐시 키 필수, 응답 최대 1MiB, 용량 변경 시 데이터 삭제, `Cache-Control: max-age=0`으로 무효화 |
| 제한 | **네 가지** 한도(AWS / 계정 / API·스테이지 / 클라이언트). 계정·리전당 10,000 RPS · 버스트 5,000. 초과는 `429`. 모두 best-effort |
| 사용량 계획 | API 키는 클라이언트 식별용이며 **인증·권한 부여에 쓰지 않습니다.** 비용 통제는 AWS Budgets, 차단은 AWS WAF |
| 할당량 | **통합 시간 초과 29초**, **페이로드 10MB**, API당 리소스 300 · 스테이지 10 · 권한 부여자 10 · 모델 400KB |
| 테스트 | 콘솔 **Test 탭** 또는 `test-invoke-method`. 호출 URL을 우회하고 배포 전에도 가능하지만 **결과는 실제**이고 로그만 시뮬레이션입니다 |
| 코드형 API | **OpenAPI v2.0·v3.0**. `x-amazon-apigateway-integration`의 `type`은 소문자(`aws_proxy` 등), Lambda 호출은 `httpMethod: POST`, `timeoutInMillis` 최대 29,000 |
| 관측 가능성 | 실행 로깅(API Gateway 관리, REST 전용)과 액세스 로깅(개발자 관리)은 독립. 지표 7개, 1분 주기, 메서드 수준 지표는 **유료**. X-Ray는 스테이지 단위 |
