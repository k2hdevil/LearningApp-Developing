...
> - 외부 문서로 판별할 수 없는 항목은 표기를 붙이지 않고 본문에서 짚은 뒤 [9장](#9-교재-대비-변경-사항)에 모았습니다.
...
앞선 모듈에서 백엔드 데이터베이스 스토리지 시스템을 구축하고 컴퓨팅 처리를 구성했습니다. 이제 사용자가 인터넷을 통해 이러한 서비스에 액세스할 수 있는 방법을 구성합니다. Amazon API Gateway는 애플리케이션의 모든 서비스를 함께 연결하는 역할을 합니다.
...
| 주제 | 이 문서 |
|---|---|
| Amazon API Gateway란 무엇입니까? | [2장](#2-amazon-api-gateway란-무엇입니까) |
| API Gateway 작업 | [3장](#3-api-gateway-작업) |
| 요청 및 응답 처리 | [4장](#4-요청-및-응답-처리) |
| 코드형 API 설계 | [5장](#5-코드형-api-설계) |
| API 테스트 | [6장](#6-api-테스트) |
| API 배포 | [7장](#7-api-배포) |
| 데모 / 실습 5 | [7.7절](#77-데모-주제) |

관측 가능성(로깅, 지표, 추적)은 [8장](#8-관측-가능성-로깅-지표-추적)에 따로 모았습니다. 애플리케이션 아키텍처에는 Amazon CloudWatch와 AWS X-Ray가 구성 요소로 등장하지만 앞 장에서 다루지 않기 때문입니다.

예제 애플리케이션의 API 리소스와 메서드는 다음과 같습니다.
...
API Gateway는 **REST, HTTP, WebSocket** 세 종류의 API를 만들 수 있습니다.
...
🔄 "HTTP API가 REST API보다 **대기 시간이 짧다**"는 설명은 자주 보이지만 주의가 필요합니다. 두 API를 비교하는 현재 문서가 제시하는 축은 **기능 수와 가격**이며, 대기 시간은 비교 항목으로 나오지 않습니다. 비용이 더 낮다는 서술은 문서와 일치합니다("HTTP APIs are designed with minimal features so that they can be offered at a lower price"). 대기 시간 비교는 이 문서에서 확인하지 못했으므로 [9.5절](#95-검증하지-못한-항목)에 남겨 두었습니다.
...
어느 한쪽이 일률적으로 낫지는 않습니다. 문서는 **필요한 기능 유무로 고르라**고 안내합니다.
...
**이 모듈이 REST API를 쓰는 데는 이유가 있습니다.** 요청 검증, 매핑 템플릿, 캐싱, 카나리 릴리스, 사용량 계획처럼 이 모듈이 다루는 기능이 대부분 REST API에만 있기 때문입니다.

> — 출처: [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html)

### 2.4 WebSocket API 작업 🔄

WebSocket API는 AWS 서비스(예: Lambda, DynamoDB)나 HTTP 엔드포인트를 위한 **상태 저장 프런트엔드**입니다. 양방향이므로 클라이언트가 명시적으로 요청하지 않아도 서비스가 데이터를 푸시할 수 있습니다. 대표적인 용도로 채팅 애플리케이션, 협업 플랫폼, 멀티플레이어 게임, 금융 거래 플랫폼이 있습니다.
...
예제 애플리케이션은 `GET`·`POST`·`DELETE`로 요청을 검증하고 Lambda 함수가 반환한 응답을 변환합니다.
...
API Gateway의 개발자 기능 여덟 가지와, 각 항목이 이 문서 어디에서 자세히 다뤄지는지 정리했습니다.
...
| SDK 생성 | 지원 언어는 Java, JavaScript, Java for Android, Objective-C 또는 Swift for iOS, Ruby입니다 | [5.3절](#53-api-정의-가져오기와-내보내기) |
| Mock 통합 | 통합 백엔드 없이 API Gateway에서 바로 응답을 생성합니다 | [6.3절](#63-mock-통합) |
| 응답 캐싱 | 엔드포인트 호출 수를 줄이고 요청 대기 시간을 개선합니다 | [7.5절](#75-응답-캐싱) |

액세스 제어 메커니즘은 🆕 현재 문서 기준으로 다음과 같이 넓습니다.
...
액세스 제어에는 프라이빗 REST API(VPC 엔드포인트 정책)와 AWS WAF 통합도 포함됩니다. 각각의 세부는 다음과 같습니다.
...
API Gateway는 요청 쪽에서 **권한 부여·구성·계측·변환·매핑**을 수행하고, 응답 쪽에서 **구성·변환·매핑**을 수행합니다. 이 구성은 개발자의 책임입니다.

```text
                      요청 방향 ─────────────────▶
  클라이언트 ─▶ [권한 부여 · 구성 · 계측 · 변환 · 매핑] ─▶ 백엔드(통합 엔드포인트)
  클라이언트 ◀─ [        구성 · 변환 · 매핑          ] ◀─ 백엔드
                      ◀───────────────── 응답 방향
```
...
통합 응답 설정은 **비프록시 통합에만** 해당합니다. 프록시 통합에서는 통합 요청도 통합 응답도 설정하지 않습니다.
...
**API 엔드포인트**는 특정 리전에 배포된 API의 호스트 이름이며 형식은 `{api-id}.execute-api.{region}.amazonaws.com` 입니다.
...
예제 애플리케이션의 API 리소스는 `/notes`와 `/notes/search` 두 개입니다([1장](#1-모듈-개요)의 메서드 목록과 일치). 일부 원본 자료에는 `/notes/list`라는 세 번째 리소스가 등장하지만 메서드 목록과 어긋나므로 이 문서는 두 리소스로 씁니다([9.1절](#91-교재와-다른-점)).

> — 출처: [Amazon API Gateway concepts](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-basic-concept.html)

### 3.3 엔드포인트 유형과 기본값 🔄

세 엔드포인트 유형의 이름과 특성은 다음과 같습니다.
...
🔄 기본값에 주의가 필요합니다. REST API의 **기본 엔드포인트 유형은 엣지 최적화**입니다. 현재 문서는 엣지 최적화 엔드포인트를 "API Gateway REST API의 기본 엔드포인트 유형"으로 명시하고, 개념 문서도 이를 "API Gateway API의 기본 호스트 이름"으로 정의합니다("리전 엔드포인트가 기본"이라는 설명은 사실과 다릅니다). 같은 리전 클라이언트가 대부분이면 리전 유형을 명시적으로 선택하세요.
...
권한 부여로 메서드 액세스를 제어합니다. `none`·`AWS_IAM` 두 값만 있는 것으로 오해하기 쉽지만, 🔄 `PutMethod` API 참조에서 `authorizationType`은 **필수 파라미터**이고 유효값은 네 개입니다.
...
`authorizerId`는 유형이 `CUSTOM` 또는 `COGNITO_USER_POOLS`일 때 쓸 `Authorizer` 식별자를 지정합니다. `apiKeyRequired`는 Boolean이며, API 키로 메서드를 호출하게 하려면 이 값을 `true`로 설정합니다.

**Lambda 권한 부여자.** 🆕 이 기능은 **이전에 사용자 지정 권한 부여자(custom authorizer)라고 불렸습니다.** 오래된 자료에서 그 이름을 만날 수 있습니다. 유형은 두 가지입니다.
...
Mock 통합은 백엔드를 호출하지 않으므로 패스스루가 적용되지 않습니다(일부 원본 자료는 이 칸을 잘못 표기하는데, 위 표에서 바로잡았습니다 — [9.1절](#91-교재와-다른-점)).
...
| 항목 | 실제 내용 |
|---|---|
| Lambda 통합 권장 | **Lambda 프록시가 "Lambda 함수를 호출하는 데 선호되는 통합 유형"** 입니다. 설정이 간소하고 기존 설정을 해체하지 않고도 백엔드와 함께 발전할 수 있습니다. Lambda 사용자 지정 통합은 입력·출력 형식 요구가 비슷한 여러 엔드포인트에서 매핑 템플릿을 재사용할 때 쓰며 더 복잡한 시나리오용입니다 |
| `HTTP`·`HTTP_PROXY` 정의 | 두 유형은 **백엔드 HTTP 엔드포인트** 통합입니다. 프라이빗 통합은 `HTTP_PROXY` + `connectionType=VPC_LINK` + VPC 링크 V2로 구성하는 **별도 설정**입니다. VPC 링크 V2는 NLB와 ALB를 모두 대상으로 삼을 수 있고 `connectionId`를 스테이지 변수로 넘길 수 있습니다 |
| Mock 통합의 용도 | 테스트와 협업 개발 외에 **CORS 관련 헤더 반환**에도 씁니다. API Gateway 콘솔은 CORS 지원을 위해 `OPTIONS` 메서드를 Mock 통합으로 통합하며 게이트웨이 응답도 Mock 통합의 예입니다 |

**HTTP API가 지원하는 통합.** 🔄 HTTP API는 Lambda 프록시, AWS 서비스, VPC 프라이빗 리소스, HTTP 프록시 통합을 지원합니다. 다만 **Mock 통합은 REST API 전용**입니다(REST·HTTP 비교 문서의 통합 표 기준).
...
`AWS_PROXY`는 클라이언트 요청을 그대로 Lambda 함수 호출과 통합합니다. 다만 **고정된 입력·출력 형식**이 있고 이를 지키지 않으면 호출이 실패합니다.
...
🔄 패스스루 동작은 **세 가지 선택지가 있는 설정**이며 권장 값이 있습니다.

| 콘솔 표기 | `passthroughBehavior` | 동작 |
|---|---|---|
| When no template matches the request Content-Type header | `WHEN_NO_MATCH` | 메서드 요청 콘텐츠 유형이 매핑 템플릿에 연결된 어떤 콘텐츠 유형과도 일치하지 않을 때 본문을 변환 없이 전달합니다 |
| When there are no templates defined (**recommended**) | `WHEN_NO_TEMPLATES` | 통합 요청에 매핑 템플릿이 정의되지 않았을 때만 변환 없이 전달합니다. 템플릿이 정의되어 있는데 콘텐츠 유형이 일치하지 않으면 **`415 Unsupported Media Type`** 으로 거부합니다 |
| Never | `NEVER` | 매핑 템플릿이 정의되지 않아도 전달하지 않고, 매핑되지 않은 콘텐츠 유형은 `415`로 거부합니다 |

`Content-Type` 헤더가 없으면 API Gateway는 `application/json`으로 기본 처리합니다. 프록시 통합에서는 API Gateway가 전체 요청을 백엔드로 전달하며 패스스루 동작을 수정할 수 없습니다.
...
요청·응답 처리는 매핑 템플릿으로 하는 것이 널리 알려져 있고 매핑 템플릿은 지금도 그대로 동작합니다. 다만 **현재 문서가 제시하는 권장 순서는 다릅니다.**
...
모델은 페이로드의 데이터 구조를 정의하며 **JSON schema draft 4** 로 표현합니다. 예제 애플리케이션의 요청 페이로드와 모델입니다.
...
🆕 함께 알아 둘 확장 기능입니다. `enum`으로 허용 값을 제한하고 `minimum`·`maximum`으로 숫자 범위를 제약할 수 있습니다. 긴 모델은 참조 프리미티브(`$ref`)로 `definitions` 안의 재사용 정의를 가리킬 수 있고, 다른 API의 모델을 `apigateway.amazonaws.com`의 모델 경로로 참조할 수도 있습니다. API당 모델 크기 상한은 400KB입니다.
...
요청 검증자 구성 예입니다. 🔄 이 두 값은 `RequestValidator` API 참조에서 **Boolean** 이므로 따옴표 없이 씁니다. 문자열 `"false"`로 적으면 JSON에서 참으로 취급될 수 있어 의도와 다르게 동작할 위험이 있습니다.
...
다음은 필수 파라미터 `q1`이 비어 있지 않고 설정되어야 하는 요청 예입니다.
...
검증할 메서드 요청 페이로드는 배열 요소 키를 **`Note`** 로 씁니다. [4.3절](#43-예-요청-모델)의 `NotesInputModel`이 배열 요소에 `Note`를 필수로 요구하기 때문입니다. 일부 원본 자료는 이 키를 `Notes`로 적는데, 그 페이로드를 이 모델로 검증하면 `400`으로 실패합니다([9.1절](#91-교재와-다른-점)).
...
🔄 흔히 보이는 다음 템플릿은 그대로 실행하면 값이 채워지지 않습니다. `#set`으로 `$inputRoot`를 선언하지만 쓰지 않고, `$elem`을 **반복 지시자 없이** 참조하기 때문입니다. 공식 예제에서 컬렉션 순회는 항상 `#foreach`로 시작하고 `#end`로 닫습니다.

문제가 있는 형태(그대로 실행하면 `Notes` 값이 비어 있습니다):
...
응답 방향에서는 JSON 페이로드를 XML로 변환하고 응답에 필요 없는 환경 정보를 제거할 수 있습니다. 백엔드 페이로드는 다음과 같습니다.
...
흔히 보이는 원본 템플릿도 [4.5절](#45-매핑-템플릿-요청-방향)과 같은 이유로 `$elem`이 정의되지 않습니다. 교정한 형태입니다.
...
"Swagger"라는 이름으로 알려진 이 기능은 🔄 현재 **"Develop REST APIs using OpenAPI in API Gateway"** 로 불리며 **OpenAPI v2.0**(Swagger 2.0에 해당)과 **OpenAPI v3.0** 두 규격을 지원합니다. 확장 목록 페이지 제목도 "OpenAPI extensions for API Gateway"입니다(URL 경로에는 여전히 `swagger-extensions`가 남아 있습니다).

기능은 이름만 바뀌었고 내용은 그대로입니다.
...
예제 API 정의를 OpenAPI 3.0 표기로 쓰면 다음과 같습니다.
...
통합 유형을 정하는 `type` 속성을 빠뜨리기 쉬운데, `type`은 통합의 성격을 결정하는 속성이므로 반드시 지정합니다.
...
`x-amazon-apigateway-integration`은 OpenAPI Operation 객체의 확장 속성이며 결과는 API Gateway `Integration` 객체입니다. 주요 속성입니다.
...
**SDK 생성.** 지원 언어는 Java, JavaScript, Java for Android, Objective-C 또는 Swift for iOS, Ruby입니다. SDK는 API를 만들고 테스트하고 스테이지에 배포한 뒤에 생성하며, 최소 한 번 배포해야 합니다. AWS CLI로도 생성할 수 있습니다.

내보내기 문서는 Postman 확장을 지원합니다. Postman 자체는 AWS가 운영하지 않는 타사 도구이며 주소는 `www.postman.com` 입니다.
...
**API URL을 호출하려면 먼저 API Gateway에 배포해야 합니다.** 배포된 API를 호출할 때 클라이언트는 API 작업을 담당하는 API Gateway 구성 요소 서비스인 **`execute-api`** 의 URL에 요청을 제출합니다.
...
🔄 "Method Execution 창의 Client 상자에서 TEST를 선택한다"는 안내는 이전 콘솔의 UI 기준입니다. 현재 절차는 다음과 같습니다.
...
중요한 주의가 있습니다. **콘솔에서의 메서드 테스트는 콘솔 외부에서 메서드를 호출하는 것과 같습니다.** 예를 들어 API 리소스를 삭제하는 메서드를 콘솔로 호출해 성공하면 그 리소스는 실제로 삭제됩니다. 되돌릴 수 없는 변경이 발생할 수 있습니다.
...
Mock 통합으로 통합 백엔드 없이 API Gateway에서 바로 응답을 생성할 수 있습니다. 세 가지 효용은 완성된 백엔드 없이 테스트, 프로덕션 배포 전에 다양한 시나리오 테스트, API 개발 속도 향상이며, 문서는 API의 개요와 내비게이션을 제공하는 **랜딩 페이지** 프로비저닝도 용도로 듭니다.
...
즉 Mock 통합은 200 상태 코드에만 응답하는 것이 아니라, 위 예처럼 500이나 조건부 상태 코드도 반환할 수 있습니다.
...
**스테이지는 API의 스냅샷인 배포에 대한 명명된 참조입니다.** API를 배포하려면 **API 배포를 만들고 스테이지에 연결**해야 합니다.
...
🔄 "배포가 완료되면 **Stage Editor 창**에서 설정한다"는 안내는 이전 콘솔 기준입니다. 현재 콘솔에서 스테이지 설정은 **Stages** 창에서 스테이지를 고른 뒤 두 곳으로 나뉘어 있습니다.
...
스테이지 변수는 REST API의 배포 스테이지와 연결된 구성 속성으로 정의하는 **이름-값 쌍**이며 **환경 변수처럼 작동**하고 **API 설정과 매핑 템플릿에서** 쓸 수 있습니다.
...
예를 들어 `/notes`가 `list_function:PROD` 또는 `list_function:DEV`를 호출하도록, Lambda **별칭** 자리에 스테이지 변수를 쓸 수 있습니다.

🔄 이때 표기에 주의합니다. 올바른 표기는 `function:<함수이름>:${stageVariables.<버전변수>}` 이므로 중괄호가 달러 기호 **뒤에** 와야 합니다.
...
예를 들어 **카나리 버전이 트래픽의 10%** 를 받고 나머지 **90%가 안정 버전**으로 라우팅되도록 구성할 수 있습니다.
...
**승격.** 🔄 카나리 버전을 승격하면 트래픽 100%가 그 버전으로 갑니다. 실제 메커니즘은 세 단계입니다.
...
🔄 카나리를 **새 스테이지**에 만든다고 오해하기 쉽지만, 카나리는 별도 스테이지가 아니라 **같은 스테이지의 `canarySettings`** 입니다. 올바른 단계는 다음과 같습니다.

| 단계 | 내용 |
|---|---|
| 1/3 | 기능 변경 수행 / **기존 스테이지에** `canarySettings`를 추가해 카나리 활성화 / 필요하면 `useStageCache`·`stageVariableOverrides` 설정, 실행·액세스 로깅 활성화 |
| 2/3 | `percentTraffic`으로 비율 설정(예: 10) / 새 배포를 만들어 카나리에 연결. 이때 카나리는 새 배포, 스테이지는 기존 배포를 가리킵니다 |
| 3/3 | **카나리 승격** — 배포 ID·스테이지 변수를 스테이지로 복사하고 `percentTraffic`을 0.0으로 내립니다. 카나리를 완전히 끄려면 `canarySettings`를 제거합니다 |

카나리 설정 코드를 쓸 때 자주 나오는 표기 문제가 있습니다. `percentTraffic`·`useStageCache`·`metricsEnabled`를 **문자열로** 적거나 따옴표가 깨지거나 닫는 중괄호를 빠뜨리는 경우입니다. 🔄 `percentTraffic`은 Double, `useStageCache`와 `metricsEnabled`는 Boolean이므로 아래처럼 씁니다.
...
캐싱은 엔드포인트에 적용되는 호출 수를 줄이고 요청 대기 시간을 개선합니다. **REST API 전용** 기능이며 **스테이지 단위**로 활성화합니다. 설정할 값들은 다음과 같습니다.
...
🔄 제한 설정을 서버 측·클라이언트당 두 가지로만 이해하기 쉽지만, 현재 문서는 **네 가지**를 제시하고 적용 순서까지 명시합니다.
...
데모에서 다루는 콘솔 속성과, 각 항목을 이 문서 어디에서 설명하는지 정리했습니다.
...
애플리케이션 아키텍처에는 Amazon CloudWatch와 AWS X-Ray가 구성 요소로 등장하고, 카나리 단계에서도 CloudWatch 로깅을 활성화합니다. 앞 장에서 이 기능을 자세히 다루지 않으므로 이 장에 모았습니다. 이 장의 기능은 **모두 REST API 전용**이거나 REST API에서 더 넓게 지원됩니다([2.3절](#23-rest-api와-http-api를-고르는-기준)).
...
API Gateway는 요청을 **단계(phase)** 로 나누어 처리하며, 애플리케이션에 구성된 기능에 따라 다음 순서로 나타납니다. 각 단계의 지연 시간을 향상된 관찰 기능 변수로 확인할 수 있습니다.
...
| 항목 | 교재의 서술 | 확인된 내용 | 근거 |
|---|---|---|---|
| 엔드포인트 유형 기본값 | "엔드포인트 유형에서는 리전 API 엔드포인트가 기본적으로 선택됩니다" | 엔드포인트 유형 문서는 엣지 최적화를 설명하면서 **"이것이 API Gateway REST API의 기본 엔드포인트 유형"** 이라고 명시하고, 개념 문서도 엣지 최적화를 "API Gateway API의 기본 호스트 이름"으로 정의합니다. 세 유형의 이름과 특성 서술은 맞습니다 | [API endpoint types](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-endpoint-types.html) |
| 메서드 권한 부여 유효값 | 유효값 `NONE`·`AWS_IAM` 두 개. "기본값은 none 또는 AWS_IAM으로 설정됩니다" | `authorizationType`은 **필수 파라미터**이고 유효값은 `NONE`·`AWS_IAM`·`CUSTOM`(Lambda 권한 부여자)·`COGNITO_USER_POOLS` **네 개**입니다. 두 값 중 하나가 기본값이라는 서술은 문서에 없습니다 | [PutMethod](https://docs.aws.amazon.com/apigateway/latest/api/API_PutMethod.html) |
| HTTP API의 Mock 통합 | HTTP API가 "Mock 및 HTTP 프록시 통합"을 지원 | 비교 문서의 통합 표는 **Mock 통합을 REST API 전용**으로 표시합니다. 나머지 항목(Lambda 프록시, AWS 서비스, VPC 프라이빗 리소스, HTTP 프록시)은 맞습니다 | [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html) |
| 요청 검증자 값 유형 | `"validateRequestBody": "false"`, `"validateRequestParameters": "true"` | 두 필드는 **Boolean** 플래그입니다. 문자열 `"false"`는 JSON에서 참으로 취급될 수 있어 의도와 다르게 동작할 위험이 있습니다 | [RequestValidator](https://docs.aws.amazon.com/apigateway/latest/api/API_RequestValidator.html) |
| 매핑 템플릿의 `$elem` | `#set`으로 `$inputRoot`를 선언하지만 쓰지 않고 `$elem`을 반복 지시자 없이 참조 | 공식 예제에서 컬렉션 순회는 항상 **`#foreach` ... `#end`** 로 표현하고 `$foreach.hasNext`로 구분자를 넣습니다. 이 템플릿을 그대로 붙여 넣으면 값이 채워지지 않습니다. 다만 AWS 문서의 매핑 템플릿 개념 페이지 예제에도 같은 표기가 있어, 문서가 이 패턴을 명시적으로 오류라고 서술한 문장은 찾지 못했습니다([9.5절](#95-검증하지-못한-항목)) | [Mapping template examples](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-mapping-variable-examples.html) |
| canary·methodSettings 값 유형 | `"percentTraffic": "10"`, `"useStageCache": "False“`, `"metricsEnabled": "true"` | `percentTraffic`은 **Double**(0.0~100.0), `useStageCache`와 `metricsEnabled`는 **Boolean** 입니다. 원본 코드는 또 닫는 따옴표가 여는 큰따옴표로 깨져 있고 마지막 줄에 닫는 중괄호가 없습니다 | [CanarySettings](https://docs.aws.amazon.com/apigateway/latest/api/API_CanarySettings.html) |
| Canary를 새 스테이지에 만든다 | "새 스테이지에 배포합니다", "새 스테이지에서 Canary를 활성화합니다". 다이어그램도 "안정적 스테이지"와 "새 canary 스테이지"를 분리 | 카나리는 별도 스테이지가 아니라 **같은 스테이지의 `canarySettings`** 입니다. 카나리를 활성화하면 비활성화하고 설정을 제거할 때까지 그 스테이지를 다른 비카나리 배포에 연결할 수 없습니다. 같은 원본 자료의 다른 노트("동일한 스테이지에서")와도 어긋납니다 | [Canary release deployment](https://docs.aws.amazon.com/apigateway/latest/developerguide/canary-release.html) |
| 스테이지 변수의 Lambda 별칭 표기 | `list_function:{$stageVariables.environment}` | 문서 표기는 `function:<함수이름>:${stageVariables.<버전변수>}` 이므로 중괄호가 달러 기호 **뒤에** 와야 합니다. 올바른 형태는 `list_function:${stageVariables.environment}` 입니다 | [Stage variables reference](https://docs.aws.amazon.com/apigateway/latest/developerguide/aws-api-gateway-stage-variables-reference.html) |
| 리소스 목록의 `/notes/list` | 한 노트는 `/notes`, `/notes/search`, `/notes/list` 세 개로 나열 | 같은 자료의 도형에는 `/notes`와 `/notes/search`만 있고 메서드 목록에도 `/notes/list`는 없습니다. 원본 내부의 자기 모순이며 어느 쪽이 옳은지는 AWS 문서로 판별할 수 없습니다. 이 문서는 도형에 맞췄습니다 | — ([9.5절](#95-검증하지-못한-항목)) |
| 통합 유형 표의 Mock 행 | '패스스루' 칸과 '통합 유형' 칸에 모두 `MOCK` | 프록시·비프록시 행의 같은 칸에는 패스스루 동작 서술이 들어 있으므로 열 의미가 어긋납니다. 원본 자체의 표기 오류입니다 | — ([9.5절](#95-검증하지-못한-항목)) |
| 검증 예 페이로드 키 | 배열 요소 키가 `Note`가 아니라 `Notes` | `NotesInputModel`은 배열 요소에 `Note`를 필수로 요구하므로 이 페이로드는 검증에서 `400`으로 실패합니다. 원본 내부의 자기 모순입니다 | — ([9.5절](#95-검증하지-못한-항목)) |
| 인용 URL의 공백 | `https://docs.aws.amazon.com/ko_kr/ apigateway/...` 처럼 URL 중간에 공백 | 여러 인용 URL의 `ko_kr/`과 `apigateway` 사이에 공백이 들어가 그대로 복사해서는 열 수 없습니다. 공백을 제거하면 11개 링크 모두 현재도 유효한 페이지를 가리킵니다 | — ([9.5절](#95-검증하지-못한-항목)) |
...
| 권한 부여자 명칭 | "Lambda 권한 부여자"(현재 이름이라 맞음) | 문서는 **"Lambda authorizer (formerly known as a custom authorizer)"** 로 표기합니다. 유형 구분(`REQUEST`·`TOKEN`), 문서가 `REQUEST`를 권장한다는 점, 메서드 권한 부여 값이 `CUSTOM`이라는 점을 함께 알아 두면 좋습니다 | [Use API Gateway Lambda authorizers](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-use-lambda-authorizer.html) |
...
| 항목 | 상태 | 대체 | 근거 |
|---|---|---|---|
| 데이터 변환의 주 경로로 매핑 템플릿 사용 | 비권장 | 문서의 권장 순서는 **① 프록시 통합 → ② 파라미터 매핑(VTL 불필요) → ③ 매핑 템플릿 변환** 입니다. 매핑 템플릿은 본문을 바꿔야 하거나 조건부 재정의가 필요하고 프록시 통합을 쓸 수 없을 때 씁니다 | [Data transformations for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/rest-api-data-transformations.html) |
| API 키로 API 액세스 통제 | 비권장 | 액세스 제어에는 **IAM 역할 · Lambda 권한 부여자 · Amazon Cognito 사용자 풀** 을 씁니다. 한 사용량 계획의 API 키 하나로 그 계획의 모든 API에 액세스할 수 있기 때문입니다. API 키는 클라이언트 식별과 사용량 계획 연결에만 씁니다 | [Usage plans and API keys](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html) |
| 사용량 계획 할당량으로 비용·액세스 통제 | 비권장 | 제한·할당량은 하드 한도가 아니라 best-effort 목표입니다. 비용 모니터링에는 **AWS Budgets**, 요청 차단에는 **AWS WAF** 를 씁니다 | [Usage plans and API keys](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html) |
| 스테이지 변수로 민감한 구성 전달 | 비권장 | 스테이지 변수는 자격 증명 같은 민감한 데이터용이 아닙니다. 민감한 데이터는 **Lambda 권한 부여자의 출력**으로 통합에 전달합니다. 민감하지 않은 구성 전달은 여전히 정당한 용도입니다 | [Use stage variables for a REST API](https://docs.aws.amazon.com/apigateway/latest/developerguide/stage-variables.html) |
| 프로덕션 스테이지에서 Data tracing 활성화 | 비권장 | 실행 로깅은 **Errors only 또는 Errors and info logs** 수준으로 쓰고 Data tracing은 문제 해결 중에만 임시로 켭니다. 민감한 데이터가 로깅될 수 있습니다 | [Set up CloudWatch logging for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-logging.html) |

### 9.4 이 자료에서 더한 점

| 항목 | 요약 | 왜 더했는가 | 근거 |
|---|---|---|---|
| REST API vs HTTP API 선택 기준 | 기능 수와 가격 기준으로 고릅니다. 이 모듈이 다루는 요청 검증·매핑 템플릿·캐싱·카나리·사용량 계획·테스트 호출·실행 로그·X-Ray는 대부분 **REST API 전용** 입니다. HTTP API 전용은 자동 배포, JWT 권한 부여자, AWS Cloud Map 프라이빗 통합입니다 | 왜 이 모듈이 REST API를 쓰는지 이해하려면 두 API의 기능 차이를 알아야 하기 때문입니다 | [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html) |
| Lambda 프록시 통합 권장 | 문서는 Lambda 프록시를 "Lambda 함수를 호출하는 데 선호되는 통합 유형"으로 강력히 권장합니다. Lambda 사용자 지정 통합은 매핑 템플릿을 여러 엔드포인트에서 재사용할 때 쓰는 고급 시나리오용입니다 | 두 Lambda 통합 중 무엇을 먼저 고려할지에 대한 현재 권장이 분명하기 때문입니다 | [Choose an API Gateway API integration type](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-integration-types.html) |
| Lambda 프록시 입력·출력 형식 | 입력 이벤트 키(`resource`·`path`·`httpMethod`·`headers`·`multiValueHeaders`·`queryStringParameters`·`requestContext`·`body`·`isBase64Encoded` 등)와 출력 형식(`isBase64Encoded`·`statusCode`·`headers`·`multiValueHeaders`·`body`). 형식이 다르면 **`502 Bad Gateway`**. `{proxy+}` + `ANY` 조합 | 형식을 지키지 않으면 502가 나므로 프록시 통합을 실제로 쓰려면 필수 지식이기 때문입니다 | [Lambda proxy integrations](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-lambda-proxy-integrations.html) |
| HTTP API 페이로드 형식 버전 | `1.0`과 `2.0`. 콘솔은 최신 버전을 기본으로 쓰지만 CLI·CloudFormation·SDK로 만들면 **필수 지정**. `2.0`은 `multiValueHeaders`가 없고 `rawPath`·`cookies`가 있으며 `statusCode` 없는 유효 JSON 응답을 추론합니다 | CLI·IaC로 만들 때 지정하지 않으면 실패하고, 두 버전의 형식이 달라 혼동하기 쉽기 때문입니다 | [Create AWS Lambda proxy integrations for HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-develop-integrations-lambda.html) |
| 파라미터 매핑 | 통합 요청·응답의 경로·쿼리 문자열·헤더 값을 **VTL 없이** 수정합니다. 본문은 수정할 수 없습니다. CORS용 정적 헤더 값에 씁니다 | 매핑 템플릿보다 먼저 고려할 변환 수단인데 잘 알려져 있지 않기 때문입니다 | [Data transformations for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/rest-api-data-transformations.html) |
| 매핑 템플릿 변수 전체 | `$input`(`body`·`json`·`path`·`params`), `$context`(52개 항목), `$stageVariables`(세 표기), `$util`(`escapeJavaScript`·`parseJson`·`urlEncode`·`urlDecode`·`base64Encode`·`base64Decode`) | 두 변수만 쓰는 예제만으로는 실제 매핑 템플릿을 작성할 수 없기 때문입니다 | [Variables for data transformations](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-mapping-template-reference.html) |
| 요청 검증의 범위 | 파라미터는 **존재만** 확인하고 유형·형식은 확인하지 않습니다. 실패 시 `400`을 반환하고 결과를 CloudWatch Logs에 게시합니다. 콘텐츠 유형과 무관하게 같은 모델을 쓰려면 `$default` | 검증이 유형까지 봐 준다고 오해하면 백엔드에서 예상치 못한 값을 만나기 때문입니다 | [Request validation for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-method-request-validation.html) |
| 모델의 확장 기능 | `enum`으로 허용 값 제한, `minimum`·`maximum`으로 범위 제약, 참조 프리미티브로 `definitions`·외부 모델 참조. API당 모델 크기 400KB | 기본 검증을 넘어 값 범위까지 제약하려면 이 확장이 필요하기 때문입니다 | [Data models for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/models-mappings-models.html) |
| WebSocket API의 제약 | 사용자 지정 경로 키에 `$` 접두사 사용 불가, 권한 부여는 `$connect`에만(값 `NONE`·`AWS_IAM`·`CUSTOM`, API 전체 적용), `$connect` 실패 시 `401`·`403`, `$disconnect`는 전달 보장 없는 best-effort | WebSocket을 설계할 때 권한 부여 위치와 `$disconnect` 신뢰성이 흔히 걸리기 때문입니다 | [Create routes for WebSocket APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/websocket-api-develop-routes.html) |
| 캐싱 세부 값 | 기본 TTL 300초 / 최대 3600초 / `TTL=0`은 비활성화, 캐시 가능한 응답 최대 1,048,576바이트, 캐시 용량 8단계(0.5~237GB), **`GET`만 기본 캐싱**, 캐시 키 필수, `Cache-Control: max-age=0`으로 무효화(교차 계정 불가), 시간당 과금 | 캐싱을 실제로 켜려면 TTL·용량·키·과금 같은 구체값이 필요하기 때문입니다 | [Cache settings for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html) |
| REST API 할당량 | 통합 시간 초과 **50ms~29초**, 페이로드 **10MB**, 유휴 연결 310초, API당 리소스 300·스테이지 10·권한 부여자 10·모델 크기 400KB, 리전 API 600·엣지 최적화 120 | 설계 단계에서 이 한도들에 부딪히기 때문입니다 | [Quotas for configuring and running a REST API](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-execution-service-limits-table.html) |
| 계정 수준 스로틀 할당량 | 계정·리전당 **10,000 RPS**, 최대 버킷 용량 **5,000 요청**. 13개 리전은 2,500 RPS / 1,250 버스트. 버스트 할당량은 고객이 조정 불가 | 스로틀링을 이해하려면 계정 수준 기본값을 알아야 하기 때문입니다 | [Amazon API Gateway quotas](https://docs.aws.amazon.com/apigateway/latest/developerguide/limits.html) |
| 실행 로깅과 액세스 로깅의 구분 | 실행 로깅은 API Gateway가 관리하고 로그 그룹 이름이 `API-Gateway-Execution-Logs_{rest-api-id}/{stage_name}`. 액세스 로깅은 개발자가 로그 그룹과 형식(CLF·JSON·XML·CSV)을 정하고 `$context.requestId`·`$context.extendedRequestId`를 포함해야 합니다. 두 로깅은 독립적으로 켭니다 | 두 로깅을 혼동하면 원하는 로그를 못 얻기 때문입니다 | [Set up CloudWatch logging for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-logging.html) |
| CloudWatch 지표 | `4XXError`·`5XXError`·`CacheHitCount`·`CacheMissCount`·`Count`·`IntegrationLatency`·`Latency` 일곱 개, 1분 주기. 메서드 수준 차원은 상세 지표를 켜야 전송되고 **추가 요금**이 발생합니다 | API를 운영하며 무엇을 볼 수 있는지, 어떤 지표가 유료인지 알아야 하기 때문입니다 | [Amazon API Gateway dimensions and metrics](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-metrics-and-dimensions.html) |
| X-Ray 추적 | REST API의 모든 엔드포인트 유형 지원, 스테이지 단위 활성화, 샘플링 규칙, 이미 추적 중인 서비스에서 호출하면 **추적 통과** | 아키텍처에 X-Ray가 등장하지만 앞 장에서 다루지 않기 때문입니다 | [Trace user requests to REST APIs using X-Ray](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-xray.html) |
| 향상된 관찰 기능 변수 | WAF → Authenticate → Authorizer → Authorize → Integration 단계별 `$context.단계.속성` 변수. Lambda 통합의 `integrationStatus`(서비스 상태)와 `status`(함수 코드 상태) 구분 | 지연 시간이 어느 단계에서 오는지 짚어 문제를 해결하는 데 쓰이기 때문입니다 | [Troubleshooting Amazon API Gateway with enhanced observability variables](https://aws.amazon.com/blogs/compute/troubleshooting-amazon-api-gateway-with-enhanced-observability-variables/) |
| AWS WAF 평가 우선순위 | AWS WAF 규칙이 리소스 정책·IAM 정책·Lambda 권한 부여자·Cognito 권한 부여자보다 **먼저** 평가됩니다. 요청 본문 검사는 첫 64KB로 제한되고 웹 ACL은 API 스테이지에 연결합니다 | 평가 순서를 모르면 리소스 정책이 허용해도 WAF에 막히는 이유를 알 수 없기 때문입니다 | [Use AWS WAF to protect your REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-control-access-aws-waf.html) |
| 프라이빗 API 세부 사항 | AWS PrivateLink 기반, Direct Connect 경유 가능, 프라이빗 DNS를 켜면 퍼블릭 API 기본 엔드포인트 액세스 불가, **TLS 1.2만 지원**, HTTP/2 요청은 HTTP/1.1로 강제 | 프라이빗 API를 쓸 때 되돌리기 어려운 제약들이기 때문입니다 | [Private REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-private-apis.html) |
| `x-amazon-apigateway-integration` 속성 | `type`(소문자 `aws_proxy` 등), `payloadFormatVersion`, `timeoutInMillis`(50~29,000ms), `responseTransferMode`(`BUFFERED`·`STREAM`), `connectionType`, `passthroughBehavior`, `contentHandling` 등 | 코드형 API 정의를 실제로 작성하려면 확장의 속성들을 알아야 하기 때문입니다 | [x-amazon-apigateway-integration object](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-swagger-extensions-integration.html) |
| Mock 통합의 다른 용도 | 랜딩 페이지 프로비저닝, CORS 헤더 반환(콘솔은 `OPTIONS` 메서드를 Mock 통합으로 구성), 게이트웨이 응답. **큰 응답 템플릿에는 부적합**하므로 그때는 Lambda 통합을 씁니다 | Mock 통합이 테스트 용도만은 아니라는 점과 그 한계를 알아야 하기 때문입니다 | [Mock integrations for REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-mock-integration.html) |
| 개발자 포털과 Quick create | 개발자 포털(API 제공자가 API와 문서를 공유하며 API를 제품으로 묶는 애플리케이션, REST API 전용)과 Quick create(Lambda·HTTP 통합, 기본 catch-all 경로, 자동 배포 스테이지로 HTTP API를 간단히 생성) | 개념 문서가 제시하는 이 두 기능이 API 운영·생성 흐름을 넓혀 주기 때문입니다 | [Amazon API Gateway concepts](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-basic-concept.html) |
...
| 항목 | 상태 |
|---|---|
| "HTTP API가 REST API보다 대기 시간이 짧다" | 두 API를 비교하는 공식 문서가 제시하는 축은 **기능 수와 가격**이고 대기 시간은 비교 항목으로 나오지 않습니다. 비용이 더 낮다는 서술은 문서로 확인했습니다("designed with minimal features so that they can be offered at a lower price"). **대기 시간이 더 짧다는 서술을 뒷받침하는 문장은 이 문서에서 찾지 못했습니다.** 교재가 틀렸다고 단정하지 않고, 확인하지 못했다고만 적습니다 |
| 콘솔이 엔드포인트 유형으로 무엇을 미리 선택하는가 | 문서로 확인한 것은 **엣지 최적화가 REST API의 기본 엔드포인트 유형**이라는 서술입니다. "콘솔의 엔드포인트 유형에서 리전이 기본 선택된다"는 콘솔 UI 설명도 있지만, **현재 콘솔이 어느 항목을 미리 선택해 두는지는 문서에서 확인하지 못했습니다.** 실무에서는 엔드포인트 유형을 명시적으로 고르세요 |
| 매핑 템플릿의 `$elem` 표기 | 공식 예제에서 컬렉션 순회가 `#foreach` ... `#end` 로 표현된다는 것은 확인했습니다. 그런데 **AWS 문서의 매핑 템플릿 개념 페이지 예제에도 `#foreach` 없이 `$elem`을 쓰는 표기가 있어**, 문서가 이 패턴을 명시적으로 오류라고 서술한 문장은 찾지 못했습니다. 이 문서는 동작하는 형태로 교정했지만 "문서가 금지한다"고 말할 근거는 없습니다 |
...
| 리소스 목록의 `/notes/list` | 원본 자료 안에서 노트와 도형·메서드 목록이 어긋납니다. 어느 쪽이 이 애플리케이션의 올바른 설계인지는 **AWS 문서로 판별할 수 없습니다.** 이 문서는 도형과 메서드 목록의 다수 근거를 따랐습니다 |
| 통합 유형 표 Mock 행의 패스스루 칸 | 열 의미가 어긋난 표기 오류입니다. AWS 문서로 검증할 성질의 사실이 아니라 원본 자체의 오류이므로 칸만 교정하고 근거 인용은 붙이지 않았습니다 |
| 검증 예 페이로드의 `Notes` 키 | 같은 이유로 원본 내부 불일치입니다. `NotesInputModel`에 맞춰 `Note`로 교정했습니다 |
| 카나리 코드 예의 깨진 따옴표와 닫히지 않은 중괄호 | 값 유형(Double·Boolean)은 API 참조로 확인했지만 따옴표 문자 깨짐과 중괄호 누락은 문서 검증 대상이 아니라 원본의 표기 오류입니다 |
| WebSocket 설명의 한국어 번역 파손 | "지속적인 지속적인 의 영구 연결", "경로을 호출합될 때"처럼 문장이 중복·파손된 원문이 있었습니다. **내용 오류가 아니라 번역 품질 문제**이므로 의미가 통하는 범위로만 요약했습니다 |
| 실습 5 워크플로 | 원본 자료에 다이어그램 요소(Amazon API Gateway, AWS 클라우드, 사용자, DynamoDB table)만 있고 설명 텍스트가 없습니다. 요약할 원문이 없어 이 문서에서는 다루지 않았습니다 |
| 콘솔 스크린샷 절차 | 원본 자료에 이미지만 있고 텍스트가 없었습니다. 콘솔 절차는 현재 공식 문서 기준으로 다시 썼습니다([6.2절](#62-api-gateway-콘솔을-사용한-테스트)) |
