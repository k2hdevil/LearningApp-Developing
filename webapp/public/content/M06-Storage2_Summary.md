...
교재 슬라이드 42의 문제를 그대로 싣습니다.

**문제 1**: 크기가 100MB를 초과하는 객체에는 멀티파트 업로드를 사용하는 것을 고려합니다.

- ✅ **정답: 참** — 100MB보다 큰 객체에 멀티파트 업로드를 사용하는 것이 모범 사례입니다. 단일 PUT 메서드에서 업로드할 수 있는 최대 객체가 5GB인 경우에도 참입니다.

**문제 2**: 일부 서비스에는 서비스가 반환한 모든 데이터에 액세스하기 위해 페이지 배열이 필요한 API가 있습니다.

- ✅ **정답: 참** — 버킷의 콘텐츠를 나열할 때 페이지를 배열한 결과의 예를 검토했습니다.

**문제 3**: 미리 서명된 URL을 사용하면 시간 제한 액세스를 통해 특정 S3 객체를 공유할 수 있습니다.

- ✅ **정답: 참** — 미리 서명된 URL을 사용하면 사용자에게 특정 S3 객체에 대한 임시 액세스 권한을 부여할 수 있습니다.

**문제 4**: S3 배치 작업을 사용하여 단일 객체를 일련의 파트로 업로드하여 처리량을 개선합니다.

- ❌ **정답: 거짓** — 단일 객체를 일련의 파트로 업로드하여 처리량을 개선하는 데에는 S3 멀티파트 업로드를 사용합니다. S3 배치 작업은 객체 메타데이터 및 속성을 변경하거나 버킷 간 객체 복사 또는 복제와 같은 기타 스토리지 관리 태스크를 수행하는 데 사용합니다.

**문제 5**: Amazon S3 이벤트 알림은 항상 PUT, POST, COPY 또는 DELETE와 같은 Amazon S3 작업에 대한 응답으로 보내집니다.

- ❌ **정답: 거짓** — PUT, POST, COPY 또는 DELETE와 같은 Amazon S3 작업에 대한 응답으로 Amazon S3 이벤트 알림을 보내도록 버킷을 구성할 수 있습니다. 🔄 다만 현재 지원 트리거는 API 호출에 한정되지 않습니다([6.6절](#66-이벤트-알림-트리거)).

**문제 6**: 웹 서버는 CORS를 사용하여 웹 사이트에 액세스하는 동일한 도메인 내에 저장된 리소스의 로드를 허용 또는 거부합니다.

- ❌ **정답: 거짓** — CORS는 다른 도메인에서 액세스할 수 없도록 데이터를 보호합니다. CORS에서 오리진은 웹 서버이고 리소스는 다른 도메인의 자산입니다. CORS에서 웹 서버는 브라우저가 리소스 로드를 허용해야 하는 자신이 아닌 다른 리소스(도메인, 체계 또는 포트)를 나타냅니다.

> — 출처: [이벤트 유형과 대상](https://docs.aws.amazon.com/AmazonS3/latest/userguide/notification-how-to-event-types-and-destinations.html)

### 🆕 보충 문제 (최신화 내용 확인)

**문제 7**: AWS 계정 하나에서 기본적으로 만들 수 있는 S3 버킷은 최대 100개이고, 요청하면 1,000개까지 늘릴 수 있습니다.

- ❌ **정답: 거짓** — 현재 기본 한도는 계정당 범용 버킷 **10,000개**이며, 그 이상은 Service Quotas 콘솔에서 증가를 요청합니다. ([2.2절](#22-버킷-생성과-계정당-버킷-한도))

> — 출처: [버킷 할당량과 제한](https://docs.aws.amazon.com/AmazonS3/latest/userguide/BucketRestrictions.html)

**문제 8**: `HeadBucket`이 400을 반환하면 버킷이 존재하는 리전이 아닌 다른 리전에서 접근했다는 뜻입니다.

- ❌ **정답: 거짓** — 400은 버킷이 없거나 권한이 없을 때 반환되는 일반 코드(400·403·404) 중 하나입니다. 오히려 파티션 내 어떤 리전의 버킷 이름으로 호출해도 올바른 버킷 위치 헤더를 받습니다. ([2.3절](#23-headbucket-api-작업))

> — 출처: [HeadBucket API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_HeadBucket.html)

**문제 9**: `aws s3api get-bucket-versioning`으로 버전 관리를 한 번도 설정하지 않은 버킷을 조회하면 `"Status": "Disabled"`가 반환됩니다.

- ❌ **정답: 거짓** — 버전 관리 상태 값은 `Enabled`와 `Suspended` 두 가지뿐이고, 한 번도 설정하지 않은 버킷은 상태 값 자체가 반환되지 않습니다. ([2.9절](#29-버킷-버전-관리-업데이트))

> — 출처: [PutBucketVersioning API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutBucketVersioning.html)

**문제 10**: 멀티파트 업로드의 파트는 크기가 5MiB 이상이어야 하며, 이 조건은 마지막 파트에도 적용됩니다.

- ❌ **정답: 거짓** — 파트 크기는 5MiB~5GiB이지만 **마지막 파트에는 최소 크기 제한이 없습니다.** 업로드당 최대 파트 수는 10,000개입니다. ([3.3절](#33-멀티파트-업로드-한도))

> — 출처: [멀티파트 업로드 한도](https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html)

**문제 11**: 미리 서명된 URL의 만료 기간을 7일로 지정하면 어떤 자격 증명으로 서명했든 7일 동안 유효합니다.

- ❌ **정답: 거짓** — 임시 보안 인증으로 만든 URL은 **그 자격 증명이 만료될 때 함께 만료됩니다.** STS `AssumeRole` 세션은 기본 1시간, EC2 인스턴스 프로파일 메타데이터 자격 증명은 최대 약 6시간입니다. 또한 URL의 기능은 그것을 만든 사용자의 권한으로 제한됩니다. ([4.2절](#42-만료-기간과-자격-증명-유형))

> — 출처: [미리 서명된 URL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-presigned-url.html)

**문제 12**: `aws s3 presign`으로 업로드용(PUT) 미리 서명된 URL을 만들 수 있습니다.

- ❌ **정답: 거짓** — `aws s3 presign`이 만드는 URL은 **HTTP GET 요청으로 객체를 가져오는 용도**입니다. PUT 업로드 URL은 SDK로 만들어야 합니다(예: boto3 `generate_presigned_url(ClientMethod='put_object')`). ([4.3절](#43-aws-cli로-미리-서명된-url-생성))

> — 출처: [aws s3 presign](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/presign.html)

**문제 13**: S3 정적 웹 사이트 엔드포인트는 HTTPS로 접근할 수 있습니다.

- ❌ **정답: 거짓** — 웹 사이트 엔드포인트는 HTTPS와 액세스 포인트를 지원하지 않고 스킴은 `http`입니다. HTTPS가 필요하면 AWS Amplify Hosting(권장) 또는 CloudFront를 사용합니다. ([6.2절](#62-웹-사이트-엔드포인트)) 교재 슬라이드 38의 표에 `https://` 로 적혀 있는 것은 같은 슬라이드의 강사 노트와도 모순되는 표기 오류입니다.

> — 출처: [웹 사이트 엔드포인트](https://docs.aws.amazon.com/AmazonS3/latest/userguide/WebsiteEndpoints.html)

**문제 14**: S3 콘솔에서 CORS를 구성할 때는 XML 파일을 작성합니다.

- ❌ **정답: 거짓** — 현재 콘솔은 **JSON CORS 구성만 지원합니다.** REST API·SDK 경로에서는 JSON과 XML을 모두 쓸 수 있습니다. ([6.5절](#65-교차-오리진-리소스-공유-cors))

> — 출처: [CORS 구성 요소](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ManageCorsUsing.html)

**문제 15**: S3 배치 작업의 대상 객체 목록은 S3 인벤토리 보고서 또는 직접 만든 CSV 파일로만 지정할 수 있습니다.

- ❌ **정답: 거짓** — 현재는 네 가지입니다. 기존 매니페스트 사용, 새 매니페스트 직접 작성, **지정한 메타데이터 기준 자동 생성**, **기존 복제 구성 기준 자동 생성**. ([5.6절](#56-s3-배치-작업))

> — 출처: [배치 작업 생성](https://docs.aws.amazon.com/AmazonS3/latest/userguide/batch-ops-create-job.html)

**문제 16**: 수명 주기 규칙으로 객체가 만료되어 삭제되면 `s3:ObjectRemoved:*` 이벤트 알림이 발생합니다.

- ❌ **정답: 거짓** — `ObjectRemoved` 이벤트 알림은 **수명 주기 구성에 의한 자동 삭제나 실패한 작업에 대해서는 알리지 않습니다.** 수명 주기 삭제를 잡으려면 `s3:LifecycleExpiration:*` 을 사용합니다. ([6.6절](#66-이벤트-알림-트리거))
...
- ✅ 주요 버킷 및 객체 작업 수행
- ✅ 대용량 파일 및 대량 파일을 처리하는 방법 설명
- ✅ 정적 웹 사이트를 호스트하기 위한 S3 버킷 생성 및 구성
- ✅ 객체에 대한 임시 액세스 권한을 부여하는 방법 설명
