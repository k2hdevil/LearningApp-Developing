...
The questions from courseware slide 50 are reproduced as they appear.

**Question 1**: Key design concepts for a NoSQL database include size, shape, and velocity.

- ✅ **Answer: True**

**Question 2**: You design the partition key around common access patterns and the level of uniqueness of items within the table.

- ✅ **Answer: True**

**Question 3**: A developer should set a table's capacity mode to provisioned when they expect traffic to be inconsistent.

- ❌ **Answer: False** — On-demand mode is best suited to inconsistent traffic.

**Question 4**: By default, DynamoDB write operations (PutItem, UpdateItem, DeleteItem) are conditional.

- ❌ **Answer: False** — By default, DynamoDB write operations (PutItem, UpdateItem, DeleteItem) are unconditional.

**Question 5**: BatchWriteItem cannot update items. To update an item, use the UpdateItem operation.

- ✅ **Answer: True** — Performing `BatchWriteItem` on an existing item overwrites the values so it looks as though it was updated, but the documentation recommends `UpdateItem` to update items.

**Question 6**: By design, a table scan is more efficient than a query.

- ❌ **Answer: False** — A table scan is less efficient than a query, so it should be used minimally.

> — Source: [BatchWriteItem API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_BatchWriteItem.html)

### 🆕 Supplementary Questions (Checking the Updated Material)

**Question 7**: You can switch read and write capacity mode once every 24 hours.

- ❌ **Answer: False** — Provisioned → on-demand is limited to **four times in a 24-hour rolling window**, and on-demand → provisioned can be done **at any time**. ([Section 2.5](#25-choosing-initial-throughput))

> — Source: [Considerations when switching capacity modes](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-switching-capacity-modes.html)

**Question 8**: Provisioned throughput can be decreased at most four times per day.

- ❌ **Answer: False** — A day (UTC) starts with 4 available decreases and **1 is added each hour**, so up to **27** decreases are possible over 24 hours. The maximum available at any one time is 4. Increases can be made as often as necessary. ([Section 3.8](#38-throughput-increase-and-decrease-limits))

> — Source: [DynamoDB quotas — Increasing or decreasing throughput](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ServiceQuotas.html)

**Question 9**: When you create a table with the AWS CLI or an SDK, you must provision read and write capacity.

- ❌ **Answer: False** — `BillingMode` on `CreateTable` is not required, and if you choose `PAY_PER_REQUEST` (on-demand) you **cannot** specify `ProvisionedThroughput`. The documentation describes on-demand as the default and recommended throughput option. In practice, stating `BillingMode` explicitly is safer (see [Section 9.5](#95-items-we-could-not-verify)). ([Section 2.5](#25-choosing-initial-throughput))

> — Source: [On-demand capacity mode](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/on-demand-capacity-mode.html)

**Question 10**: A query that returns one 2 KB item consumes 1 RCU.

- ❌ **Answer: False** — 1 RCU is the **strongly consistent** read figure. With the default eventually consistent read used by `Query`, `GetItem`, and `Scan` it is **0.5 RCU**, and a transactional read is 2 RCU. Item sizes for reads are rounded up to the next 4 KB multiple. ([Section 2.5](#25-choosing-initial-throughput))

> — Source: [DynamoDB read and write operations](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/read-write-operations.html)

**Question 11**: Using a filter expression reduces the number of items a `Query` reads, so it also reduces the read capacity consumed.

- ❌ **Answer: False** — DynamoDB calculates capacity from **item size**, not the amount of data returned, and **consumed capacity is the same** whether or not a `FilterExpression` is used. The same applies to `Scan`. What reduces the amount read is `KeyConditionExpression`, `Limit`, and secondary indexes. ([Section 5.2](#52-querying-data-query))

> — Source: [Query API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_Query.html)

**Question 12**: If the response contains a `LastEvaluatedKey`, more items matching the condition remain.

- ❌ **Answer: False** — A non-empty `LastEvaluatedKey` only means the previous request **stopped at a page boundary** (1 MB or `Limit`). With a filter, a page with zero matching items can still include a `LastEvaluatedKey`. The only way to know you have reached the end is when `LastEvaluatedKey` is empty. ([Section 5.3](#53-paginating-results))

> — Source: [Paginating table query results](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Query.Pagination.html)

**Question 13**: `--condition-expression "Favorite NOT yes"` is a valid condition expression meaning "when `Favorite` is not `yes`."

- ❌ **Answer: False** — `NOT` is a **logical operator that negates a single condition** and cannot be used as a binary comparison, and a value such as `yes` cannot be a literal and must be passed as an expression attribute value. The correct form is `"attribute_not_exists(Favorite) OR Favorite <> :fav"` with the `:fav` value supplied through `--expression-attribute-values`. ([Section 6.2](#62-conditional-write-operations))

> — Source: [Condition and filter expressions, operators, and functions](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.OperatorsAndFunctions.html)

**Question 14**: When a conditional write fails because the condition is not met, no write capacity is consumed.

- ❌ **Answer: False** — DynamoDB **consumes write capacity units** even when the condition evaluates to `false`. The amount depends on the size of the existing item or of the new item you are trying to create or update. A failed conditional write returns `ConditionalCheckFailedException` and the response carries no consumed-capacity information, so check the `ConsumedWriteCapacityUnits` metric in CloudWatch. ([Section 6.2](#62-conditional-write-operations))

> — Source: [DynamoDB read and write operations](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/read-write-operations.html)

**Question 15**: Increasing `TotalSegments` on a parallel scan always makes the scan faster.

- ❌ **Answer: False** — DynamoDB assigns segments by **partition key hash**, so all items with the same partition key always land in the same segment. Segment distribution can be uneven, so increasing the number of segments **does not guarantee** better performance. A large number of workers also makes it easy to consume all provisioned throughput. ([Section 5.6](#56-parallel-scan))

> — Source: [Scan documentation — Parallel scan](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html)

**Question 16**: Global secondary indexes can also perform strongly consistent reads with `ConsistentRead=true`.

- ❌ **Answer: False** — GSIs support **eventually consistent reads only.** Querying a GSI with `ConsistentRead=true` raises a `ValidationException`. Tables and LSIs can perform strongly consistent reads. A table can have up to 5 LSIs and a default quota of 20 GSIs. ([Section 2.4](#24-index-design-and-secondary-index-quotas))

> — Source: [Query API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_Query.html)

**Question 17**: You cannot use an empty string as an attribute value when putting an item with `PutItem`.

- ❌ **Answer: False** — **Empty string and empty binary values are allowed** on non-key attributes. Only string and binary values used as key attributes for a table or index must have a length greater than zero. Set type attributes cannot be empty. ([Section 4.1](#41-creating-an-item-putitem))

> — Source: [PutItem API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_PutItem.html)

**Question 18**: Because `ListTables` requires no parameters, one call returns every table name in the account.

- ❌ **Answer: False** — It is true that no parameters are required, but the output is **paginated at a maximum of 100 names per page.** To retrieve everything, pass the response's `LastEvaluatedTableName` as the next request's `ExclusiveStartTableName`, or use an SDK paginator. ([Section 3.9](#39-listtables-is-paginated))

> — Source: [ListTables API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_ListTables.html)

**Question 19**: Use `BatchWriteItem` when you need an all-or-nothing write across multiple items.

- ❌ **Answer: False** — With `BatchWriteItem`, **only some operations might succeed.** Failed operations are returned in `UnprocessedItems` and must be retried with exponential backoff. For all-or-nothing behavior, use **`TransactWriteItems`** (up to 100 actions, 100 distinct items, 4 MB total). Note that transactions cannot be performed using indexes. ([Section 7.5](#75-two-interfaces-not-in-the-courseware-partiql-and-transactions))

> — Source: [DynamoDB transactions](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/transaction-apis.html)

**Question 20**: A partition key attribute can also use a document type such as a list or a map.

- ❌ **Answer: False** — Primary key attributes must be **scalar**, and the only data types allowed are **string, number, and binary.** Non-key attributes have no such restriction. ([Section 2.3](#23-partition-key-design))

> — Source: [Core components of Amazon DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.CoreComponents.html)

**Question 21**: When you send a strongly consistent read to a DAX cluster, DAX caches the result and serves the next request from the cache.

- ❌ **Answer: False** — DAX **passes strongly consistent reads through to DynamoDB and does not cache the results.** DAX exists to accelerate access to eventually consistent data, and the first case where DAX is not ideal is an application that requires strongly consistent reads. ([Section 8.3](#83-requests-dax-handles))

> — Source: [DAX: How it works](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DAX.concepts.html)

**Question 22**: You can create a table by calling `CreateTable` through the DAX client.

- ❌ **Answer: False** — **DAX does not recognize table management operations such as `CreateTable` and `UpdateTable`.** If you need those operations, access DynamoDB directly rather than using DAX. ([Section 8.3](#83-requests-dax-handles))

> — Source: [DAX: How it works](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DAX.concepts.html)

**Question 23**: A production DAX cluster is fault-tolerant with a single node.

- ❌ **Answer: False** — For production, **at least three nodes** placed in different Availability Zones are strongly recommended, and three nodes are required for fault tolerance. One- and two-node clusters are for development and test and are not fault-tolerant. A cluster supports up to 11 nodes (1 primary + 10 read replicas). ([Section 8.5](#85-dax-cluster-configuration))

> — Source: [DAX cluster components](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DAX.concepts.cluster.html)

**Question 24**: Amazon ElastiCache supports only two engines, Memcached and Redis.

- ❌ **Answer: False** — ElastiCache works with **Valkey, Memcached, and Redis OSS**, and offers a **serverless cache** format in addition to node-based clusters. ([Section 8.1](#81-dynamodb-caching-options))
...
|---|---|
| Design | Do not create a table until every access pattern is identified. Partition keys need high cardinality plus common access patterns |
| Keys | Primary key attributes are scalar and can only be string, number, or binary. 5 LSIs / 20 GSIs by default, and GSIs are eventually consistent only |
| Capacity | On-demand is the default and recommended option. A strongly consistent read is 1 RCU, an eventually consistent read 0.5 RCU, a write 1 WCU per 1 KB. Decreases are capped at 27 per 24 hours |
| Writes | Write operations are unconditional by default. `PutItem` replaces completely, `UpdateItem` upserts and is billed on the larger of the before and after sizes. Failed conditions still consume capacity |
| Reads | `Query` requires a single partition key value; `Scan` reads everything. Both page at 1 MB. **A filter does not reduce consumed capacity** |
| Batch and transactions | `BatchWriteItem` 25 items/16 MB (no updates, partial failure possible), `BatchGetItem` 100 items/16 MB, `TransactWriteItems` 100 actions/4 MB (all-or-nothing, no indexes) |
| Higher-level | Java uses the enhanced client (`@DynamoDbBean`, `@DynamoDbPartitionKey`, `@DynamoDbSortKey`), .NET the object persistence and document models (neither can create tables), Python the client interface |
| Caching | DAX accelerates eventually consistent reads. Five write-through operations, strongly consistent reads pass through uncached, table management operations unrecognized. At least three nodes in production |
