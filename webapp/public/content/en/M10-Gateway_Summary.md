...
| Demo / knowledge check / Lab 5 / summary | 37–45 | [Section 7.7](#77-demo-topics) · [Chapter 10](#10-knowledge-check-and-summary) |
...
These are the questions from courseware slide 40, as written.

**Question 1**: With Amazon API Gateway, developers can create, publish, maintain, monitor, and secure APIs.

- ✅ **Answer: True**

**Question 2**: In a canary release deployment, you partially deploy a new software feature and shift a percentage of API traffic to the new version of the application.

- ✅ **Answer: True**

**Question 3**: Mock integrations only respond with a 200 status code for API methods.

- ❌ **Answer: False** — The API developer configures the status code and determines how API Gateway responds to a mock integration request.

**Question 4**: A resource is a logical entity that an application can access through a resource path.

- ✅ **Answer: True**

**Question 5**: Stage variables are name-value pairs that you can define as configuration attributes associated with a deployment stage of a REST API.

- ✅ **Answer: True**

**Question 6**: To intelligently handle a list of API calls with varying functions, you can use AWS Lambda functions as the CRUD backend.
...
**Question 7**: The default endpoint type for an API Gateway REST API is Regional.

- ❌ **Answer: False** — The endpoint types page, describing edge-optimized endpoints, states **"This is the default endpoint type for API Gateway REST APIs."** If most of your clients are in the same Region, select the Regional type explicitly. ([Section 3.3](#33-endpoint-types-and-the-default))

> — Source: [API endpoint types for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-endpoint-types.html)

**Question 8**: There are two values available for method authorization: `NONE` and `AWS_IAM`.

- ❌ **Answer: False** — `authorizationType` is a **required parameter** and there are **four** valid values: `NONE`, `AWS_IAM`, `CUSTOM` (Lambda authorizer), and `COGNITO_USER_POOLS`. The courseware list omits Lambda authorizers and Cognito user pools. ([Section 3.4](#34-valid-method-authorization-values))

> — Source: [PutMethod](https://docs.aws.amazon.com/apigateway/latest/api/API_PutMethod.html)

**Question 9**: HTTP APIs also support mock integrations, so you can produce responses without a backend.

- ❌ **Answer: False** — The integrations table in the REST versus HTTP comparison document marks **mock integrations as REST API only.** Test invocations, caching, request validation, canary release deployments, request body transformation, execution logs, and X-Ray tracing are also REST API only. ([Section 2.3](#23-choosing-between-rest-apis-and-http-apis))

> — Source: [Choose between REST APIs and HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html)

**Question 10**: When integrating with a Lambda function, the documentation treats Lambda proxy integration and Lambda custom integration as equivalent choices.

- ❌ **Answer: False** — The documentation highly recommends Lambda proxy as **"the preferred integration type to call a Lambda function through API Gateway,"** because the setup is simple and can evolve with the backend without tearing down the existing setup. Lambda custom integration is for the advanced scenario of reusing mapping templates across endpoints. ([Section 3.5](#35-integration-types))

> — Source: [Choose an API Gateway API integration type](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-integration-types.html)

**Question 11**: In a Lambda proxy integration, whatever JSON the function returns is passed through to the client as-is.

- ❌ **Answer: False** — A REST API Lambda proxy integration requires the `isBase64Encoded`, `statusCode`, `headers`, `multiValueHeaders`, and `body` format, and **a different format returns `502 Bad Gateway`.** Only HTTP API payload format `2.0` infers a 200 response from valid JSON without a `statusCode`. ([Section 3.6](#36-lambda-proxy-integration-input-and-output-formats))

> — Source: [Lambda proxy integrations in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-lambda-proxy-integrations.html)

**Question 12**: Integration passthrough behavior is determined by two conditions.

- ❌ **Answer: False** — It is a **setting** with three options: `WHEN_NO_MATCH`, `WHEN_NO_TEMPLATES` (**recommended**), and `NEVER`. The latter two reject unmapped content types with **`415 Unsupported Media Type`**. The courseware statement that passthrough behavior cannot be modified for proxy integrations is correct. ([Section 3.7](#37-integration-passthrough-behavior))

> — Source: [Method request behavior for payloads without mapping templates for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/integration-passthrough-behaviors.html)

**Question 13**: When you need to transform data, using a mapping template is the documentation's first recommendation.

- ❌ **Answer: False** — The recommended order is **① proxy integration → ② parameter mapping (no VTL) → ③ mapping template transformations.** Parameter mapping modifies path, query string, and header values but **cannot modify the body.** Use a mapping template when you need to change the body or perform conditional overrides and cannot use a proxy integration. ([Section 4.2](#42-three-choices-for-data-transformation))

> — Source: [Data transformations for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/rest-api-data-transformations.html)

**Question 14**: When request validation is enabled, the type and format of required request parameters are also checked.

- ❌ **Answer: False** — API Gateway **only checks the existence of a parameter and does not check the type or format.** The payload is validated against the JSON schema configured for the content type, and if no matching content type is found, validation is not performed. Failure returns `400` and publishes the results to CloudWatch Logs. ([Section 4.4](#44-request-validation))

> — Source: [Request validation for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-method-request-validation.html)

**Question 15**: In a request validator configuration, `validateRequestBody` and `validateRequestParameters` are strings.

- ❌ **Answer: False** — Both fields are **Boolean** flags. The `"false"` and `"true"` notation on courseware slide 18 should be written without quotes as `false` and `true`. The string `"false"` can be treated as truthy in JSON. ([Section 4.4](#44-request-validation))

> — Source: [RequestValidator](https://docs.aws.amazon.com/apigateway/latest/api/API_RequestValidator.html)

**Question 16**: To iterate an array in a mapping template you can simply use the `$elem` variable.

- ❌ **Answer: False** — In the official examples, collection iteration is expressed with **`#foreach($elem in $inputRoot.Notes)` ... `#end`** and commas are inserted with `$foreach.hasNext`. A variable referenced without an iteration directive is not filled in. Note that AWS documentation examples contain the same notation, so the documentation does not declare it an error ([Section 9.5](#95-items-we-could-not-verify)). ([Section 4.5](#45-mapping-templates-request-direction))

> — Source: [Examples using variables for mapping template transformations for API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-mapping-variable-examples.html)

**Question 17**: To create a canary release you create a new stage and enable the canary on that stage.

- ❌ **Answer: False** — The canary is not a separate stage but **`canarySettings` on the same stage where the production release is deployed.** The stage is associated with the initial deployment and the canary with subsequent deployments. Once the canary is enabled, that stage cannot be associated with another non-canary deployment until the canary is disabled and the settings removed. ([Section 7.4](#74-canary-release-steps))

> — Source: [Set up an API Gateway canary release deployment](https://docs.aws.amazon.com/apigateway/latest/developerguide/canary-release.html)

**Question 18**: Promoting a canary disables the canary and returns the deployment to a regular production deployment.

- ❌ **Answer: False** — **Promoting a canary release does not disable the canary on the stage.** Promotion copies the deployment ID and stage variables to the stage and sets `percentTraffic` to **0.0**; to return to a regular production release deployment you must remove `canarySettings`. ([Section 7.3](#73-canary-releases))

> — Source: [Promote a canary release](https://docs.aws.amazon.com/apigateway/latest/developerguide/promote-canary-deployment.html)

**Question 19**: When you enable API caching, all methods on the stage are cached.

- ❌ **Answer: False** — For API safety and availability, **only `GET` methods** have caching enabled by default; you enable caching for other methods by overriding method settings. The default TTL is 300 seconds, the maximum is 3600, and `TTL=0` disables caching. The maximum cacheable response size is 1,048,576 bytes. ([Section 7.5](#75-response-caching))

> — Source: [Cache settings for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html)

**Question 20**: Increasing the cache capacity preserves the existing cached data.

- ❌ **Answer: False** — Changing the capacity makes API Gateway **remove the existing cache instance and create a new one, so all existing cached data is deleted.** Creating or deleting a cache takes about 4 minutes. ([Section 7.5](#75-response-caching))

> — Source: [Cache settings for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html)

**Question 21**: API Gateway has two throttling settings: server-side throttling and per-client throttling.

- ❌ **Answer: False** — There are **four**: AWS throttling limits, per-account limits, per-API and per-stage limits, and per-client limits. They apply in the order per-client and per-method → stage per-method → account level → AWS Regional. The default throttle quota per account per Region is 10,000 RPS with a maximum bucket capacity of 5,000 requests. ([Section 7.6](#76-throttling-and-usage-plans))

> — Source: [Throttle requests to your REST APIs for better throughput in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-request-throttling.html)

**Question 22**: Requiring an API key lets you authenticate and authorize access to your API.

- ❌ **Answer: False** — The documented best practice is **"Don't use API keys for authentication or authorization to control access to your APIs."** If you have multiple APIs in a usage plan, a user with a valid API key for one of them can access all of them. Use IAM roles, Lambda authorizers, or Amazon Cognito user pools for access control. ([Section 7.6](#76-throttling-and-usage-plans))

> — Source: [Usage plans and API keys for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html)

**Question 23**: Setting a usage plan quota guarantees a ceiling on your API costs.

- ❌ **Answer: False** — Throttles and quotas are not hard limits and are **applied on a best-effort basis, so clients can exceed them.** The documentation advises not relying on them to control costs or block access, and to consider **AWS Budgets** to monitor costs and **AWS WAF** to manage requests. ([Section 7.6](#76-throttling-and-usage-plans))

> — Source: [Usage plans and API keys for REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-api-usage-plans.html)

**Question 24**: The integration timeout for a Lambda integration can be raised to the Lambda function's maximum execution time.

- ❌ **Answer: False** — The default integration timeout range for a REST API is **50 milliseconds to 29 seconds** for all integration types. Regional and private APIs can be raised above 29 seconds but this might require reducing the Region-level throttle quota for the account, and **edge-optimized APIs cannot be raised at all.** The 10 MB payload size limit also cannot be increased. ([Section 3.8](#38-rest-api-quotas))

> — Source: [Quotas for configuring and running a REST API in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-execution-service-limits-table.html)

**Question 25**: In a WebSocket API you can configure authorization differently for each route.

- ❌ **Answer: False** — Because a WebSocket connection is stateful, authorization can be configured **on the `$connect` route only**, and authentication and authorization are performed only at connection time. The setting applies to **the entire API**, not just the `$connect` route. `$connect` is what protects the other routes. ([Section 2.4](#24-working-with-websocket-apis))

> — Source: [Create routes for WebSocket APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/websocket-api-develop-routes.html)

**Question 26**: Calling a method with the API Gateway console's Test feature does not change real resources.

- ❌ **Answer: False** — **Testing a method with the console is the same as calling the method outside of the console.** If you use the console to call a method that deletes an API resource and the call succeeds, that resource really is deleted. Changes may be impossible to undo. The Logs in the result are simulated but the call results are real. ([Section 6.2](#62-testing-with-the-api-gateway-console))

> — Source: [Use the API Gateway console to test a REST API method](https://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-test-method.html)

**Question 27**: API Gateway can import only Swagger 2.0 definition files.

- ❌ **Answer: False** — It supports **OpenAPI v2.0 (equivalent to Swagger 2.0) and OpenAPI v3.0.** The export paths split into `exports/oas30` for OpenAPI 3.0 and `exports/swagger` for OpenAPI 2.0. The documentation titles this section "OpenAPI extensions for API Gateway." ([Section 5.1](#51-from-swagger-to-openapi))

> — Source: [Develop REST APIs using OpenAPI in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-import-api.html)

**Question 28**: When you attach AWS WAF to an API, the resource policy is evaluated first and the WAF rules second.

- ❌ **Answer: False** — **AWS WAF rules are evaluated before resource policies, IAM policies, Lambda authorizers, and Amazon Cognito authorizers.** If AWS WAF blocks a CIDR block that a resource policy allows, AWS WAF takes precedence and the resource policy is not evaluated. The web ACL is associated with an API stage. ([Section 2.7](#27-private-rest-apis-and-aws-waf))

> — Source: [Use AWS WAF to protect your REST APIs in API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-control-access-aws-waf.html)

**Question 29**: Method-level CloudWatch metrics are sent by default and incur no additional charge.

- ❌ **Answer: False** — Metrics for the `ApiName, Method, Resource, Stage` dimension **are not sent unless you explicitly enable detailed CloudWatch metrics**, and enabling them **incurs additional charges** to your account. API-level and stage-level metrics are not charged. ([Section 8.2](#82-cloudwatch-metrics))

> — Source: [Amazon API Gateway dimensions and metrics](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-metrics-and-dimensions.html)

**Question 30**: A private REST API supports every TLS version from 1.0 upward.

- ❌ **Answer: False** — Private APIs support **only TLS 1.2** and do not support earlier versions. HTTP/2 requests are enforced to HTTP/1.1, and only the dualstack IP address type is supported. You also cannot convert a private API to an edge-optimized API. ([Section 2.7](#27-private-rest-apis-and-aws-waf))
...
|---|---|
| API types | REST / HTTP / WebSocket. **Choose on features and price.** Request validation, mapping templates, caching, canary, usage plans, test invocations, execution logs, X-Ray, and mock integrations are REST only. Automatic deployments and JWT authorizers are HTTP only |
| Endpoints | `{api-id}.execute-api.{region}.amazonaws.com`. Types are edge-optimized (**default**) / Regional / private. Private is reachable only through an interface VPC endpoint and supports only TLS 1.2 |
| Authorization | `authorizationType` is **required**, with values `NONE` · `AWS_IAM` (SigV4) · `CUSTOM` (Lambda authorizer, `REQUEST` recommended) · `COGNITO_USER_POOLS`. AWS WAF is evaluated **before** all of them |
| Integrations | Proxy (`AWS_PROXY`, `HTTP_PROXY`) sets no request or response; non-proxy (`AWS`, `HTTP`) configures both; `MOCK` calls no backend. **Lambda proxy is recommended** and a format violation returns `502`. Private integration is `HTTP_PROXY` + `VPC_LINK` |
| Data transformation | The recommended order is **proxy integration → parameter mapping → mapping template**. Mapping templates use VTL plus JSONPath, keyed on `Content-Type`. Iterate with `#foreach` ... `#end` |
| Validation and models | Models use JSON schema draft 4. Parameter validation checks **existence only**. `validateRequestBody` and `validateRequestParameters` are **Booleans**. Failure returns `400` |
| Stages | A stage is a **named reference** to a deployment (a snapshot). It configures caching, throttling, logging, stage variables, and the canary, and changes require a **redeployment** |
| Stage variables | `$stageVariables.name`. A Lambda alias is `function:name:${stageVariables.variable}`. Same account only, no sensitive data |
| Canary | **`canarySettings` on the same stage.** `percentTraffic` is a Double (0.0–100.0) and `useStageCache` is a Boolean. Promotion copies the deployment ID and variables and sets the percentage to 0.0, and the canary remains |
| Caching | Per stage. Default TTL 300 seconds / maximum 3600 / `0` disables. **Only `GET` by default**, cache keys required, maximum response 1 MiB, changing capacity deletes data, invalidate with `Cache-Control: max-age=0` |
| Throttling | **Four** limit types (AWS / account / API and stage / client). 10,000 RPS per account per Region with a 5,000 burst. Exceeding returns `429`. All best-effort |
| Usage plans | API keys identify clients and **are not for authentication or authorization.** Use AWS Budgets for cost control and AWS WAF for blocking |
| Quotas | **Integration timeout 29 seconds**, **payload 10 MB**, per API 300 resources · 10 stages · 10 authorizers · 400 KB models |
| Testing | The console **Test tab** or `test-invoke-method`. It bypasses the invoke URL and works before deployment, but **the results are real** and only the logs are simulated |
| API as code | **OpenAPI v2.0 and v3.0**. In `x-amazon-apigateway-integration`, `type` is lowercase (such as `aws_proxy`), Lambda invocation uses `httpMethod: POST`, and `timeoutInMillis` maxes at 29,000 |
| Observability | Execution logging (managed by API Gateway, REST only) and access logging (managed by the developer) are independent. Seven metrics every minute, with method-level metrics **charged**. X-Ray is enabled per stage |
