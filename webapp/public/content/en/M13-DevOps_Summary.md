...
| Checkpoint | 29–30 | [Section 12](#12-knowledge-check-and-summary) |
| Summary | 31–33 | [Section 1.2](#12-terminology) · [Section 12](#12-knowledge-check-and-summary) |
...
| All knowledge check answer boxes are empty (slide 30) | All six answer text boxes for the six true/false questions are empty | The check marks are present only as animations or images, so **the extracted text cannot tell which answer is correct and it can only be determined from the instructor notes.** This is the same structure as M12 slide 29. This document presents each question with its answer and explanation ([Section 12](#12-knowledge-check-and-summary)) | — |
...
The questions from courseware slide 30 appear **exactly as written.** **All six answers are the same as the courseware's.**

All six answer text boxes in the courseware slide body are **empty**(the check marks exist only as animations or images), so the answers were **determined from the instructor notes** ([Section 11.1](#111-courseware-statements-that-do-not-match-the-facts)).

**Question 1**: Human error is a risk associated with traditional software deployment.

- ✅ **Answer: True** — The courseware explanation is "if some deployment processes are manual, human error can occur." The countermeasures verified in this document are **review before execution** through change sets (`--no-execute-changeset`) and **automatic rollback** on deployment failure. ([Section 2.1](#21-problems-with-traditional-software-deployment) · [Section 9.5](#95-change-sets-and-rollback))

**Question 2**: Infrastructure as code is an important DevOps practice.

- ✅ **Answer: True** — The courseware explanation is "infrastructure should be defined in code and treated as software." The `What is DevOps?` page also presents infrastructure as code as an infrastructure automation practice, and that page presents **configuration management** alongside it. ([Section 2.5](#25-devops-practices) · [Section 3.1](#31-infrastructure-as-code))

**Question 3**: AWS CodePipeline can orchestrate AWS CodeBuild and AWS CodeDeploy.

- ✅ **Answer: True** — CodePipeline is a continuous delivery service used to **model, visualize, and automate** the steps required to release software. The documentation's `DevOps pipeline example` places a **CodeBuild build action → a CodeBuild unit test action → a deploy action → a CodeBuild integration test action** after the source stage, and the CodeBuild documentation states that CodeBuild can be added to a pipeline as a build or test action. That confirms the point that CodePipeline orchestrates other services as stage actions. ([Section 3.2](#32-aws-tools-for-devops))

**Question 4**: An AWS SAM template is an extension of an AWS CloudFormation template.

- ✅ **Answer: True** — The documentation lists **`An extension of CloudFormation`** as one of the AWS SAM template specification's characteristics and states that CloudFormation syntax and AWS SAM syntax can be used together in the same template. The courseware explanation ("AWS SAM transforms its own template into an AWS CloudFormation template and then processes it") also matches the documentation's **Transformational** characteristic. ([Section 4.3](#43-how-it-works-transformation))

**Question 5**: You can build serverless applications with AWS SAM only if they are written in Node.js, Java, Python, or Go.

- ❌ **Answer: False** — The courseware explanation is "you can use any runtime that AWS Lambda supports." That holds up against the documentation as well. The `Runtime` value on `AWS::Serverless::Function` is **passed directly to the `Runtime` property of `AWS::Lambda::Function`**, so any runtime Lambda supports can be used, and specifying the `provided` identifier allows even **custom runtimes.**

  🆕 **Two additions.** ① With `PackageType: Image` you can also define **functions packaged as container images**, which makes the range of choices broader than the runtime list. ② The list of values `sam init --runtime` accepts **has no Go-specific identifier**, and Go functions are built with `provided`-family runtimes, so **do not read the answer options (Node.js · Java · Python · Go) as runtime identifiers.** ([Section 5.6](#56-function-permissions-policies-and-role) · [Section 8.4](#84-sam-init))

  Note also that the latter part of the courseware explanation ("there are various AWS toolkits that work with a variety of IDE and runtime combinations") **is content with no corresponding slide in the deck** ([Section 11.1](#111-courseware-statements-that-do-not-match-the-facts)).

**Question 6**: The `sam build` command initializes a new AWS SAM project with the required parameters.

- ❌ **Answer: False** — The courseware explanation is "the `sam init` command starts a new AWS SAM project." Per the documentation, **`sam build` prepares the application for the next step**, such as local testing or deployment, while **`sam init` initializes a new serverless application.** ([Section 8.4](#84-sam-init) · [Section 9.2](#92-sam-build-and-container-builds))
...
**Question 7**: The transform declaration in a SAM template may be written in lowercase, as `Transform: AWS::serverless-2016-10-31`.

- ❌ **Answer: False** — The value the documentation specifies is **`AWS::Serverless-2016-10-31`**, with a capital `S`. Courseware slide 14 writes a lowercase `s`, while the `Type: AWS::Serverless::Function` in the same code block is capitalized, so the notation splits inside the courseware. ([Section 5.2](#52-the-transform-declaration))

> — Source: [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html)

**Question 8**: The required sections in a SAM template are `Transform` and `Resources`.

- ✅ **Answer: True** — The documentation states **"Only the Transform and Resources sections are required."** `Globals`, `Description`, `Metadata`, `Parameters`, `Mappings`, `Conditions`, and `Outputs` are all optional. The courseware does not distinguish required from optional. ([Section 5.1](#51-template-sections))

> — Source: [AWS SAM template anatomy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy.html)

**Question 9**: The `Globals` section is where you declare global variables for use in the template.

- ❌ **Answer: False** — `Globals` is the section where you declare **properties that several resources have in common** once and let them inherit. The section that parameterizes values is **`Parameters`.** Courseware slide 14's annotation says "global variables," but what the same slide puts inside `Globals` is `MethodSettings` on `Api` — a common resource property value, not a variable. ([Section 5.3](#53-the-globals-section))

> — Source: [Globals section of the AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.html)

**Question 10**: A property declared in `Globals` can be removed by an individual resource.

- ❌ **Answer: False** — A resource can **give a new value** to a property declared in `Globals` **but cannot remove it.** That is why the documentation advises not declaring in `Globals` a property that only some resources use. The override rules also differ by data type: primitives replace, maps merge (with the resource entry winning on duplicate keys), and lists have the `Globals` items prepended. ([Section 5.3](#53-the-globals-section))

> — Source: [Globals section of the AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.html)

**Question 11**: You cannot use `sam build --use-container` for functions written in .NET or Python.

- ❌ **Answer: False** 🔄 — **Courseware slide 23's instructor note says this, but the current `sam build` reference contains no such statement.** On the contrary, the `--build-image` description gives `sam build --use-container --build-image amazon/aws-sam-cli-build-image-python3.8` as an example. `--use-container` is the option that builds your function in a Lambda-like Docker container when it depends on packages with **natively compiled dependencies.** ([Section 9.2](#92-sam-build-and-container-builds))

> — Source: [sam build](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-build.html)

**Question 12**: `--use-container` and `--build-in-source` can be used together.

- ❌ **Answer: False** — This is precisely **the only incompatibility** the documentation states for `--use-container`. Conversely, `--build-image`, `--container-env-var`, and `--container-env-var-file` **error out without `--use-container`.** The supported runtimes for `--build-in-source` are all Node.js runtimes `sam init --runtime` supports, and the supported build methods are Makefile and esbuild. ([Section 9.2](#92-sam-build-and-container-builds))

> — Source: [sam build](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-build.html)

**Question 13**: To deploy with `sam deploy` you must first create artifacts with `sam package`.

- ❌ **Answer: False** — The documentation states in a Note that **`sam deploy` implicitly performs the functionality of `sam package`.** You can use the `sam deploy` command directly to package and deploy. Create the bucket automatically with **`--resolve-s3`**; specifying it together with `--s3-bucket` produces an error. That said, `sam package` has not been retired and is still used when you need the packaged template as a file (`--output-template-file`). ([Section 9.3](#93-sam-package-is-no-longer-a-separate-step))

> — Source: [sam package](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-package.html)

**Question 14**: `sam deploy --template-file deploy.yml` deploys using a configuration file.

- ❌ **Answer: False** 🔄 — **Courseware slide 24 attaches the comment "# Deploy using a configuration file" to this example, but** `--template-file`(`--template`, `-t`) specifies the path and name of the **AWS SAM template.** The configuration file is specified with **`--config-file`**, defaults to `samconfig.toml`, and its environment name is chosen with `--config-env`(default `default`). ([Section 9.4](#94-sam-deploy-and-samconfigtoml))

> — Source: [sam deploy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-deploy.html)

**Question 15**: The answers from `sam deploy --guided` are recorded in the project's `samconfig.toml` and reused on later deployments.

- ✅ **Answer: True** — The AWS SAM CLI records the interactive flow's answers in `samconfig.toml`, and **on later deployments running just `sam deploy` uses those values.** To reconfigure them, run `sam deploy --guided` again or edit the file directly. The interactive flow's defaults come from `~/.aws/config`, `~/.aws/credentials`, and the project's `samconfig.toml`. **The courseware never mentions this file.** ([Section 9.4](#94-sam-deploy-and-samconfigtoml))

> — Source: [Introduction to deploying with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-deploy.html)

**Question 16**: `sam deploy` updates the CloudFormation stack directly, so it does not go through a change set.

- ❌ **Answer: False** — Deployment **goes through a CloudFormation change set.** `--confirm-changeset` displays a confirmation prompt before execution, and `--no-execute-changeset` creates the change set and exits. The `Confirm changes before deploy` prompt in `sam deploy --guided` is exactly that choice. **The courseware does not cover change sets at all.** ([Section 9.5](#95-change-sets-and-rollback))

> — Source: [Update CloudFormation stacks using change sets](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-updating-stacks-changesets.html)

**Question 17**: Creating and reviewing a change set guarantees that the stack update will succeed.

- ❌ **Answer: False** — The documentation states that a change set **does not guarantee that the stack update will succeed.** During change set creation it performs **pre-deployment validation** for common failure causes such as property syntax errors, resource name conflicts, and service quota limits, but failures caused by **runtime conditions** such as custom resource logic or service-specific constraints can still occur during execution. ([Section 9.5](#95-change-sets-and-rollback))

> — Source: [Update CloudFormation stacks using change sets](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-updating-stacks-changesets.html)

**Question 18**: Specifying `--disable-rollback` together with `--on-failure` gives finer control over failure behavior.

- ❌ **Answer: False** — **The two options cannot be used together.** The default behavior is that the stack is **rolled back to the last stable state** if an error occurs during deployment, and the values for `--on-failure` are `ROLLBACK`(default), `DELETE`, and `DO_NOTHING`(the same effect as `--disable-rollback`). ([Section 9.5](#95-change-sets-and-rollback))

> — Source: [sam deploy](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-deploy.html)

**Question 19**: If you specify both `Role` and `Policies` on a function, the two sets of permissions are combined.

- ❌ **Answer: False** — The documentation states that **"if you set the Role property, this property (`Policies`) is ignored."** Without `Role`, a role with the logical ID `<function-logical-id>Role` is created automatically, and `RolePath` and `PermissionsBoundary` work **only when the role is created automatically.** The courseware uses `Policies` on slide 14 and `Role` on slide 15 without explaining the difference. ([Section 5.6](#56-function-permissions-policies-and-role))

> — Source: [AWS::Serverless::Function](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-function.html)

**Question 20**: A SAM policy template that needs no placeholder values can be given by name alone.

- ❌ **Answer: False** — A policy template that needs no placeholders **must still be given an empty object (`{}`).** Omitting it makes `sam build` fail with `Must specify valid parameter values for policy template '<policy-template-name>'`. Regular IAM policies and AWS managed policies pass straight through to CloudFormation and can be used without an empty object, and a single `Policies` list can mix both. ([Section 5.7](#57-sam-policy-templates))

> — Source: [AWS SAM policy templates](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-policy-templates.html)

**Question 21**: A function that deletes DynamoDB items can be given `DynamoDBReadPolicy`.

- ❌ **Answer: False** — Deleting requires **write permission**, so `DynamoDBCrudPolicy`(create, read, update, delete) or `DynamoDBWritePolicy`(write only) is correct. Courseware slide 15 attaches a **read role** named `DynamoDBReadRole` to the Delete function, so the name and the purpose disagree. ([Section 5.5](#55-correcting-the-slide-15-template) · [Section 5.7](#57-sam-policy-templates))

> — Source: [AWS SAM policy templates](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-policy-templates.html)

**Question 22**: AWS SAM currently provides six `AWS::Serverless::*` resource types.

- ❌ **Answer: False** — There are **thirteen.** All six from courseware slide 16 (`Api` · `HttpApi` · `Function` · `LayerVersion` · `SimpleTable` · `StateMachine`) are still valid, and `Application` · `CapacityProvider` · **`Connector`** · `GraphQLApi` · `WebSocketApi` · `MicrovmImage` · `NetworkConnector` were added. ([Section 6.1](#61-thirteen-resource-types))

> — Source: [AWS SAM resources and properties](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-specification-resources-and-properties.html)

**Question 23**: The `Permissions` property of `AWS::Serverless::Connector` takes an arbitrary list of IAM actions.

- ❌ **Answer: False** — The valid values are only **`Read` and `Write`.** `Read` includes IAM actions that allow reading data from the resource, and `Write` includes IAM actions that allow initiating or writing data to it. In other words, **you declare the intent and SAM turns it into the IAM permissions needed.** The documentation recommends the **embedded connector syntax** for most use cases. ([Section 6.3](#63-connectors))

> — Source: [AWS::Serverless::Connector](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-connector.html)

**Question 24**: `AWS::Serverless::HttpApi` can control API access with IAM permissions.

- ❌ **Answer: False** — **IAM permissions are supported only by `AWS::Serverless::Api`.** The empty IAM permissions cell for `HttpApi` in courseware slide 17's table is correct, and **all six rows of that table match the current documentation table exactly.** `HttpApi` supports Lambda authorizers, Amazon Cognito user pools (as a JWT issuer), and OAuth 2.0/JWT authorizers. ([Section 7.1](#71-the-six-mechanisms))

> — Source: [Control API access with your AWS SAM template](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-controlling-access-to-apis.html)

**Question 25**: The property name for declaring API authorization in a SAM template is `Auth`.

- ✅ **Answer: True** — You put `Auth` inside the `Properties` of `AWS::Serverless::Api` and declare **`DefaultAuthorizer`** and **`Authorizers`** under it (the data type name is `ApiAuth`). The function's `Api` event references that API with **`RestApiId`.** **Courseware slide 17 says only "you must enable authorization in the AWS SAM template" and gives no property name at all.** ([Section 7.3](#73-declaring-it-with-the-auth-property))

> — Source: [Amazon Cognito user pool example for AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-controlling-access-to-apis-cognito-user-pool.html)

**Question 26**: Docker is listed as a requirement on the AWS SAM CLI prerequisites page.

- ❌ **Answer: False** — **The current prerequisites page has no Docker item.** Instead, (optional) **AWS Toolkit for VS Code** installation is step 3. That said, `sam local invoke` **builds and invokes the function in a local container using Docker**, so Docker is effectively required for local testing. That is a different placement from courseware slide 19 listing Docker as "optional." ([Section 8.1](#81-prerequisites))

> — Source: [AWS SAM prerequisites](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/prerequisites.html)

**Question 27**: Using the AWS SAM CLI requires the AWS CLI.

- ✅ **Answer: True** — The documentation states that using the AWS SAM CLI requires an AWS account, IAM credentials, an IAM access key pair, and **the AWS Command Line Interface (AWS CLI)** to configure AWS credentials, and makes installing the AWS CLI step 1. **The courseware does not mention the AWS CLI.** ([Section 8.1](#81-prerequisites))

> — Source: [AWS SAM prerequisites](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/prerequisites.html)

**Question 28**: On macOS, the AWS-managed Homebrew installer is the recommended path for installing the AWS SAM CLI.

- ❌ **Answer: False** — **Since September 2023 AWS no longer maintains the AWS-managed Homebrew installer.** Use the first-party installation method for your operating system. The AWS SAM CLI also **does not support versions older than macOS 13.x.** ([Section 8.2](#82-changes-in-the-installation-path))

> — Source: [AWS SAM prerequisites](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/prerequisites.html)

**Question 29**: `sam local` has three subcommands: `invoke`, `start-api`, and `generate-event`.

- ❌ **Answer: False** — There are **six.** In addition to the courseware's three there are `start-lambda`(a local HTTP server **for use with the AWS CLI and SDKs**), `callback`, and `execution`. The full CLI also has **twenty-four** commands rather than the courseware's seven. ([Section 8.3](#83-twenty-four-cli-commands) · [Section 8.6](#86-six-sam-local-subcommands))

> — Source: [AWS SAM CLI command reference](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-command-reference.html)

**Question 30**: The command that performs the "verify that the template is written to specification" capability mentioned on courseware slide 12 is `sam validate`.

- ✅ **Answer: True** — `sam validate` verifies whether an AWS SAM template file is valid. **With `--lint` it additionally performs `cfn-lint` linting**, and extra parameters are specified in a `cfnlintrc` configuration file. **The courseware never gives this command name.** ([Section 8.5](#85-sam-validate))

> — Source: [sam validate](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-validate.html)

**Question 31**: The `python3.8` runtime used on courseware slides 14, 15, and 22 is still supported.

- ❌ **Answer: False** — The **Deprecated runtimes table** in the Lambda runtimes documentation lists a deprecation date of **October 14, 2024**(with block function create on February 1, 2027 and block function update on March 3, 2027). `sam init --runtime` still lists this value as allowed, but a function created with it uses a deprecated runtime. The currently supported Python runtimes are `python3.15`, `python3.14`, `python3.13`, `python3.12`, `python3.11`, and `python3.10`. ([Section 11.3](#113-discouraged-and-unsupported-items))

> — Source: [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html)

**Question 32**: The `python3.9` runtime shown in the `sam build` output on courseware slide 21 is still supported.

- ❌ **Answer: False** — The same Deprecated runtimes table lists a deprecation date of **December 15, 2025.** Slides 14, 15, and 22 in the same deck use a different runtime, so **the runtime notation splits inside the courseware.** Reproducing the demonstration verbatim means using a deprecated runtime. ([Section 11.3](#113-discouraged-and-unsupported-items))

> — Source: [Lambda runtimes](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html)

**Question 33**: You can choose in-place deployment on the AWS Lambda compute platform.

- ❌ **Answer: False** — The documentation states that **"AWS Lambda and Amazon ECS deployments cannot use an in-place deployment type"** and that **"all AWS Lambda and Amazon ECS deployments are blue/green."** In-place deployment is **exclusive to the EC2/on-premises compute platform**(the courseware is correct on that point). So `Canary`, `Linear`, and `All-at-once` are **deployment configurations that shift traffic within a Lambda blue/green deployment.** ([Section 10.1](#101-the-two-deployment-types))

> — Source: [Overview of CodeDeploy deployment types](https://docs.aws.amazon.com/codedeploy/latest/userguide/welcome.html)

**Question 34**: CodeDeploy has two compute platforms, EC2/on-premises and AWS Lambda.

- ❌ **Answer: False** — There are **three**: EC2/On-Premises, AWS Lambda, and **Amazon ECS.** The courseware does not mention ECS. The `canary`, `linear`, and `all-at-once` deployment configurations exist **on the ECS platform** as well as on Lambda. ([Section 10.2](#102-the-three-compute-platforms))

> — Source: [CodeDeploy primary components](https://docs.aws.amazon.com/codedeploy/latest/userguide/primary-components.html)

**Question 35**: An EC2/on-premises deployment fails if you do not specify a deployment configuration.

- ❌ **Answer: False** — If you do not specify one, CodeDeploy uses **`CodeDeployDefault.OneAtATime`.** The predefined configurations for EC2/on-premises are `CodeDeployDefault.AllAtOnce`, `HalfAtATime`, and `OneAtATime`, and a deployment configuration on this platform specifies the **minimum healthy hosts**(and an optional minimum healthy hosts per zone) — the number or percentage of instances that must remain available during the deployment. **The courseware does not cover this default.** ([Section 10.3](#103-predefined-deployment-configurations))

> — Source: [Working with deployment configurations in CodeDeploy](https://docs.aws.amazon.com/codedeploy/latest/userguide/deployment-configurations.html)

**Question 36**: The Canary deployment configuration shifts traffic in 2% increments.

- ❌ **Answer: False** 🔄 — **The courseware instructor note's "traffic is shifted in 2 increments at a time" is a mistranslation.** The original means traffic is shifted **in two increments.** For example `CodeDeployDefault.LambdaCanary10Percent10Minutes` **shifts 10% in the first increment and deploys the remaining 90% ten minutes later.** The later sentence in the same instructor note correctly assumes two increments. ([Section 10.3](#103-predefined-deployment-configurations))

> — Source: [Working with deployment configurations in CodeDeploy](https://docs.aws.amazon.com/codedeploy/latest/userguide/deployment-configurations.html)

**Question 37**: You can write `All-at-once` for `DeploymentPreference.Type` in a SAM template.

- ❌ **Answer: False** — The SAM spelling is **`AllAtOnce`**, one word with no hyphens. Courseware slide 26 writes `All-at-once`, so copying it verbatim produces an invalid template. The nine predefined `Type` values are `Canary10Percent5Minutes`, `Canary10Percent10Minutes`, `Canary10Percent15Minutes`, `Canary10Percent30Minutes`, `Linear10PercentEvery1Minute`, `Linear10PercentEvery2Minutes`, `Linear10PercentEvery3Minutes`, `Linear10PercentEvery10Minutes`, and `AllAtOnce`. ([Section 10.5](#105-deploymentpreference-fields))

> — Source: [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html)

**Question 38**: If you put both `AutoPublishAlias` and `DeploymentPreference` on a new function and deploy once, the gradual shift happens from the first deployment.

- ❌ **Answer: False** — Gradual deployment **requires a previous version for CodeDeploy to shift traffic from**, so the first deployment must be done in two steps. **Step 1 deploys with `AutoPublishAlias` only** to create the alias, and **step 2 adds `DeploymentPreference`** to perform the gradual deployment. **This constraint is absent from the courseware.** ([Section 10.6](#106-the-first-gradual-deployment-takes-two-steps))

> — Source: [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html)

**Question 39**: Specifying CloudWatch alarms on `DeploymentPreference` makes the deployment roll back automatically when an alarm is triggered.

- ✅ **Answer: True** — `Alarms` is a list of CloudWatch alarms triggered by errors the deployment causes, and when triggered the deployment **rolls back automatically.** The companion `Hooks` are validation functions run before traffic shifting starts (`PreTraffic`) and after it completes (`PostTraffic`); **both must call back to CodeDeploy with success or failure**, and on failure the deployment stops and reports failure to CloudFormation. These two are the answer to the "no rollback plan" problem raised on courseware slide 5. ([Section 10.5](#105-deploymentpreference-fields))

> — Source: [Deploying serverless applications gradually with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/automating-updates-to-serverless-apps.html)

**Question 40**: A Lambda alias can split traffic across three or more function versions.

- ❌ **Answer: False** — An alias can point to **at most two** function versions. The two versions must share an execution role, must have the same dead-letter queue configuration or none, and **must both be published versions — `$LATEST` cannot be used.** Also, because Lambda uses a **simple probabilistic model** to distribute traffic, at low traffic the actual ratio can deviate substantially from the configured one. ([Section 10.7](#107-weighted-alias-routing))

> — Source: [Implement Lambda canary deployments using a weighted alias](https://docs.aws.amazon.com/lambda/latest/dg/configuring-alias-routing.html)

**Question 41**: `sam sync --watch` is a command for speeding up production deployments.

- ❌ **Answer: False** — The documentation **recommends `sam sync` for development environments** and **recommends `sam deploy` or a CI/CD pipeline configuration for production environments.** Running it produces a confirmation prompt stating that the command should be used only on development stacks. In other words the courseware's four-step workflow remains valid as a production path, and `sam sync` is a separate path for the development loop. ([Section 9.6](#96-reducing-the-development-loop-with-sam-sync))

> — Source: [Introduction to using sam sync to sync to AWS Cloud](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/accelerate.html)

**Question 42**: Continuous delivery must go through a human manual approval before deploying to production.

- ❌ **Answer: False** 🔄 — **Courseware slide 7's instructor note asserts this, but** the documentation states that before the final push to production **a person, an automated test, or a business rule** determines when that push happens. Manual approval is therefore not required. The documentation also states that while every successful change can be released immediately, **not every change has to be released right away.** ([Section 2.3](#23-the-three-devops-processes))

> — Source: [Continuous delivery and continuous integration](https://docs.aws.amazon.com/codepipeline/latest/userguide/concepts-continuous-delivery-integration.html)

**Question 43**: To clean up deployed lab and demonstration resources you must find and delete the stack in the console.

- ❌ **Answer: False** — **`sam delete`** deletes the CloudFormation stack, the artifacts packaged and deployed to Amazon S3 and Amazon ECR, and the AWS SAM template file. If an ECR companion stack exists it asks whether to delete it, and with `--no-prompts` **the companion stack and ECR repository are deleted by default** while the stack name must be provided through `--stack-name` or a configuration TOML file. **The courseware never covers cleanup.** ([Section 9.8](#98-cleaning-up))

> — Source: [sam delete](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-delete.html)

**Question 44**: The resources and endpoints created by a deployment can be checked only in the console.

- ❌ **Answer: False** — **`sam list`** has three subcommands for this: `resources`(resources that will be created at deployment), `endpoints`(cloud and local endpoints), and `stack-outputs`(stack outputs). The documentation states that the command is used **both before and after** deployment, so you can preview what will be created before deploying. ([Section 9.7](#97-verifying-deployment-results))

> — Source: [sam list](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-cli-command-reference-sam-list.html)

**Question 45**: AWS SAM can define only serverless resources, so it cannot be used with an existing CloudFormation template.

- ❌ **Answer: False** — The documentation defines the SAM template specification as **`An extension of CloudFormation`** and states that both syntaxes can be used in the same template. **CloudFormation resources and AWS SAM resources can be mixed** in the `Resources` section, and one of the use scenarios is **"augmenting CloudFormation" — adding serverless components to an existing CloudFormation template.** The courseware slide 16 instructor note says the same in its own way: "an AWS SAM template can define any AWS CloudFormation resource." ([Section 3.3](#33-comparing-iac-tools) · [Section 5.1](#51-template-sections))

> — Source: [What is the AWS Serverless Application Model (AWS SAM)?](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam.html)

**Question 46**: `sam deploy` reads source files directly, so the latest code is deployed even if you skip `sam build`.

- ❌ **Answer: False** — `sam deploy` deploys **the build artifacts in the `.aws-sam` directory.** That is why the documentation gives it as a best practice to **run `sam build` to refresh `.aws-sam` before deploying** after changing source files. The build output goes to `.aws-sam/build` and the built template to `.aws-sam/build/template.yaml`. ([Section 9.2](#92-sam-build-and-container-builds) · [Section 9.4](#94-sam-deploy-and-samconfigtoml))

> — Source: [Introduction to deploying with AWS SAM](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-deploy.html)

**Question 47**: The AWS SAM CLI performs the SAM template transformation locally.

- ❌ **Answer: False** — The documentation states that the transformation is performed **during deployment.** When the template is deployed to CloudFormation, CloudFormation reads the `Transform` declaration and applies the serverless transform, which is why that declaration is needed. The documentation's example states that **a 23-line SAM template expands into over 200 lines** of CloudFormation syntax. ([Section 4.3](#43-how-it-works-transformation) · [Section 5.2](#52-the-transform-declaration))

> — Source: [How AWS SAM works](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam-overview.html)

**Question 48**: A DynamoDB table created by `AWS::Serverless::SimpleTable` uses provisioned capacity mode.

- ❌ **Answer: False** — The transformation result is an `AWS::DynamoDB::Table` with **a string attribute using `id` as the `HASH` key** and **`BillingMode` `PAY_PER_REQUEST`**(on-demand). If you need a composite key or secondary indexes, declare `AWS::DynamoDB::Table` directly in the same template. ([Section 6.4](#64-what-simpletable-creates))

> — Source: [How AWS SAM works](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/what-is-sam-overview.html)

**Question 49**: `sam local generate-event` supports three services: Amazon S3, API Gateway, and Amazon SNS.

- ❌ **Answer: False** — Courseware slide 20 names three as "representative services," but the actual list is far longer. The documentation's output example alone shows `alb`, `alexa-skills-kit`, `alexa-smart-home`, `apigateway`, `appsync`, `batch`, and `cloudformation`, and the list continues past that. **That said, the documentation example is cut off with an ellipsis so we could not verify the full list** ([Section 11.5](#115-items-we-could-not-verify)). Also, giving a service name lists the event types and giving a service plus a type prints a sample event. ([Section 8.8](#88-sam-local-generate-event))

> — Source: [Introduction to testing with sam local generate-event](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/using-sam-cli-local-generate-event.html)

**Question 50**: To share a SAM application only within your team, you have to build the infrastructure for it yourself.

- ❌ **Answer: False** — The **AWS Serverless Application Repository** lets you share serverless applications **publicly** with the whole community or **privately** within a team or organization. You upload a **manifest file (an AWS SAM template)** along with the code, and the `sam publish` command and the `AWS::Serverless::Application` resource type connect to it. **The courseware does not cover this service at all.** ([Section 4.5](#45-aws-serverless-application-repository))
...
- ✅ Identify risks associated with traditional software development practices — [Section 2.1](#21-problems-with-traditional-software-deployment)
- ✅ Explain how DevOps addresses the risks associated with traditional software development practices — [Section 2](#2-devops) · [Section 3](#3-infrastructure-as-code-and-deployment-tools)
- ✅ Construct an AWS SAM template to deploy a serverless application — [Section 5](#5-the-sam-template) · [Section 6](#6-sam-resource-types-and-connectors) · [Section 7](#7-api-access-control)
- ✅ Describe the various AWS SAM deployment strategies — **AWS SAM is absent from the courseware's deployment strategies section, so [Section 10](#10-deployment-strategies) fills it in from official documentation**

### One-Page Summary

| Topic | What to remember |
|---|---|
| The three DevOps processes | **Continuous integration** focuses on automatically building and testing code; **continuous delivery** automates the entire release process to production. The final push is decided by **a person, an automated test, or a business rule**, so **manual approval is not required.** Only the abbreviation `CI` is used |
| DevOps practices | Continuous integration · continuous delivery · microservices · infrastructure as code · monitoring and logging · communication and collaboration. The documentation adds **configuration management** |
| What AWS SAM is | **An open-source framework for building serverless applications with infrastructure as code.** Its components are **the template specification plus the CLI** |
| Transformation | A SAM template is **an extension of CloudFormation** and the transformation happens **during deployment.** 23 lines expand into over 200 lines of CloudFormation |
| Required sections | **Only `Transform` and `Resources`.** The value is **`AWS::Serverless-2016-10-31`**(capital `S`) |
| `Globals` | Declares **properties several resources have in common** once for inheritance. It is not a variable declaration, and **a declared property cannot be removed by a resource.** Value parameterization belongs to `Parameters` |
| Function permissions | **Setting `Role` makes `Policies` ignored.** `Policies` accepts policy templates, managed policy ARNs, managed policy names, and inline policies, and **templates with no placeholder need `{}`** |
| Two ways to narrow permissions | **Policy templates**(`DynamoDBReadPolicy` with `TableName`) and **connectors**(`Read` / `Write` in a `Connectors` block). A managed policy name opens up every table in the account |
| Resource types | **13.** The courseware's 6 plus `Application` · `CapacityProvider` · `Connector` · `GraphQLApi` · `WebSocketApi` · `MicrovmImage` · `NetworkConnector` |
| API access control | **All six rows of courseware slide 17's table match current documentation.** **IAM permissions, API keys, and resource policies are `Api` only**; OAuth 2.0/JWT is `HttpApi` only. The declaration property is **`Auth`**(`DefaultAuthorizer` + `Authorizers`) |
| CLI commands | **24.** The courseware's 7 plus `validate` · `sync` · `delete` · `list` · `logs` · `traces` · `publish` · `pipeline` and the `remote` family. `sam local` has **6** subcommands |
| Installation prerequisites | **The AWS CLI is required.** Docker is **not** on the prerequisites list but is effectively required for local testing. (Optional) AWS Toolkit for VS Code. **The AWS-managed Homebrew installer ended in 2023-09**, and macOS earlier than 13.x is unsupported |
| Building | The **only stated restriction on `--use-container`(`-u`) is incompatibility with `--build-in-source`.** There is **no** statement about unsupported languages. Output goes to `.aws-sam/build` |
| Deploying | **`sam deploy` performs packaging implicitly.** `--stack-name` is required and the bucket can be created with `--resolve-s3`. **The configuration file is `--config-file`**(default `samconfig.toml`) |
| Safety mechanisms | Deployment goes through a **change set.** `--no-execute-changeset` inspects without applying and `--confirm-changeset` prompts. Rollback is the default behavior, controlled with `--on-failure`(`ROLLBACK`/`DELETE`/`DO_NOTHING`). **`--disable-rollback` and `--on-failure` cannot be combined** |
| The development loop | **`sam sync --watch`(AWS SAM Accelerate) is for development environments only.** Production uses `sam deploy` or CI/CD |
| Deployment types | **Lambda and ECS deployments are all blue/green** and cannot use in-place deployment. In-place is **exclusive to EC2/on-premises** |
| Compute platforms | **Three** — EC2/On-Premises · AWS Lambda · **Amazon ECS** |
| Deployment configurations | **Nine** predefined for Lambda (`CodeDeployDefault.Lambda*`). EC2/on-premises has three with a default of **`CodeDeployDefault.OneAtATime`**. Canary shifts **in two increments** |
| SAM deployment strategy | **`AutoPublishAlias` + `DeploymentPreference`**(`Type` · `Alarms` · `Hooks`). The SAM spelling is **`AllAtOnce`**. **The first gradual deployment takes two steps**, and invocations need the **alias qualifier** |
| Rollback paths | The stack uses `--on-failure`, Lambda traffic uses **`Alarms` for automatic rollback**, validation uses `Hooks`, and traffic uses a **weighted alias**(at most two versions, no `$LATEST`) |
| Cleanup | **`sam delete`** removes the stack, artifacts, and template file. `sam list` shows resources, endpoints, and stack outputs |
| Deprecated runtimes | **`python3.8` on 2024-10-14** and **`python3.9` on 2025-12-15.** Reproducing the courseware templates verbatim means using a deprecated runtime |
