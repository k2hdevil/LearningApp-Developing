...
| Checking your knowledge | 31–32 | [Section 11](#11-knowledge-check-and-summary) |
| Summary | 33–36 | [Section 8.3](#83-example-from-dynamodb-getitem-to-lambda-invoke) · [Section 11](#11-knowledge-check-and-summary) |
...
The questions from courseware slide 32, carried over as is.

**Question 1**: With a serverless architecture, teams can focus on the application while the cloud service provider manages the infrastructure.

- ✅ **Answer: True**

**Question 2**: Modern application development is an approach to designing, building, and managing applications in the cloud.

- ✅ **Answer: True**

**Question 3**: In Step Functions, all work in a state machine is performed using AWS Lambda functions.

- ❌ **Answer: False** — All work in a state machine is performed by **tasks**, and a task can invoke a Lambda function or another service.

**Question 4**: A Step Functions state machine supports branching, parallel execution, retry/error handling, and starting tasks.

- ✅ **Answer: True**

**Question 5**: Microservices are loosely coupled, have standalone functionality, and are either event driven or API driven.

- ✅ **Answer: True**

**Question 6**: A Step Functions workflow manages parallelization and service integrations, but you must manage failures and retries yourself.

- ❌ **Answer: False** — The workflow manages **failures, retries, parallelization, service integrations, and observability.**

> — Source: [What is Step Functions?](https://docs.aws.amazon.com/step-functions/latest/dg/welcome.html)

### 🆕 Supplementary Questions (Verifying Updated Content)

**Question 7**: After creating a state machine you can change its workflow type from Standard to Express.

- ❌ **Answer: False** — You must choose Standard (the default) or Express when you create a state machine, and that type **cannot be changed after creation.** ([Section 6.4](#64-workflow-types-standard-and-express))

> — Source: [Choosing workflow type in Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/choosing-workflow-type.html)

**Question 8**: In an Express workflow you can also use the `.sync` suffix to wait until a job completes.

- ❌ **Answer: False** — Express **supports all service integrations but does not support the Run a Job (`.sync`) or Wait for a Callback (`.waitForTaskToken`) patterns.** Distributed Map, Activities, and redrive are also Standard only. ([Section 6.4](#64-workflow-types-standard-and-express) · [Section 8.1](#81-the-three-integration-patterns))

> — Source: [Discover service integration patterns in Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html)

**Question 9**: The Amazon States Language has seven state types: task, choice, succeed or fail, pass, wait, parallel, and map.

- ❌ **Answer: False** — The official workflow state reference list has eight: `Task`, `Choice`, `Parallel`, `Map`, `Pass`, `Wait`, **`Succeed`, and `Fail`.** The courseware groups `Succeed` and `Fail` into one item to present seven. The seven Workflow Studio classifies as Flow states are the ones other than `Task`. ([Section 7.1](#71-the-eight-state-types))

> — Source: [Discovering workflow states](https://docs.aws.amazon.com/step-functions/latest/dg/concepts-states.html)

**Question 10**: In a state machine that uses JSONPath, the fields that control data flow between states are `InputPath`, `Parameters`, `ResultPath`, and `OutputPath` — four fields.

- ❌ **Answer: False** — There are **five**, applied in the order `InputPath` → `Parameters` → (resource invocation) → **`ResultSelector`** → `ResultPath` → `OutputPath`. The courseware lists four in the diagram and three in the instructor notes, which disagree. ([Section 7.3](#73-the-five-jsonpath-data-flow-fields))

> — Source: [Processing input and output in Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/concepts-input-output-filtering.html)

**Question 11**: JSONPath is the recommended choice when creating a new state machine.

- ❌ **Answer: False** — The documentation **recommends JSONata for new state machines.** A state machine that does not specify a query language defaults to JSONPath for backward compatibility, so the courseware examples still work. With JSONata the five JSONPath fields reduce to just `Arguments` and `Output`. ([Section 7.4](#74-jsonata-and-variables))

> — Source: [Transforming data with JSONata in Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/transforming-data.html)

**Question 12**: The output of a state can be referenced from any later step.

- ❌ **Answer: False** — State output is by default passed only to the **immediately following state.** If a later step needs the data, store it in a **variable.** The states that support `Assign` are `Pass`, `Task`, `Map`, `Parallel`, `Choice`, and `Wait`, and up to 256 KiB can be stored per state. ([Section 7.4](#74-jsonata-and-variables))

> — Source: [Passing data between states with variables](https://docs.aws.amazon.com/step-functions/latest/dg/workflow-variables.html)

**Question 13**: In a `Choice` state, if no Choice Rule evaluates to `true` and there is no `Default`, the execution simply ends.

- ❌ **Answer: False** — The state cannot be exited, so **the state machine throws an error.** That is why `Default` is an optional field the documentation recommends. A `Choice` state does not support the `End` field, and `Next` is used only inside `Choices`. ([Section 7.5](#75-the-choice-state))

> — Source: [Choice workflow state](https://docs.aws.amazon.com/step-functions/latest/dg/state-choice.html)

**Question 14**: The `Map` state supports at most 40 concurrent iterations.

- ❌ **Answer: False** — 40 is the limit for **Inline mode.** **Distributed mode** runs each iteration as a child workflow execution and runs **10,000 in parallel** if not specified. Use Distributed when the dataset exceeds 256 KiB, the execution event history exceeds 25,000 entries, or you need more than 40 concurrent iterations. Note that Distributed is Standard only. ([Section 7.6](#76-the-map-state-inline-and-distributed))

> — Source: [Using Map state in Distributed mode](https://docs.aws.amazon.com/step-functions/latest/dg/state-map-distributed.html)

**Question 15**: The field that defines the steps to repeat inside a `Map` state is `Iterator`.

- ❌ **Answer: False** — The `Iterator` field is **deprecated and replaced by `ItemProcessor`**, and the `Parameters` field that overrode array item values is **replaced by `ItemSelector`.** Existing definitions continue to work, but the documentation strongly recommends changing them. The `Iterator` on courseware slide 27 is **the name of a Task state**, not a field of the Map state, so it is unrelated to this deprecation. ([Section 7.6](#76-the-map-state-inline-and-distributed))

> — Source: [Using Map state in Inline mode](https://docs.aws.amazon.com/step-functions/latest/dg/state-map-inline.html)

**Question 16**: If you omit `MaxAttempts` in a `Retry` field, no retry is performed.

- ❌ **Answer: False** — The default value of `MaxAttempts` is **3**. To perform no retry you must explicitly specify **0**. The other defaults are `IntervalSeconds` 1 and `BackoffRate` 2.0. ([Section 7.8](#78-error-handling-retry-and-catch))

> — Source: [Handling errors in Step Functions workflows](https://docs.aws.amazon.com/step-functions/latest/dg/concepts-error-handling.html)

**Question 17**: Specifying `States.ALL` in a `Catch` lets you catch every error that occurs in a workflow.

- ❌ **Answer: False** — `States.ALL` is a wildcard that matches every known error name, but it **cannot catch the `States.DataLimitExceeded` terminal error or `States.Runtime` errors**, and it must appear alone in a catcher. A catcher can also only be used on `Task`, `Parallel`, and `Map` states, not for a top-level state machine execution failure. ([Section 7.8](#78-error-handling-retry-and-catch))

> — Source: [Handling errors in Step Functions workflows](https://docs.aws.amazon.com/step-functions/latest/dg/concepts-error-handling.html)

**Question 18**: A Step Functions task can only call AWS services.

- ❌ **Answer: False** — An **HTTP Task** (`arn:aws:states:::http:invoke`) can call the public APIs of third-party SaaS such as Stripe and Salesforce, as well as private HTTPS APIs inside an Amazon VPC. Authentication is handled by an EventBridge connection and the secret is stored in Secrets Manager. A request times out after 60 seconds, and that is a hard quota. ([Section 8.6](#86-http-task))

> — Source: [Call HTTPS APIs in Step Functions workflows](https://docs.aws.amazon.com/step-functions/latest/dg/call-https-apis.html)

**Question 19**: When you build a state machine with an AWS SDK integration, Step Functions creates the required IAM policy for you.

- ❌ **Answer: False** — Step Functions **cannot automatically generate IAM policies for AWS SDK integrations.** You must configure the role policy yourself after creating the state machine. The `Resource` syntax for an AWS SDK integration is `arn:aws:states:::aws-sdk:serviceName:apiAction`, and parameters are written in PascalCase. ([Section 8.2](#82-optimized-integrations-and-aws-sdk-integrations))

> — Source: [Learning to use AWS service SDK integrations](https://docs.aws.amazon.com/step-functions/latest/dg/supported-services-awssdk.html)

**Question 20**: Adding retries to a workflow step does not change the cost of a Standard workflow.

- ❌ **Answer: False** — Standard is billed by the **number of state transitions**, counting the total **including retries.** If you add retry error handling to a step, **each retry is billed as an additional state transition.** redrive also counts as state transitions. ([Section 6.6](#66-pricing-model))

> — Source: [AWS Step Functions Pricing](https://aws.amazon.com/step-functions/pricing/)

**Question 21**: The only way to deal with a failed Standard workflow execution is to run it again from the beginning.

- ❌ **Answer: False** — **redrive** can restart a Standard execution that did not complete successfully within the last 14 days, **from the failed step with the same input.** The results and execution history of successful steps are preserved and not run again. However, if you updated the state machine definition you must start a new execution, and Express does not support redrive. ([Section 9.4](#94-restarting-a-failed-execution-with-redrive))

> — Source: [Restarting state machine executions with redrive](https://docs.aws.amazon.com/step-functions/latest/dg/redrive-executions.html)

**Question 22**: AWS Step Functions Local is the recommended path for testing a state machine locally before deployment.

- ❌ **Answer: False** — **AWS Step Functions Local is explicitly marked unsupported in the current documentation** and does not provide feature parity (no optimized service integrations, cross-account access, or Distributed Map). The data flow simulator for testing JSONPath syntax is also unsupported. The documentation directs you to unit test state machine logic before deployment with the **`TestState` API**. ([Section 9.1](#91-testing-an-individual-state))

> — Source: [Testing and debugging Step Functions state machines](https://docs.aws.amazon.com/step-functions/latest/dg/test-and-debug.html)

**Question 23**: Amazon CloudWatch Events is a separate service from Amazon EventBridge, so existing code has to be rewritten against the EventBridge API.

- ❌ **Answer: False** — **EventBridge was previously called CloudWatch Events.** The default event bus and the rules created in CloudWatch Events also appear in the EventBridge console, and because it **uses the same API, existing code continues to work.** However, new features added to EventBridge are not added to CloudWatch Events. ([Section 3.6](#36-services-that-implement-event-driven-integration))

> — Source: [EventBridge is the evolution of Amazon CloudWatch Events](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-cwe-now-eb.html)

**Question 24**: An Amazon SQS standard queue delivers each message exactly once, so the application does not need to handle duplicates.

- ❌ **Answer: False** — A standard queue guarantees **at-least-once delivery**, so a message can be delivered more than once and the application must be designed to handle duplicates with **idempotent operations.** Ordering is also **best-effort.** If you need exactly-once processing and guaranteed ordering, use a **FIFO queue.** ([Section 3.6](#36-services-that-implement-event-driven-integration))

> — Source: [Amazon SQS queue types](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-queue-types.html)

**Question 25**: Because it is serverless, a Lambda function scales automatically without limit.

- ❌ **Answer: False** — The account's **total concurrency limit is 1,000 by default per Region** and can be raised on request. Required concurrency is calculated as (average requests per second) × (average request processing time in seconds). Even though each microservice scales independently, they **share the same account concurrency.** ([Section 4.5](#45-automatic-scaling-has-limits-too))

> — Source: [Understanding Lambda function scaling](https://docs.aws.amazon.com/lambda/latest/dg/lambda-concurrency.html)

**Question 26**: If a transaction spanning several services fails partway through, Step Functions automatically rolls back changes that were already committed.

- ❌ **Answer: False** — There is no automatic rollback. Because **two-phase commit (2PC) cannot be used** in a database-per-service design, you have to design the **saga orchestration pattern** yourself, in which the orchestrator runs a **compensatory transaction** to return the data to its initial state. It can be implemented with a Step Functions Standard workflow, and saga participants must be idempotent. ([Section 3.8](#38-distributed-transactions-and-the-saga-orchestration-pattern))

> — Source: [Saga orchestration pattern](https://docs.aws.amazon.com/prescriptive-guidance/latest/cloud-design-patterns/saga-orchestration.html)

**Question 27**: Microservices are always a better choice than a monolith.

- ❌ **Answer: False** — While describing the benefits of microservices, the whitepaper explicitly states that you **must evaluate the requirements and cost of each use case, that in some cases a monolithic architecture or another approach may be more appropriate, and that the decision must be made case by case with the scale, complexity, and specific use case in mind.** The instructor notes for courseware slide 9 also acknowledge that "creating microservices or refactoring an existing monolithic application is not easy." ([Section 2.4](#24-microservices-are-not-always-the-answer))
...
|---|---|
| Modern applications | Microservices architecture + serverless operating model + automated software delivery. But **a monolith is more appropriate in some cases, so decide case by case** |
| Service boundaries | Cut by **business domain**, not by technical layer. Draw boundaries with bounded contexts and do not share domain dependencies across services |
| Integration | API driven (synchronous) · event driven (asynchronous) · data streaming. SQS standard is at-least-once with no ordering guarantee; FIFO is exactly-once with ordering within a group |
| Decoupling | The strangler fig pattern — proxy → pass-through → move to a new service → repeat → retire the legacy system. Distributed transactions use saga orchestration and compensatory transactions |
| Serverless | Provisioning, patching, and scaling go away. Building and deploying the application and application monitoring stay. Lambda concurrency is 1,000 per Region by default |
| Step Functions terms | State machine = workflow, each step = a state, task = a unit of work performed by another service, execution = a running instance. **All work is performed by tasks** |
| Workflow types | **Chosen at creation and cannot be changed.** Standard is 1 year, exactly-once, billed by state transitions; Express is 5 minutes, at-least-once, billed by executions, duration, and memory. Express does not support `.sync`, callbacks, Distributed Map, Activities, or redrive |
| State types | `Task` · `Choice` · `Parallel` · `Map` · `Pass` · `Wait` · `Succeed` · `Fail` — **eight.** Every state other than `Succeed` and `Fail` needs `Next`, and state names are unique across the state machine |
| Data flow | JSONPath applies `InputPath` → `Parameters` → `ResultSelector` → `ResultPath` → `OutputPath` — **five fields, in that order.** New state machines should use **JSONata** (`Arguments`, `Output`). Data needed in later steps goes in a **variable** |
| Error handling | Every state except `Pass` and `Wait` can raise an error. A catcher works on `Task`, `Parallel`, and `Map`. Retrier defaults are `IntervalSeconds` 1 · `MaxAttempts` 3 · `BackoffRate` 2.0. `States.ALL` cannot catch `States.Runtime` or `States.DataLimitExceeded` |
| Integration patterns | Request Response (no suffix) / Run a Job (`.sync`) / Callback (`.waitForTaskToken`). 21 services with optimized integrations + over 200 services through AWS SDK integrations + HTTP Task. **SDK integrations need manual IAM policies** |
| Operations | Test individual states with `TestState` (Step Functions Local is unsupported). Deploy with versions and aliases, and use redrive to resume a failed execution from within 14 days at the failed step |
| Quotas | Definition 1 MB, input/output 256 KiB, Standard execution history 25,000 events, names 80 characters, HTTP Task 60 seconds |
