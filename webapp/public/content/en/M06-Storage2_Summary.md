...
8. [Knowledge Check and Summary](#8-knowledge-check-and-summary)
...
## 8. Knowledge Check and Summary

### Knowledge Check (True/False)

The questions from courseware slide 42, kept as they are.

**Question 1**: For objects larger than 100 MB, you should consider using multipart upload.

- ✅ **Answer: True** — Using multipart upload for objects larger than 100 MB is a best practice. This holds even though the largest object you can upload in a single PUT is 5 GB.

**Question 2**: Some services have APIs that require pagination to access all the data the service returns.

- ✅ **Answer: True** — You reviewed an example of paginated results when listing the contents of a bucket.

**Question 3**: Presigned URLs let you share a specific S3 object through time-limited access.

- ✅ **Answer: True** — With a presigned URL you can grant a user temporary access to a specific S3 object.

**Question 4**: You use S3 Batch Operations to upload a single object as a series of parts to improve throughput.

- ❌ **Answer: False** — S3 multipart upload is what uploads a single object as a series of parts to improve throughput. S3 Batch Operations is for changing object metadata and properties, or performing other storage management tasks such as copying or replicating objects between buckets.

**Question 5**: Amazon S3 event notifications are always sent in response to Amazon S3 operations such as PUT, POST, COPY, or DELETE.

- ❌ **Answer: False** — You can configure a bucket to send Amazon S3 event notifications in response to Amazon S3 operations such as PUT, POST, COPY, or DELETE. 🔄 That said, the supported triggers today are not limited to API calls (see [Section 6.6](#66-event-notification-triggers)).

**Question 6**: A web server uses CORS to allow or deny loading of resources stored within the same domain that accesses the website.

- ❌ **Answer: False** — CORS protects data from being accessed by other domains. In CORS the origin is the web server and the resource is an asset in another domain. In CORS the web server indicates resources other than its own (domain, scheme, or port) that the browser should be allowed to load.

> — Source: [Event types and destinations](https://docs.aws.amazon.com/AmazonS3/latest/userguide/notification-how-to-event-types-and-destinations.html)

### 🆕 Supplementary Questions (Covering the Updated Content)

**Question 7**: A single AWS account can create at most 100 S3 buckets by default, and you can request an increase up to 1,000.

- ❌ **Answer: False** — The current default quota is **10,000** general purpose buckets per account, and anything beyond that is requested in the Service Quotas console. (See [Section 2.2](#22-creating-a-bucket-and-the-per-account-bucket-quota))

> — Source: [Bucket quotas and limitations](https://docs.aws.amazon.com/AmazonS3/latest/userguide/BucketRestrictions.html)

**Question 8**: If `HeadBucket` returns 400, it means you accessed the bucket from a Region other than the one it exists in.

- ❌ **Answer: False** — 400 is one of the general codes (400 / 403 / 404) returned when a bucket does not exist or access is denied. In fact, calling with a bucket name from any Region in the partition still returns the correct bucket location header. (See [Section 2.3](#23-the-headbucket-api-operation))

> — Source: [HeadBucket API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_HeadBucket.html)

**Question 9**: Running `aws s3api get-bucket-versioning` against a bucket that has never had versioning configured returns `"Status": "Disabled"`.

- ❌ **Answer: False** — The versioning status values are only `Enabled` and `Suspended`, and a bucket that has never been configured returns no status value at all. (See [Section 2.9](#29-updating-bucket-versioning))

> — Source: [PutBucketVersioning API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutBucketVersioning.html)

**Question 10**: Multipart upload parts must be at least 5 MiB, and this applies to the last part as well.

- ❌ **Answer: False** — Part size ranges from 5 MiB to 5 GiB, but **there is no minimum size for the last part.** The maximum number of parts per upload is 10,000. (See [Section 3.3](#33-multipart-upload-limits))

> — Source: [Multipart upload limits](https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html)

**Question 11**: If you set a presigned URL's expiration to 7 days, it is valid for 7 days regardless of which credentials signed it.

- ❌ **Answer: False** — A URL created with temporary security credentials **expires when those credentials expire.** An STS `AssumeRole` session lasts 1 hour by default, and EC2 instance profile metadata credentials last up to about 6 hours. The URL's capability is also limited to the permissions of the user who created it. (See [Section 4.2](#42-expiration-and-credential-types))

> — Source: [Presigned URLs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-presigned-url.html)

**Question 12**: You can create a presigned URL for uploads (PUT) with `aws s3 presign`.

- ❌ **Answer: False** — The URL `aws s3 presign` produces is for **retrieving an object with an HTTP GET request**. A PUT upload URL must be created with an SDK (for example, Boto3's `generate_presigned_url(ClientMethod='put_object')`). (See [Section 4.3](#43-creating-a-presigned-url-with-the-aws-cli))

> — Source: [aws s3 presign](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/presign.html)

**Question 13**: An S3 static website endpoint can be reached over HTTPS.

- ❌ **Answer: False** — Website endpoints support neither HTTPS nor access points, and the scheme is `http`. If you need HTTPS, use AWS Amplify Hosting (recommended) or CloudFront. (See [Section 6.2](#62-website-endpoints)) The `https://` in the table on courseware slide 38 is a notation error that contradicts the instructor notes on the same slide.

> — Source: [Website endpoints](https://docs.aws.amazon.com/AmazonS3/latest/userguide/WebsiteEndpoints.html)

**Question 14**: To configure CORS in the S3 console you write an XML file.

- ❌ **Answer: False** — The console now **supports only JSON CORS configurations.** The REST API and SDK paths accept both JSON and XML. (See [Section 6.5](#65-cross-origin-resource-sharing-cors))

> — Source: [Elements of a CORS configuration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ManageCorsUsing.html)

**Question 15**: The object list for an S3 Batch Operations job can only be specified with an S3 Inventory report or a CSV file you create yourself.

- ❌ **Answer: False** — There are four ways today: use an existing manifest, write a new manifest yourself, **generate one from metadata you specify**, and **generate one from an existing replication configuration**. (See [Section 5.6](#56-s3-batch-operations))

> — Source: [Creating a Batch Operations job](https://docs.aws.amazon.com/AmazonS3/latest/userguide/batch-ops-create-job.html)

**Question 16**: When a lifecycle rule expires and deletes an object, an `s3:ObjectRemoved:*` event notification fires.

- ❌ **Answer: False** — `ObjectRemoved` event notifications **do not notify you about automatic deletions from a lifecycle configuration or about failed operations.** To catch lifecycle deletions, use `s3:LifecycleExpiration:*`. (See [Section 6.6](#66-event-notification-triggers))
